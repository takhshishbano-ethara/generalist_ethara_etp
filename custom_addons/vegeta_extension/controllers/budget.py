...
def _project_owns_batches(env, project_id):
    ...
    Batches reference ``vegeta.job`` rows; a project is linked to that
...
def _delivered_batch_count(env, project_id):
    ...
    if not _project_owns_batches(env, project_id):
        return 0
    return env["vegeta.batch.delivery"].sudo().search_count(
...
def _scoped_batches(env, project_id):
    if not _project_owns_batches(env, project_id):
        return env["vegeta.batch.delivery"].sudo().browse()
    return env["vegeta.batch.delivery"].sudo().search(
...
class VegetaBudgetController(http.Controller):

    @http.route(
        "/api/v1/vegeta_ext/budget/info",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def vegeta_ext_budget_info(self, **kwargs):
        env = request.env
        if _user_role_tag(env) is None:
            return return_Response(
                message="You are not allowed to access Vegeta budget.",
                status=403,
            )
        ...
        try:
            ...
            _logger.exception("vegeta_ext_budget_info failed")
            return return_Response(
    ...
    @http.route(
        "/api/v1/vegeta_ext/budget/fetch",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    ...
    @validate_token
    def vegeta_ext_budget_fetch(self, **kwargs):
        env = request.env
        if _user_role_tag(env) is None:
            return return_Response(
                message="You are not allowed to access Vegeta budget.",
        ...
        for budget in budgets_to_fetch:
            try:
                budget._fetch_cost_one(source='api_vegeta')
                budget._propagate_batch_alerts()
                fetched_count += 1
            except Exception as e:
                _logger.exception(
                    "vegeta_ext_budget_fetch: budget %s failed", budget.id,
                )
                fetch_errors.append({
                    "budget_id": budget.id,
                    "name": budget.name,
        ...
        try:
            ...
            _logger.exception("vegeta_ext_budget_fetch failed")
            return return_Response(
