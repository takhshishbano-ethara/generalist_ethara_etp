...
# ─────────────────────────────────────────────────────────────────────────
# Shared widget / badge layer (Vegeta pen design).
#
...
# vegeta.job.state -> (Status badge label, color_token). Labels match the pen
...
# vegeta.job.qc_verdict -> (pen label, color_token). Pen relabels
...
def _category_badge(category):
    """``category`` is a vegeta.category record (may be empty)."""
...
# to the vegeta.job domain: cost is `llm_qc_cost_usd` (not crowley.attempt
...
def _build_kpi_v2(env, ctx):
    ...
    empty-by-design (vegeta.job has no blocker/retry source field; renders "—").
    """
    Job = env["vegeta.job"].sudo()
    scope = ctx["scope"]
    rng = ctx["rng"]
    ...
    items = [
        ...
        # Open Blockers — empty by design (no blocker/retry source on vegeta.job).
...
def _build_spend_by_category(env, ctx):
    ...
    # Only categories with real spend. vegeta does not populate llm_qc_cost_usd
...
def _build_daily_burn_rate(env, ctx):
    ...
    # No spend in range (llm_qc_cost_usd is not populated by the vegeta pipeline
...
def _failed_day_counts(env, scope, win_start, win_end):
    """Failed jobs per day. vegeta.job has no failed_at, so the failure day is
...
def _build_analytics(env, ctx):
    ...
    Vegeta is internal/cost-free, so the crowley spend/burn keys
    ...
    """
    rng = ctx["rng"]
    total_tasks = env["vegeta.job"].sudo().search_count(ctx["scope"])
    return {
        ...
        # Parity stubs — vegeta has no cost data; the pen shows no spend/burn.
        "spend_by_category": {},
        "qc_pass_rate_by_ql": {},
        "daily_burn_rate": {},
...
class VegetaAnalyticsDashboardController(http.Controller):
    ...
    def vegeta_ext_analytics_dashboard(self, **kwargs):
        """Project-scoped analytics for vegeta.job, in the crowley contract.
