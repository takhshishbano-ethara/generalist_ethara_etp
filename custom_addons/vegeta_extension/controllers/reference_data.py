...
def _serialize_category(category):
    return {
        ...
        "technical_key": category.technical_key or "",
...
class VegetaReferenceDataController(http.Controller):

    @http.route(
        "/api/v1/vegeta_ext/categories",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def vegeta_ext_list_categories(self, **kwargs):
        """List vegeta.job PRD categories."""
        env = request.env
        if _user_role_tag(env) is None:
            return return_Response(
                message="You are not allowed to access Vegeta data.",
                status=403,
            )
        ...
        if search:
            domain = [
                "|",
                ("name", "ilike", search),
                ("technical_key", "ilike", search),
            ]

        Category = env["vegeta.category"].sudo()
    ...
    @validate_token
    def vegeta_ext_list_taskers(self, **kwargs):
