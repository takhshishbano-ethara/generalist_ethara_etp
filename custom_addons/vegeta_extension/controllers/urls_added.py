"""URLs Added tab endpoint for the Vegeta internal project.

The project-detail "URLs Added" tab is a URL-centric view of the same
``vegeta.job`` rows that power the score-centric "Tasks" tab
...
Field mapping (vegeta.job -> column):
  url            -> Website URL
...
# "Source" column labels for the vegeta.job.via_batch flag.
...
class VegetaUrlsAddedController(http.Controller):

    @http.route(
        "/api/v1/vegeta_ext/urls_added",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
