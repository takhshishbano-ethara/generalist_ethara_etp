...
class VegetaBatchDelivery(models.Model):
    _name = "vegeta.batch.delivery"
    _description = "Vegeta Batch Delivery"
    ...
    column_ids = fields.Many2many(
        "ir.model.fields",
        "vegeta_batch_delivery_column_rel",
        "batch_id",
        "field_id",
        string="Columns to Export",
        domain="[('model', '=', 'vegeta.job')]",
        help="Fields of vegeta.job to include in the exported CSV. Defaults are used when empty.",
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("name") or vals["name"] == _("New"):
                seq = self.env["ir.sequence"].next_by_code("vegeta.batch.delivery")
    ...
    def action_generate_csv(self):
        ...
        for field_name in column_names:
            f = self.env["vegeta.job"]._fields.get(field_name)
        ...
        return {
            ...
            "url": (
                "/web/content?model=vegeta.batch.delivery"
