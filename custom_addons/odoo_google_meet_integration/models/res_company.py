...
class ResCompany(models.Model):
    ...
    def google_meet_company_authenticate(self):
        """Method for authentication - Updated to include Google Drive Scopes"""
        if not self.hangout_client_id:
            raise ValidationError(_("Please Enter Client ID"))
        client_id = self.hangout_client_id

        if not self.hangout_redirect_uri:
            raise ValidationError(_("Please Enter Redirect URI"))
        redirect_url = self.hangout_redirect_uri

        # Define all required scopes
        calendar_scope = 'https://www.googleapis.com/auth/calendar'
        calendar_event_scope = 'https://www.googleapis.com/auth/calendar.events'
        # ADDED DRIVE SCOPE HERE
        drive_scope = 'https://www.googleapis.com/auth/drive.file'

        # We combine them with a '+' or '%20' (space) for the URL
        # We also add &prompt=consent to force the checkbox screen to appear
        url = (
            "https://accounts.google.com/o/oauth2/v2/auth?response_type=code"
            "&access_type=offline&prompt=consent&client_id={}&redirect_uri={}"
            "&scope={}+{}+{}"
        ).format(client_id, redirect_url, calendar_scope,
                 calendar_event_scope, drive_scope)
