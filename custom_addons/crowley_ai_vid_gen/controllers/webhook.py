...
class CrowleyWebhookController(http.Controller):
    ...
    def openrouter_webhook(self, **kwargs):
        ...
        idempotency_key = request.httprequest.headers.get("X-OpenRouter-Idempotency-Key", "") or ""

        ICP = request.env["ir.config_parameter"].sudo()
        secret = ICP.get_param("crowley_ai_vid_gen.webhook_secret")
        tolerance = int(ICP.get_param("crowley_ai_vid_gen.webhook_signature_tolerance", "300"))
        ...
        verifier = request.env["crowley.ai.vid.gen.webhook.verifier"].sudo()
        ...
        Attempt = request.env["crowley.ai.vid.gen.attempt"].sudo()
        ...
        request.env.cr.execute(
            "SELECT webhook_idempotency_key FROM crowley_ai_vid_gen_attempt "
