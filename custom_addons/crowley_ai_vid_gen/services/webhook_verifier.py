...
class CrowleyWebhookVerifier(models.AbstractModel):
    _name = "crowley.ai.vid.gen.webhook.verifier"
    ...
    def verify(self, raw_body, sig_header, secret, tolerance_seconds=300):
        ...
        Returns True only if the signature is well-formed, fresh (within tolerance),
        ...
        if abs(time.time() - ts_val) > tolerance_seconds:
            _logger.warning("Crowley webhook: timestamp out of tolerance (%s vs %s)", ts_val, time.time())
            return False

        signed_payload = f"{ts_val}.".encode() + (raw_body if isinstance(raw_body, bytes) else raw_body.encode())
