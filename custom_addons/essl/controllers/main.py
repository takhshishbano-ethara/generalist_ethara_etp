...
class EsslPullApiController(http.Controller):
    ...
    @http.route(
        "/essl/api/pull-attendance-logs",
        type="http",
        auth="public",
        methods=["GET"],
        csrf=False,
    )
    def pull_attendance_logs(self, token=None, **kwargs):
        if not self._is_token_valid(token):
            return self._json_response({"status": "error", "message": "Invalid or missing token"}, 401)
        ...
        try:
            ...
            date_from_ist = _parse(kwargs.get("from"), today_start_ist)
            date_to_ist = _parse(kwargs.get("to"), today_start_ist) + timedelta(days=1)
        except Exception as exc:
            return self._json_response(
                {"status": "error", "message": "date parse failed: %s" % str(exc)[:200]}, 400)
        ...
        hr_totals = {"created": 0, "updated": 0, "unchanged": 0, "skipped": 0, "failed": 0}
        for srv in servers:
            entry = {"server_id": srv.id, "server_name": srv.name}
            try:
                result = srv._pull_attendance_logs(date_from_ist, date_to_ist)
                request.env.cr.commit()
                entry.update({"status": "ok", **result})
                for key in totals:
                    totals[key] += int(result.get(key, 0))
                for key in hr_totals:
                    hr_totals[key] += int((result.get("hr_attendance") or {}).get(key, 0))
            except Exception as exc:
                request.env.cr.rollback()
                _logger.exception("ESSL pull-attendance-logs API: server %s failed", srv.name)
                entry.update({"status": "error", "message": str(exc)[:500]})
            per_server.append(entry)
        ...
        return self._json_response({
            "status": "ok",
            "range": {
                "from": date_from_ist.strftime("%Y-%m-%d"),
                "to": (date_to_ist - timedelta(days=1)).strftime("%Y-%m-%d"),
                "timezone": "IST (Asia/Kolkata)",
            },
            "totals": totals,
            "hr_attendance_totals": hr_totals,
            "servers": per_server,
            "duration_ms": int((time.monotonic() - started_at) * 1000),
        })
