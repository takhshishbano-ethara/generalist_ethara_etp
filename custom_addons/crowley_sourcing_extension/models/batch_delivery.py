...
class CrowleySourcingBatchDelivery(models.Model):
    _name = "crowley.sourcing.batch.delivery"
    _description = "Crowley Sourcing Batch Delivery"
    ...
    job_ids = fields.Many2many(
        ...
        relation="crowley_sourcing_batch_delivery_rel",
        column1="batch_id",
        column2="job_id",
        string="Projects",
    )
    job_count = fields.Integer(
        string="Project Count", compute="_compute_job_count", store=True
    )
    column_ids = fields.Many2many(
        "ir.model.fields",
        relation="crowley_sourcing_batch_delivery_col_rel",
        column1="batch_id",
        column2="field_id",
        string="CSV Columns",
        domain=[("model", "=", "video.editor.project")],
    ...
    def action_export_csv(self):
        self.ensure_one()
        if not self.job_ids:
            raise UserError(_("Nothing to export — batch has no projects."))
