...
@tagged("post_install", "-at_install", "vegeta")
class TestStartTaskWizard(VegetaTestCase):

    def test_confirm_without_category_picks_any(self):
        job = self._create_job()
        wiz = self.env["vegeta.start.task.wizard"].create({})
...
@tagged("post_install", "-at_install", "vegeta")
class TestBulkAssignWizard(VegetaTestCase):

    def test_no_active_ids_raises(self):
        wiz = self.env["vegeta.bulk.assign.wizard"].with_context(
            active_ids=[],
