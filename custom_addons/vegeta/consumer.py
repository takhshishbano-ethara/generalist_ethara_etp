...
This file is intentionally NON-FUNCTIONAL. The Vegeta pipeline no longer
...
1. In Odoo, select N rows in the Vegeta list view.
...
The old ``consumer.py`` EKS Deployment YAML must be **deleted**. Specifically:
    ...
    kubectl -n ethara delete deployment etp-vegeta-consumer  # if it exists
    kubectl -n ethara delete configmap vegeta-consumer-env  # if it exists
...
def main() -> int:
    sys.stderr.write(
        "consumer.py was removed in vegeta 19.0.2.0.0.\n"
