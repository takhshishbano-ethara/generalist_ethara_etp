# Vegeta EKS Deployment Guide (19.0.2.0.0+)
...
- [ ] Module version is **`19.0.2.1.0`** — see §10 for what shipped. The
      ...
      `-u vegeta` — no migration file needed.
...
- [ ] `Settings → Vegeta` has **Lambda Function Name** + **Region** set
...
| `vegeta.batch_concurrency` | 250 | Odoo Settings → Vegeta | Max parallel `lambda:Invoke` calls per batch dispatch |
| `VEGETA_BATCH_FANOUT_SIZE` (env) | 250 | Odoo pod env | Fallback when ICP unset |
| `VEGETA_PRD_POOL_SIZE` (env) | 50 | Odoo pod env | Thread pool for **post-extraction** PRD generation |
...
```bash
cat > vegeta-odoo-policy.json <<'EOF'
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Resource": "arn:aws:lambda:ap-south-1:<ACCOUNT_ID>:function:vegeta-extraction"
    },
    {
      "Sid": "BedrockInvoke",
      "Effect": "Allow",
    ...
    },
    {
      ...
      "Condition": {"StringLike": {"s3:prefix": ["vegeta/*"]}}
    }
  ]
}
EOF
...
aws iam create-policy \
  --policy-name vegeta-odoo-policy \
  --policy-document file://vegeta-odoo-policy.json
```
...
eksctl create iamserviceaccount \
  ...
  --attach-policy-arn arn:aws:iam::<ACCOUNT_ID>:policy/vegeta-odoo-policy \
  --approve \
...
set in Odoo's `Settings → Vegeta → Lambda Function`.
  ...
  template:
    spec:
      ...
      - name: odoo
        ...
        env:
        - name: VEGETA_WEBHOOK_TOKEN
          valueFrom:
            secretKeyRef:
              name: vegeta-secrets
              key: webhook-token
        - name: VEGETA_BATCH_FANOUT_SIZE
          value: "250"
        - name: VEGETA_PRD_POOL_SIZE
          value: "50"
        resources:
          requests:
            cpu: "2"
...
```bash
kubectl -n ethara get deployment -l app=etp-vegeta-consumer
kubectl -n ethara delete deployment etp-vegeta-consumer 2>/dev/null || true
kubectl -n ethara delete configmap vegeta-consumer-env 2>/dev/null || true
kubectl -n ethara delete secret vegeta-consumer-secrets 2>/dev/null || true
```

The RabbitMQ broker itself can be kept running (other apps may use it) or
torn down; the Vegeta module no longer connects to it under any condition.
...
1. **Odoo**: redeploy the previous image; the `via_rabbitmq` migration is
   ...
   old column can be re-added with `ALTER TABLE vegeta_job ADD COLUMN
   via_rabbitmq BOOLEAN DEFAULT FALSE; UPDATE vegeta_job SET via_rabbitmq = via_batch;`
...
- **Log prefixing** — key lifecycle logs prefixed `[vegeta][job=<name>]`.
...
selection value are auto-created on `-u vegeta`. The version bump
(`19.0.2.0.0` → `19.0.2.1.0`) triggers it automatically on deploy.

### 10.3 Reading the logs on stage (Grafana)

The Odoo instance is shared across projects. To see only Vegeta during a
stage test, filter by logger name or the `[vegeta]` prefix:

```
{namespace="ethara", container="etp-be"} |= "addons.vegeta"        # all Vegeta logs
{namespace="ethara", container="etp-be"} |= "[vegeta][job="        # per-job lifecycle
{namespace="ethara", container="etp-be"} |~ "vegeta.*ERROR|WARNING" # vegeta problems only
```
