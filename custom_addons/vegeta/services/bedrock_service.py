...
# can override via Settings UI (vegeta.bedrock_inner_retries) or env
# (VEGETA_BEDROCK_INNER_RETRIES). The Settings UI value takes effect for
...
_BEDROCK_INNER_RETRIES = int(os.environ.get("VEGETA_BEDROCK_INNER_RETRIES", "2"))
...
_BEDROCK_MAX_CONCURRENT = int(
    os.environ.get("VEGETA_BEDROCK_MAX_CONCURRENT", "5")
)
_BEDROCK_SEMAPHORE = threading.Semaphore(_BEDROCK_MAX_CONCURRENT)

_logger.info(
    "[vegeta] Bedrock concurrency cap initialised: max_concurrent=%d "
    "(env VEGETA_BEDROCK_MAX_CONCURRENT)",
...
def _bedrock_slot(call_label="bedrock"):
    ...
    _logger.info(
        "[vegeta] %s WAITING for Bedrock slot (in_flight=%d/%d, pid=%d)",
    ...
    if not acquired:
        _logger.error(
            "[vegeta] %s GAVE UP waiting for Bedrock slot after %.0fs "
        ...
        raise TimeoutError(
            ...
            f"Either raise VEGETA_BEDROCK_MAX_CONCURRENT or reduce "
            f"concurrent PRD-gen workers (VEGETA_PRD_POOL_SIZE)."
        )
    if wait_seconds > 10:
        _logger.warning(
            "[vegeta] %s waited %.1fs for Bedrock slot (cap=%d) — "
            "concurrency cap is bound; raise VEGETA_BEDROCK_MAX_CONCURRENT "
            "if your AWS quota allows",
            call_label, wait_seconds, _BEDROCK_MAX_CONCURRENT,
        )
    else:
        _logger.info(
            "[vegeta] %s ACQUIRED Bedrock slot after %.1fs "
    ...
    try:
        ...
        _logger.info(
            "[vegeta] %s RELEASED Bedrock slot (held %.1fs, in_flight now "
