...
def _call_bedrock_bearer(
    ...
    The ARN contains `/` (e.g. `application-inference-profile/<id>`) which httpx
    treats as a path separator and does NOT encode. Bedrock requires the ARN
    as a single path segment, so we percent-encode it ourselves.
    """
    import urllib.parse
