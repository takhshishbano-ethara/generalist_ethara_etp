"""Async invocation of the Vegeta extraction Lambda.
...
arrive only via the webhook callback to ``/api/v1/vegeta/webhook/extraction-complete``.
...
def _invoke_lambda_local(local_url: str, payload: dict, job_id: int) -> dict:
    ...
    threading.Thread(
        target=_fire,
        daemon=True,
        name=f"vegeta-local-invoke-{job_id}",
    ).start()
