# SKOLL — TRAJECTORY STRUCTURE & META_INFO QC PROMPT

> **Purpose**: This prompt defines the QC checks for validating trajectory JSON files against the Skoll schema. It covers structural integrity (top-level keys, message envelope, content blocks) and `meta_info` field correctness (required fields, valid values, non-empty constraints). These checks are deterministic and scriptable.
>
...
- [ ] `toolCall.toolCall.name` matches a valid tool from the Skoll tool registry (see Appendix A) → **BLOCK** if unknown tool
