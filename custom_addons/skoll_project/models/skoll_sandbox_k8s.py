...
from .skoll import _DEFAULT_LITELLM_CONFIG, _load_dotenv
from .skoll_sandbox import MODEL_DEFAULTS
...
WS_ROUTER_NAME = "skoll-ws-router"
...
SANDBOX_SERVICE_ACCOUNT = "skoll-sandbox"

S3_BUCKET = "production-grtlabs-tag"
S3_SKOLL_PREFIX = "skoll"
...
server {
    ...
    location ~ ^/sandbox/(\\d+)(?:/(.*))? {
        set $task_id $1;
        set $path $2;
        set $backend skoll-sandbox-$task_id.%(namespace)s.svc.cluster.local:18789;
    ...
    location ~ ^/litellm/(\\d+)(?:/(.*))? {
        set $task_id $1;
        set $path $2;
        set $backend skoll-sandbox-$task_id.%(namespace)s.svc.cluster.local:4000;
        proxy_pass http://$backend/$path$is_args$args;
        proxy_http_version 1.1;
        proxy_set_header Host localhost;
...
WS_ROUTER_LABELS = {
    "platform": "skoll",
    "component": "ws-router",
    "app.kubernetes.io/name": "skoll-ws-router",
...
def _resource_name(sandbox_record):
    return "skoll-sandbox-%s" % sandbox_record.id
...
def _build_prestop_script(task_id, persona_name):
    session_path = "s3://%s/%s/sessions/%s/" % (S3_BUCKET, S3_SKOLL_PREFIX, task_id)
    ...
    return (
        "RUN_ID=$(cat /home/node/.openclaw/.skoll-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
        'echo "[skoll] preStop: run=$RUN_ID backing up session data to S3..." && '
        ...
        "--no-progress --quiet 2>/dev/null || true && "
        "echo '[skoll] preStop: backup complete, waiting for connections to drain...' && "
        "sleep 10"
...
class SkollSandboxK8s(models.AbstractModel):
    _name = "skoll.sandbox.k8s"
    _description = "Skoll K8s Sandbox Deployer"
    ...
    def deploy_sandbox(self, sandbox_record):
        ...
        task_id = sandbox_id
        persona = sandbox_record.skoll_id.persona_id
        if not persona:
            raise UserError(
                "No persona selected on task '%s'. Please select a persona and save before starting."
                % (sandbox_record.skoll_id.display_name or sandbox_record.skoll_id.id)
            )
        ...
        if not litellm_master_key:
            litellm_master_key = "sk-skoll-%s" % secrets.token_hex(8)
        ...
        openclaw_image = self._get_config_param(
            "skoll.openclaw_image",
            "426628337772.dkr.ecr.ap-south-1.amazonaws.com/skoll-q1-coding:v1.0.0",
        )
        litellm_image = self._get_config_param(
            "skoll.litellm_image", "ghcr.io/berriai/litellm:main-stable"
        )
        postgres_image = self._get_config_param("skoll.postgres_image", "postgres:16")
        aws_cli_image = self._get_config_param(
            "skoll.aws_cli_image", "amazon/aws-cli:latest"
        )
        ...
        skoll_ws_host = self._get_config_param("skoll.ws_router_host", "")
        nginx_image = self._get_config_param("skoll.nginx_image", "nginx:alpine")
        self._ensure_ws_router(
            core_v1, apps_v1, networking_v1, skoll_ws_host, nginx_image
        )
    ...
    def _create_secret(
        ...
        secret = client.V1Secret(
            ...
            metadata=client.V1ObjectMeta(
                name="skoll-sandbox-creds-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            string_data={
    ...
    def _create_persona_configmap(self, core_v1, task_id, labels, persona):
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="skoll-sandbox-persona-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            data=data,
    ...
    def _create_gog_secret(self, core_v1, task_id, labels, sandbox_record):
        ...
        gog_auth_raw = sandbox_record.skoll_id.gog_auth or ""
        if gog_auth_raw:
            try:
        ...
        gog_auth_token_raw = sandbox_record.skoll_id.gog_auth_token or ""
        ...
        gog_kp = sandbox_record.skoll_id.password or ""
        gog_account = sandbox_record.skoll_id.email or ""
        ...
        if _has_token_files and not gog_kp:
            _logger.warning(
                ...
                "decrypt JWE tokens. Ensure skoll_id.password is set.",
                task_id,
        ...
        secret = client.V1Secret(
            ...
            kind="Secret",
            metadata=client.V1ObjectMeta(
                name="skoll-sandbox-gog-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_openclaw_config_configmap(
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="skoll-sandbox-openclaw-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_litellm_configmap(self, core_v1, task_id, labels, litellm_yaml):
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="skoll-litellm-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_deployment(
        ...
        secret_name = "skoll-sandbox-creds-%s" % task_id
        gog_secret_name = "skoll-sandbox-gog-%s" % task_id
        persona_cm = "skoll-sandbox-persona-%s" % task_id
        openclaw_config_cm = "skoll-sandbox-openclaw-config-%s" % task_id
        litellm_config_cm = "skoll-litellm-config-%s" % task_id
        ...
        init_containers = [
            client.V1Container(
                ...
                command=[
                    ...
                    "echo $RUN_ID > /data/session/.skoll-run-id && "
            ...
            client.V1Container(
                ...
                command=[
                    "sh",
                    "-c",
                    "RUN_ID=$(cat /data/.skoll-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
                    "echo '[skoll] snapshot-start: saving fresh state as run-'$RUN_ID && "
    ...
    def _ensure_ws_router(self, core_v1, apps_v1, networking_v1, ws_host, nginx_image):
        ...
        cm_body = client.V1ConfigMap(
            metadata=client.V1ObjectMeta(
                name="skoll-ws-router-conf",
                namespace=NAMESPACE,
                labels=WS_ROUTER_LABELS,
                annotations={"skoll.ethara.ai/conf-hash": desired_hash},
            ),
        ...
        try:
            existing_cm = core_v1.read_namespaced_config_map(
                name="skoll-ws-router-conf", namespace=NAMESPACE
            )
                ...
                patch = {
                    "spec": {
                        "template": {
                            "metadata": {
                                "annotations": {
                                    "skoll.ethara.ai/conf-hash": desired_hash,
                                    "skoll.ethara.ai/restarted-at": _dt.datetime.utcnow().isoformat(),
                                }
                            }
                        }
                    }
    ...
    def destroy_sandbox(self, sandbox_record):
        ...
        self._delete_resource(
            ...
            "skoll-sandbox-creds-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_secret,
            "skoll-sandbox-gog-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "skoll-sandbox-persona-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "skoll-sandbox-openclaw-config-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "skoll-litellm-config-%s" % task_id,
            NAMESPACE,
        )
    ...
    def reconcile_sandboxes(self):
        ...
        NOTE: Prefer using skoll.sandbox._cron_reconcile() (called via the
