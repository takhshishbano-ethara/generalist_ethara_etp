...
def _wrap_messages_with_turn_feedback(messages, turns):
    ...
    ``turns`` is an iterable of SkollTurn records (sorted by turn_number).
...
class Skoll(models.Model):
    ...
    # `is_skoll_admin` (QL-only) so widening access later doesn't silently
    ...
    parsona = fields.Many2one("skoll.domain", string="Parsona")
    ...
    persona_id = fields.Many2one(
        "skoll.persona", string="Persona", required=True, ondelete="restrict"
    )
    heart_taxonomy = fields.Many2many("skoll.taxonomy", string="HEART Taxonomy")
    ...
    # Sandboxes
    sandbox_ids = fields.One2many("skoll.sandbox", "skoll_id", string="Sandboxes")
    ...
    claude_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="Claude Sandbox"
    )
    glm_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="GLM Sandbox"
    )
    oneP_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="1P Sandbox"
    )
    onePA_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="1PA Sandbox"
    )
    onePB_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="1PB Sandbox"
    )
    onePC_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="1PC Sandbox"
    )
    onePD_sandbox_id = fields.Many2one(
        "skoll.sandbox", compute="_compute_sandbox_ids", string="1PD Sandbox"
    )
    ...
    golden_prompt = fields.Text(
        string="Golden Generation Prompt",
        help="Scenario prompt for streaming golden trajectory generation.",
    )

    # Golden Trajectory QC (LLM + deterministic)
    golden_qc_status = fields.Selection(
        [
            ("pending", "Pending"),
            ("running", "Running"),
            ("pass", "Pass"),
        ...
        string="Golden QC Status",
        default="pending",
    )
    golden_qc_result = fields.Text(string="Golden QC Result (LLM)")
    golden_structural_result = fields.Text(string="Golden Structural Validation")
    golden_spawn_tree = fields.Text(string="Golden Spawn Tree")
    spawned_agents = fields.Text(string="Spawned Agents (JSON)")
    ...
    def action_view_turns(self):
        ...
        return {
            ...
            "name": "Turns",
            "res_model": "skoll.turn",
            "view_mode": "list,form",
            "domain": [("sandbox_id", "in", self.sandbox_ids.ids)],
            "context": {"default_skoll_id": self.id},
        }
    ...
    def action_export_session(self):
        ...
        return {
            ...
            "url": f"/skoll/chat/export_session?task_id={self.id}",
            "target": "self",
        }
    ...
    def action_delete_trajectory_for_sandbox(self, sandbox_id):
        ...
        sandbox = self.env["skoll.sandbox"].browse(int(sandbox_id or 0))
        if not sandbox.exists() or sandbox.skoll_id.id != self.id:
    ...
    def action_delete_trajectory_by_field(self, field_name):
        ...
        sandbox = self.env["skoll.sandbox"].search(
            [("skoll_id", "=", self.id), ("model_type", "=", model_type)],
            limit=1,
        )
        if not sandbox:
            raise UserError(
    ...
    def _build_trajectory_fallback(self):
        ...
        for t in self._get_all_turns():
            run_id = t.run_id or ""
            if t.is_hint_turn and t.hints:
                is_accepted = 1
                hints = (t.hints or "").strip() or None
            else:
                is_accepted = 0
                hints = None
    ...
    def _cron_reconcile_sandboxes(self):
        self.env["skoll.sandbox"]._cron_reconcile()
    ...
    def auto_process_claim_task(self, task_id):
        ...
        self.env.cr.execute(
            "UPDATE skoll_skoll SET auto_process_status = 'processing' "
        ...
        # Find Claude sandbox
        claude_sandbox = self.env["skoll.sandbox"].search(
            [("skoll_id", "=", task_id), ("model_type", "=", "claude")], limit=1
        )
    ...
    def action_upload_trajectories(self):
        ...
        self.ensure_one()
        from .skoll_sandbox_k8s import S3_BUCKET, S3_SKOLL_PREFIX

        icp = self.env["ir.config_parameter"].sudo()
        bucket = icp.get_param("skoll.s3_bucket") or S3_BUCKET
        region = icp.get_param("skoll.s3_region") or "us-east-1"
        prefix = icp.get_param("skoll.s3_prefix") or S3_SKOLL_PREFIX

        if not bucket:
            raise UserError("S3 bucket not configured. Go to Settings → Skoll.")
        ...
        def _upload_single(bkt, rgn, pfx, upload_files, ak, sk):
            try:
                ...
                _logger.info(
                    "Skoll trajectory upload: %d files → s3://%s/%s/",
                    len(upload_files), bkt, pfx,
                )
            except Exception:
                _logger.exception("Skoll trajectory upload failed for %s", pfx)
        ...
        def _upload_batch(bkt, rgn, uploads, ak, sk):
            try:
                ...
                for pfx, files in uploads:
                    ...
                    total += 1
                _logger.info("Skoll trajectory mass upload: %d tasks → s3://%s/", total, bkt)
            except Exception:
                _logger.exception("Skoll trajectory mass upload failed")
...
class SkollTurn(models.Model):
    _name = "skoll.turn"
    _description = "Skoll Turn"
    _order = "turn_number asc, id asc"

    sandbox_id = fields.Many2one(
        "skoll.sandbox", string="Sandbox", ondelete="cascade", index=True
    )
    skoll_id = fields.Many2one(related="sandbox_id.skoll_id", store=True, readonly=True)
    employee_id = fields.Many2one(
    ...
    sub_agent_messages = fields.Text(string="Sub-Agent Messages (JSON)")
    spawn_tree = fields.Text(string="Agent Spawn Tree (JSON)")
...
class SkollTaxonomy(models.Model):
    _name = "skoll.taxonomy"
    _description = "Skoll Taxonomy"
