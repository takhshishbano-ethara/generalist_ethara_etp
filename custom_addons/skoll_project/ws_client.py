"""
Asyncio-based WebSocket client for OpenClaw gateways in Skoll sandbox containers.
...
logger = logging.getLogger("skoll.ws_client")
...
class OpenClawClient:
    ...
    def __init__(self, ws_url, gateway_token, sandbox_id, logger_override=None):
        ...
        self._child_sessions: dict = {}  # childSessionKey -> {"messages": list}
        self._sub_agent_messages: list = []  # flat list of all captured messages
        self._child_subscriptions: set = set()  # keys we've subscribed to
    ...
    def _next_id(self):
        self._msg_counter += 1
        ts = int(time.time())
        return f"skoll-{self._msg_counter}-{ts:x}"
    ...
    def send_message(self, text):
        ...
        fut = asyncio.run_coroutine_threadsafe(self._async_send_message(text), loop)
...
"minProtocol": 3,
            "maxProtocol": 4,
                ...
                "userAgent": "skoll-auto-process/1.0",
                "locale": "en-US",
            },
        }
    ...
    # ------------------------------------------------------------------
    # Session subscription (sub-agent detection)
    # ------------------------------------------------------------------

    async def _subscribe_sessions(self, ws):
        """Subscribe to session lifecycle events to detect sub-agent spawns."""
        msg = {
            "type": "req",
            "id": self._next_id(),
        ...
        self._log.debug("Subscribing to session lifecycle events")
        await ws.send(json.dumps(msg))

    async def _subscribe_child_session(self, child_key):
        """Subscribe to transcript events for a child session."""
        if child_key in self._child_subscriptions:
            return
        ws = self._ws
        if ws is None:
            return
        msg = {
            ...
            "params": {"key": child_key},
        }
        self._child_subscriptions.add(child_key)
        self._log.info("Subscribing to child session: %s", child_key)
        await ws.send(json.dumps(msg))

    def _on_subagent_spawned(self, child_key, session_data):
        """Track a newly spawned sub-agent session."""
        self._child_sessions[child_key] = {
            "messages": [],
            "parentKey": session_data.get("parentKey", ""),
            "agent": session_data.get("agent", ""),
            "kind": session_data.get("kind", "subagent"),
            "spawned_at": session_data.get("createdAt", ""),
        }
        self._log.info(
            "Sub-agent spawned: key=%s agent=%s parentKey=%s",
            child_key,
            session_data.get("agent", "unknown"),
            session_data.get("parentKey", ""),
        )
        loop = self._loop
        if loop:
            asyncio.run_coroutine_threadsafe(
                self._subscribe_child_session(child_key), loop
            )

    def _handle_sessions_changed(self, payload):
        """Handle sessions.changed events to detect sub-agent spawns."""
        reason = payload.get("reason")
        session_data = payload.get("session", {})
        if reason == "create":
            child_key = session_data.get("key", "")
            if ":subagent:" in child_key or session_data.get("kind") == "subagent":
                self._on_subagent_spawned(child_key, session_data)

    def _handle_session_message(self, payload):
        """Capture transcript appends from subscribed child sessions."""
        session_key = payload.get("sessionKey", "")
        message = payload.get("message", {})
        provenance = message.get("provenance", {}) if isinstance(message, dict) else {}
        kind = provenance.get("kind", "")

        if kind == "inter_session" or session_key in self._child_sessions:
            entry = {
            ...
            if session_key in self._child_sessions:
                self._child_sessions[session_key]["messages"].append(entry)

            self._sub_agent_messages.append(entry)
            self._log.info(
                "Sub-agent message captured: key=%s role=%s kind=%s text=%.100s",
                session_key,
                entry["role"],
                kind,
                entry["text"],
    ...
    def _handle_raw(self, raw):
        ...
        # Sub-agent lifecycle: detect spawns
        if frame_type == "event" and event == "sessions.changed":
            payload = frame.get("payload", {})
            self._handle_sessions_changed(payload)
            return

        # Sub-agent transcript: capture messages from child sessions
        if frame_type == "event" and event == "session.message":
            payload = frame.get("payload", {})
    ...
    async def _async_send_message(self, text):
        ...
        self._log.info("Sending message: %s", truncated)
        await ws.send(json.dumps(msg))
    ...
    # ------------------------------------------------------------------
    # Public API: sub-agent message access
    # ------------------------------------------------------------------

    def get_sub_agent_messages(self):
        return list(self._sub_agent_messages)

    def get_child_sessions(self):
        return dict(self._child_sessions)

    def get_spawn_tree(self):
        """Build a hierarchical spawn tree from tracked child sessions.

        Returns a list of tree nodes, each representing a root-level spawn.
        Nested spawns appear as children within their parent node.
        """
        if not self._child_sessions:
            return []

        nodes = {}
        for key, data in self._child_sessions.items():
            nodes[key] = {
                "sessionKey": key,
    ...
    def fetch_child_history(self, session_key, limit=500):
        if not self._connected.is_set():
            raise OpenClawError("Not connected")
        loop = self._require_loop()
        fut = asyncio.run_coroutine_threadsafe(
            self._async_fetch_child_history(session_key, limit), loop
        )
        return fut.result(timeout=35)

    async def _async_fetch_child_history(self, session_key, limit):
        ws = self._ws
        if ws is None:
            raise OpenClawError("WebSocket not available")
        msg_id = self._next_id()
        ...
        try:
            ...
            self._pending_rpcs.pop(msg_id, None)
            raise OpenClawTimeoutError(
                "chat.history RPC timed out for child session"
            )
        result = frame.get("result", {})
        if isinstance(result, dict) and "messages" in result:
            return result["messages"]
