...
export class TaskProgressWidget extends Component {
    static template = "skoll.TaskProgressWidget";
    ...
    stepClass(step) {
        return `o_skoll_progress_step o_skoll_progress_step_${step.state}`;
    }

    connectorClass(conn) {
        return `o_skoll_progress_connector ${conn.filled ? "o_skoll_progress_connector_filled" : ""}`;
    }
}

export const taskProgressWidgetDef = { component: TaskProgressWidget };
registry.category("view_widgets").add("skoll_task_progress", taskProgressWidgetDef);
