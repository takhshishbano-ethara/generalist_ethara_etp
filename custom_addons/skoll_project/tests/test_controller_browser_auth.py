...
_BROWSER_MOD = "odoo.addons.skoll.controllers.browser_auth"


@tagged("post_install", "-at_install")
class TestBrowserApiBaseUrl(SkollTestCase):

    def _ctrl(self):
        from odoo.addons.skoll.controllers.browser_auth import SkollBrowserAuthController
        return SkollBrowserAuthController
    ...
    def test_url_k8s_mode_with_ws_host(self):
        ctrl_cls = self._ctrl()
        self._set_param("skoll.ws_router_host", "ws.ethara.ai")
        with patch.object(
    ...
    def test_url_k8s_mode_no_ws_host(self):
        ...
        with patch.object(
            ...
            url = ctrl_cls._browser_api_base(self.claude_sandbox)
        svc_name = "skoll-sandbox-%s" % self.claude_sandbox.id
        expected = "http://%s.skoll.svc.cluster.local:18789/browser-api" % svc_name
        self.assertEqual(url, expected)


@tagged("post_install", "-at_install")
class TestBrowserScreenshot(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserInjectCookies(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserStatus(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserScreenshotErrorPaths(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserInjectCookiesErrorPaths(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserStatusErrorPaths(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserValidateAndApiBaseEdgeCases(SkollTestCase):
