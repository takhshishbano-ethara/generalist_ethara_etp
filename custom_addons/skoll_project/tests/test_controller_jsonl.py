# -*- coding: utf-8 -*-
"""Tests for controllers/skoll_controller.py — the JSONL import logic.
...
from ..controllers.skoll_controller import Skoll as SkollController
...
class TestJSONLImport(SkollTestCase):
    ...
    def _call_controller(self, jsonl_lines, url="https://example.com/data.jsonl"):
        ...
        ctrl = SkollController()
        mock_resp = _make_mock_response(jsonl_lines)
        with patch("odoo.http.request") as mock_req, \
             patch("odoo.addons.skoll.controllers.skoll_controller.requests.get", return_value=mock_resp) as mock_get:
    ...
    def test_import_creates_records(self):
        ...
        rec1 = self.Skoll.search([("task_id", "=", "JSONL-001")])
        rec2 = self.Skoll.search([("task_id", "=", "JSONL-002")])
    ...
    def test_import_maps_credentials(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-CRED-001")])
        self.assertTrue(rec)
    ...
    def test_import_maps_gog_auth(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-GOG-001")])
        self.assertTrue(rec)
    ...
    def test_import_maps_gmail_credentials(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-GMAIL-001")])
        self.assertTrue(rec)
    ...
    def test_import_normalizes_persona_name(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-NORM-001")])
        self.assertTrue(rec)
    ...
    def test_import_handles_empty_fields(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-EMPTY-001")])
        self.assertTrue(rec)
    ...
    def test_import_handles_malformed_json_lines(self):
        ...
        self._call_controller(lines)
        rec1 = self.Skoll.search([("task_id", "=", "JSONL-MAL-001")])
        rec2 = self.Skoll.search([("task_id", "=", "JSONL-MAL-002")])
        self.assertTrue(rec1)
        self.assertTrue(rec2)
    ...
    def test_import_default_persona_created(self):
        ...
        self._call_controller(lines)
        rec = self.Skoll.search([("task_id", "=", "JSONL-DEFPERS-001")])
        self.assertTrue(rec)
