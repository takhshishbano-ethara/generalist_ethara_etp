# -*- coding: utf-8 -*-
"""Shared test fixtures for the Skoll test suite.

All Skoll test classes should inherit from ``SkollTestCase`` to get
...
class SkollTestCase(TransactionCase):
    """Base test case with shared fixtures for all Skoll tests."""
    ...
    def setUpClass(cls):
        ...
        # ── Model shortcuts ─────────────────────────────────────────
        cls.Skoll = cls.env["skoll.skoll"]
        cls.Sandbox = cls.env["skoll.sandbox"]
        cls.Turn = cls.env["skoll.turn"]
        cls.Persona = cls.env["skoll.persona"]
        cls.Domain = cls.env["skoll.domain"]
        cls.Taxonomy = cls.env["skoll.taxonomy"]
    ...
    def _create_task(cls, **overrides):
        """Create a skoll.skoll record with sensible defaults."""
        vals = {
            "persona_id": cls.persona.id,
            "task_status": "NotSubmitted",
        }
        vals.update(overrides)
        return cls.Skoll.create(vals)

    @classmethod
    def _create_sandbox(cls, task=None, model_type="claude", **overrides):
        """Create a skoll.sandbox record."""
    ...
    def _make_session_entry(trajectory_json_str=None, session_id="test-session-001"):
        ...
        if trajectory_json_str is None:
            traj = json.loads(SkollTestCase._make_trajectory_json())
        else:
