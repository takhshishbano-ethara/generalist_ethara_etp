...
_AUTO_HINT_POOL = ThreadPoolExecutor(
    max_workers=3, thread_name_prefix="skoll-auto-hint"
)
...
def _push_bus(env, sandbox, notify_partner_id, payload):
    ...
    env["bus.bus"]._sendone(partner, "skoll/auto_hint_result", payload)
...
class AutoHintController(http.Controller):
    ...
    @http.route("/skoll/auto_hint_eval", type="json", auth="user")
