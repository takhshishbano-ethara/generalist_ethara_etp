...
from ..models.kensei2_sandbox import MODEL_DEFAULTS, TRAJECTORY_FIELD_MAP
...
class Kensei2ChatController(http.Controller):
    @http.route("/kensei2/chat/create_turn", type="json", auth="user")
    def create_turn(
        self,
        sandbox_id=0,
        message="",
        ...
        **kw,
        ...
        sandbox = request.env["kensei2.sandbox"].browse(sandbox_id)
        ...
        turn = request.env["kensei2.turn"].create(vals)
    ...
    @http.route("/kensei2/chat/save_response", type="json", auth="user")
    def save_response(
        self,
        turn_id=0,
        response="",
        ...
        **kw,
        ...
        turn = request.env["kensei2.turn"].browse(turn_id)
        if not turn.exists():
    ...
    @http.route("/kensei2/chat/mark_timeout", type="json", auth="user")
    def mark_timeout(self, turn_id=0, timeout_seconds=300, **kw):
        ...
        _logger.warning(
            "Kensei2 chat turn %s marked TimedOut after %ss",
            turn_id,
            timeout_seconds,
        )
        return {"success": True, "status": "TimedOut"}

    @http.route("/kensei2/chat/save_qc", type="json", auth="user")
    def save_qc(
        self,
        turn_id=0,
        severity="",
    ...
    @http.route("/kensei2/chat/save_feedback", type="json", auth="user")
    ...
    @http.route("/kensei2/chat/save_trajectory", type="json", auth="user")
    ...
    @http.route("/kensei2/chat/history", type="json", auth="user")
    def chat_history(self, sandbox_id=0, **kw):
        ...
        s3_url_base = ""
        try:
            from ..models.kensei2_sandbox_k8s import S3_KENSEI2_PREFIX
    ...
    @http.route("/kensei2/chat/export_session", type="http", auth="user")
    def export_session(self, sandbox_id=0, task_id=0, **kw):
        ...
        if sandbox_id:
            ...
            saved = field_name and sandbox.kensei2_id and sandbox.kensei2_id[field_name]
            trajectory = None
            if saved:
                try:
            ...
            task = request.env["kensei2.kensei2"].browse(task_id)
            if not task.exists():
                return request.not_found()
    ...
    @http.route("/kensei2/chat/sandbox_state", type="json", auth="user")
    ...
    @http.route("/kensei2/chat/upload", type="http", auth="user", methods=["POST"], csrf=False)
    ...
    @http.route("/kensei2/chat/persist_attachments", type="json", auth="user")
    ...
    def _write_to_k8s_pod(self, sandbox, filename, file_bytes):
        ...
        from ..models.kensei2_sandbox_k8s import NAMESPACE

        pod_name = "kensei2-sandbox-%s" % sandbox.id
    ...
    @http.route("/kensei2/chat/media", type="http", auth="user", methods=["GET"])
    def serve_media(self, sandbox_id=0, source="", **kw):
        ...
        is_admin = user.has_group("kensei2.group_kensei2_ql")
    ...
    @http.route("/kensei2/chat/persist_output_media", type="json", auth="user")
    ...
    def _read_artifact_from_s3(self, task, filename):
        from ..models.kensei2_sandbox_k8s import S3_BUCKET, S3_KENSEI2_PREFIX
