...
_LIST_STATS = {
    "kensei2.tracker.allocation": _stats_allocation,
    "kensei2.persona": _stats_persona,
    "kensei2.tracker.team.member": _stats_team,
}
...
class Kensei2TrackerController(http.Controller):
    ...
    @http.route("/kensei2/tracker/persona/sample_csv", type="http", auth="user")
    ...
    @http.route("/kensei2/tracker/team/sample_csv", type="http", auth="user")
    ...
    # ------------------------------------------------------------------ #
    @http.route("/kensei2/tracker/list_stats", type="json", auth="user")
    ...
    # ------------------------------------------------------------------ #
    @http.route("/kensei2/tracker/daily/filters", type="json", auth="user")
    def daily_filters(self, **kw):
        ...
        Alloc = request.env["kensei2.tracker.allocation"]
    ...
    def tracker_dashboard(self, date_from=None, date_to=None,
        ...
        is_admin = (user.has_group("base.group_system")
                    or user.has_group("kensei2.group_kensei2_admin"))
        is_lead = user.has_group("kensei2.group_kensei2_ql")  # PL implies QL
        ...
        Member = request.env["kensei2.tracker.team.member"]
    ...
    # ------------------------------------------------------------------ #
    @http.route("/kensei2/tracker/performance", type="json", auth="user")
    def tracker_performance(self, member_id=None, date_from=None, date_to=None, **kw):
        env = request.env
        caller = env.user
        if member_id:
            mid = _to_int(member_id)
            member = env["kensei2.tracker.team.member"].browse(mid).exists() if mid else None
            if not member:
                return {"error": "not_found"}
            target = member.user_id
            is_lead = (caller.has_group("kensei2.group_kensei2_ql")
