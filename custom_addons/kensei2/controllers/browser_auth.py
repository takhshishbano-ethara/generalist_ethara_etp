...
class Kensei2BrowserAuthController(http.Controller):
    ...
    def _browser_api_base(sandbox):
        ...
        if mode == "k8s":
            ...
            svc_name = "kensei2-sandbox-%s" % sandbox.id
            return (
                "http://%s.kensei2.svc.cluster.local:18789/browser-api"
                % svc_name
            )
    ...
    @http.route("/kensei2/browser/screenshot", type="json", auth="user")
    ...
    @http.route("/kensei2/browser/inject_cookies", type="json", auth="user")
    ...
    @http.route("/kensei2/browser/status", type="json", auth="user")
