...
from ..models.kensei2 import _load_dotenv
...
def _update_qc_entry(env, record_id, field_name, entry_index, qc_status, qc_result):
    ...
    for attempt in range(max_retries):
        try:
            task = env["kensei2.kensei2"].browse(record_id)
            if not task.exists():
                return
...
class LlmAssistQc(http.Controller):
    @http.route("/kensei2/qc", type="json", auth="user")
    def qc_prompt(
        self, prompt="", system_prompt="", max_tokens=4096, temperature=0.3, **kw
    ):
        ICP = request.env["ir.config_parameter"].sudo()
        if ICP.get_param("kensei2.disable_prompt_qc", "False").lower() == "true":
        ...
        env = _load_dotenv()
        api_key = (env.get("KENSEI2_AWS_BEARER_TOKEN") or env.get("AWS_BEARER_TOKEN_BEDROCK", "")).strip()
        if not api_key:
            return {"error": "KENSEI2_AWS_BEARER_TOKEN (or AWS_BEARER_TOKEN_BEDROCK) not set in .env"}
        ...
        if not inference_arn:
            return {"error": "Bedrock Inference ARN not configured in Settings > Kensei2"}
    ...
    @http.route("/kensei2/trajectory_qc", type="json", auth="user")
    def trajectory_qc(self, record_id=0, field_name="", entry_index=-1, **kw):
        ICP = request.env["ir.config_parameter"].sudo()
        if ICP.get_param("kensei2.disable_trajectory_qc", "False").lower() == "true":
    ...
    @http.route("/kensei2/generate_task_description", type="json", auth="user")
    ...
    def llm_assist_qc_legacy(self, **params):
        try:
            ...
            if not api_key:
                return request.make_json_response(
                    {
                        "error": "KENSEI2_AWS_BEARER_TOKEN (or AWS_BEARER_TOKEN_BEDROCK) not set in .env",
                        "status": 500,
                    },
                    status=500,
                )
