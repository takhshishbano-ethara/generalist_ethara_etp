# -*- coding: utf-8 -*-
"""Tests for models/kensei2_sandbox_k8s.py — the K8s deployer AbstractModel."""
import json
...
_K8S_MOD = "odoo.addons.kensei2.models.kensei2_sandbox_k8s"
...
class TestK8sHelperFunctions(Kensei2TestCase):
    """Tests for module-level helper functions."""

    def test_sandbox_labels(self):
        """_sandbox_labels returns correct label dict with task-id."""
        from odoo.addons.kensei2.models.kensei2_sandbox_k8s import _sandbox_labels
        labels = _sandbox_labels(self.claude_sandbox)
        self.assertEqual(labels["platform"], "kensei2")
        ...
        self.assertEqual(labels["app.kubernetes.io/managed-by"], "kensei2-odoo")

    def test_resource_name(self):
        """_resource_name returns 'kensei2-sandbox-{id}'."""
        from odoo.addons.kensei2.models.kensei2_sandbox_k8s import _resource_name
        name = _resource_name(self.claude_sandbox)
        self.assertEqual(name, "kensei2-sandbox-%s" % self.claude_sandbox.id)

    def test_s3_session_path(self):
        """_s3_session_path returns correct S3 URI format."""
        from odoo.addons.kensei2.models.kensei2_sandbox_k8s import (
            _s3_session_path, S3_BUCKET, S3_KENSEI2_PREFIX,
        )
        path = _s3_session_path(self.claude_sandbox)
        expected = "s3://%s/%s/tasks/%s/sessions/" % (
            S3_BUCKET, S3_KENSEI2_PREFIX, self.claude_sandbox.id,
        )
        self.assertEqual(path, expected)

    def test_build_prestop_script(self):
        """_build_prestop_script contains task_id and persona."""
        from odoo.addons.kensei2.models.kensei2_sandbox_k8s import _build_prestop_script
    ...
    def test_build_openclaw_config_model_defaults(self):
        ...
        from odoo.addons.kensei2.models.kensei2_sandbox_k8s import _build_openclaw_config
        from odoo.addons.kensei2.models.kensei2_sandbox import MODEL_DEFAULTS
...
@tagged("post_install", "-at_install")
class TestK8sDeploy(Kensei2TestCase):
    """Tests for Kensei2SandboxK8s.deploy_sandbox — all K8s calls mocked."""

    def _get_deployer(self):
        return self.env["kensei2.sandbox.k8s"]
    ...
    def test_deploy_creates_secret(self, _dotenv, mock_client, _config):
        ...
        for call in mock_core.create_namespaced_secret.call_args_list:
            self.assertEqual(call[1]["namespace"], "kensei2")
...
@tagged("post_install", "-at_install")
class TestK8sDestroy(Kensei2TestCase):
    """Tests for Kensei2SandboxK8s.destroy_sandbox."""
...
@tagged("post_install", "-at_install")
class TestK8sStatus(Kensei2TestCase):
    """Tests for Kensei2SandboxK8s.get_sandbox_status."""
