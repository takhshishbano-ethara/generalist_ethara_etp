...
_GOG_MOD = "odoo.addons.kensei2.controllers.gog_auth"


@tagged("post_install", "-at_install")
class TestGogStartAuth(Kensei2TestCase):

    def _ctrl(self):
        from odoo.addons.kensei2.controllers.gog_auth import Kensei2GogAuthController
        return Kensei2GogAuthController()

    def _task_with_gog(self):
        task = self._create_task(
            task_id="GOG-START-%s" % self.env["ir.sequence"].next_by_code("kensei2.kensei2") or "001",
...
@tagged("post_install", "-at_install")
class TestExtractClientSecret(Kensei2TestCase):

    def _extract(self, raw):
        from odoo.addons.kensei2.controllers.gog_auth import Kensei2GogAuthController
        return Kensei2GogAuthController._extract_client_secret(raw)
...
@tagged("post_install", "-at_install")
class TestGogExchangeToken(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestGogStatus(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestGogAuthErrorPaths(Kensei2TestCase):
    ...
    def _task_with_gog(self):
        task = self._create_task(
            task_id="GOG-ERR-%s" % (self.env["ir.sequence"].next_by_code("kensei2.kensei2") or "001"),
            email="user@example.com",
            password="keyring-pw",
            gog_auth=json.dumps({"installed": {"client_id": "abc"}}),
        )
...
@tagged("post_install", "-at_install")
class TestExtractClientSecretErrorPaths(Kensei2TestCase):
