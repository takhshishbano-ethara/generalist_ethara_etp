# -*- coding: utf-8 -*-
"""Tests for controllers/kensei2_controller.py — the JSONL import logic.
...
from ..controllers.kensei2_controller import Kensei2 as Kensei2Controller
...
class TestJSONLImport(Kensei2TestCase):
    ...
    def _call_controller(self, jsonl_lines, url="https://example.com/data.jsonl"):
        ...
        ctrl = Kensei2Controller()
        mock_resp = _make_mock_response(jsonl_lines)
        with patch("odoo.http.request") as mock_req, \
             patch("odoo.addons.kensei2.controllers.kensei2_controller.requests.get", return_value=mock_resp) as mock_get:
    ...
    def test_import_auto_creates_persona(self):
        """New persona name → creates kensei2.persona."""
        lines = [
    ...
    def test_import_auto_creates_taxonomy(self):
        """New domain tags → creates kensei2.taxonomy."""
        lines = [
