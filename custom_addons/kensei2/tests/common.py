# -*- coding: utf-8 -*-
"""Shared test fixtures for the Kensei2 test suite.

All Kensei2 test classes should inherit from ``Kensei2TestCase`` to get
...
class Kensei2TestCase(TransactionCase):
    """Base test case with shared fixtures for all Kensei2 tests."""
    ...
    def setUpClass(cls):
        ...
        # ── Model shortcuts ─────────────────────────────────────────
        cls.Talos = cls.env["kensei2.kensei2"]
        cls.Sandbox = cls.env["kensei2.sandbox"]
        cls.Turn = cls.env["kensei2.turn"]
        cls.Persona = cls.env["kensei2.persona"]
        cls.Domain = cls.env["kensei2.domain"]
        cls.Taxonomy = cls.env["kensei2.taxonomy"]
    ...
    def _create_task(cls, **overrides):
        """Create a kensei2.kensei2 record with sensible defaults."""
        vals = {
            "persona_id": cls.persona.id,
            "task_status": "NotSubmitted",
        }
    ...
    def _create_sandbox(cls, task=None, model_type="claude", **overrides):
        """Create a kensei2.sandbox record."""
    ...
    def _make_session_entry(trajectory_json_str=None, session_id="test-session-001"):
        ...
        if trajectory_json_str is None:
            traj = json.loads(Kensei2TestCase._make_trajectory_json())
        else:
