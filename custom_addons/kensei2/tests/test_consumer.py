...
class TestConsumerOdooHelpers(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer.xmlrpc.client.ServerProxy")
    def test_get_odoo_uid_success(self, mock_proxy_cls):
        from ..consumer import _get_odoo_uid
        import odoo.addons.kensei2.consumer as consumer_mod
    ...
    @patch("odoo.addons.kensei2.consumer._get_odoo_uid", return_value=1)
    @patch("odoo.addons.kensei2.consumer._get_odoo_models")
    def test_call_odoo_delegates_to_xmlrpc(self, mock_models, mock_uid):
        ...
        result = _call_odoo("kensei2.kensei2", "auto_process_claim_task", [99])
    ...
    def test_read_fields_passes_correct_params(self, mock_models, mock_uid):
        ...
        result = _read_fields("kensei2.sandbox", [1], ["docker_status", "docker_error"])
...
class TestWaitForSandboxRunning(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer._read_fields")
    ...
    def test_wait_transitions_to_running(self, mock_read):
        ...
        with patch("odoo.addons.kensei2.consumer.time.sleep"):
    ...
    @patch("odoo.addons.kensei2.consumer._read_fields")
    @patch("odoo.addons.kensei2.consumer.time")
...
class TestProcessTask(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer._call_odoo")
    @patch("odoo.addons.kensei2.consumer._wait_for_sandbox_running", return_value="running")
    @patch("odoo.addons.kensei2.consumer._read_fields")
...
class TestStartConsumer(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer._get_odoo_uid", return_value=1)
    @patch("odoo.addons.kensei2.consumer.pika")
    def test_start_consumer_connects_and_declares_queue(self, mock_pika, mock_uid):
        ...
        mock_channel.queue_declare.assert_called_once_with(
            queue="kensei2_auto_process", durable=True
        )
...
class TestProcessTaskFlow(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer._run_auto_hint_loop")
    @patch("odoo.addons.kensei2.consumer._wait_for_sandbox_running")
    @patch("odoo.addons.kensei2.consumer._call_odoo")
    def test_process_task_sandbox_already_running(
        ...
        with patch("odoo.addons.kensei2.consumer.OpenClawClient", mock_ws_cls):
            mock_conn = MagicMock()
            mock_channel = MagicMock()
            mock_channel.is_open = True
            from ..consumer import _process_task
    ...
    @patch("odoo.addons.kensei2.consumer.time.sleep")
    @patch("odoo.addons.kensei2.consumer._call_odoo")
    def test_process_task_ws_connect_all_retries_fail(self, mock_call, mock_sleep):
        ...
        with patch("odoo.addons.kensei2.consumer.OpenClawClient", mock_ws_cls):
            with patch("odoo.addons.kensei2.consumer.OpenClawError", exc_cls):
                with patch("odoo.addons.kensei2.consumer.OpenClawTimeoutError", exc_cls):
                    mock_conn = MagicMock()
                    mock_channel = MagicMock()
                    mock_channel.is_open = True
                    _process_task(mock_conn, mock_channel, 1, None, self._make_body())
    ...
    def test_process_task_ws_response_timeout(self, mock_call):
        ...
        with patch("odoo.addons.kensei2.consumer.OpenClawClient", mock_ws_cls):
            with patch("odoo.addons.kensei2.consumer.OpenClawTimeoutError", timeout_exc):
                with patch("odoo.addons.kensei2.consumer.OpenClawError", timeout_exc):
                    mock_conn = MagicMock()
                    mock_channel = MagicMock()
                    mock_channel.is_open = True
                    from ..consumer import _process_task
...
class TestAutoHintLoopExtended(TransactionCase):
    ...
    def test_hint_loop_ws_error_during_hint(self, mock_call):
        ...
        with patch("odoo.addons.kensei2.consumer.time.sleep"):
            with patch("odoo.addons.kensei2.consumer.OpenClawError", ws_exc):
                with patch("odoo.addons.kensei2.consumer.OpenClawTimeoutError", ws_exc):
                    from ..consumer import _run_auto_hint_loop
                    _run_auto_hint_loop(10, mock_ws, 1)
    ...
    @patch("odoo.addons.kensei2.consumer.HINT_EVAL_TIMEOUT", 0)
    @patch("odoo.addons.kensei2.consumer._call_odoo")
    def test_hint_loop_eval_timeout(self, mock_call):
        ...
        with patch("odoo.addons.kensei2.consumer.time.sleep"):
            with patch("odoo.addons.kensei2.consumer.time.time") as mock_time:
...
class TestConsumerHelpers(TransactionCase):
    ...
    @patch("odoo.addons.kensei2.consumer.ODOO_URL", "https://example.com")
    ...
    @patch("odoo.addons.kensei2.consumer.ODOO_URL", "http://localhost:8069")
