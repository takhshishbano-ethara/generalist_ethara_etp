...
class TestCostingData(Kensei2TestCase):
    ...
    def _call_costing(self, period="week"):
        from ..controllers.costing import Kensei2CostingController
        controller = Kensei2CostingController()
        with patch.object(type(controller), "__init__", lambda s: None):
            controller = Kensei2CostingController.__new__(Kensei2CostingController)
    ...
    def test_costing_week_returns_rows_and_totals(self):
        ...
        with patch("odoo.addons.kensei2.controllers.costing.request") as mock_request:
    ...
    def test_costing_empty_no_tasks(self):
        ...
        with patch("odoo.addons.kensei2.controllers.costing.request") as mock_request:
            mock_request.env = self.env
            mock_request.env["kensei2.kensei2"].sudo = lambda: self.env["kensei2.kensei2"].browse([])
            result = controller.costing_data(period="all")
