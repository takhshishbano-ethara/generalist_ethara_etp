# -*- coding: utf-8 -*-
"""Cross-cutting integration tests for the Kensei2 module.
...
from ..models.kensei2_sandbox import TRAJECTORY_FIELD_MAP
...
class TestFullLifecycle(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestCascadingDeletes(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestTokenAggregation(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestPersonaPropagation(Kensei2TestCase):
    """Verify persona data flows through task → sandbox chain."""

    def test_persona_fields_available_for_sandbox(self):
        """Sandbox can reach persona fields through kensei2_id.persona_id."""
        task = self._create_task(seed_prompt="Persona propagation test")
        sandbox = task.claude_sandbox_id

        persona = sandbox.kensei2_id.persona_id
        self.assertTrue(persona, "Sandbox should reach persona via kensei2_id")
