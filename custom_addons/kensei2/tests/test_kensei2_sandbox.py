...
class TestKensei2SandboxCreate(Kensei2TestCase):
    ...
    def test_unique_constraint_task_model(self):
        """Cannot create two sandboxes with the same (kensei2_id, model_type)."""
        with self.assertRaises(Exception):
            self.Sandbox.create({
                "kensei2_id": self.task.id,
                "model_type": "claude",
            })
    ...
    def test_ordering_by_model_type(self):
        ...
        sandboxes = self.Sandbox.search([("kensei2_id", "=", task2.id)])
        types = sandboxes.mapped("model_type")
        self.assertEqual(types, sorted(types))


@tagged("post_install", "-at_install")
class TestSandboxComputedFields(Kensei2TestCase):
    ...
    def test_dashboard_url_running_k8s_no_ws_host(self):
        ...
        self._make_running(self.claude_sandbox)
        url = self.claude_sandbox.docker_dashboard_url
        self.assertIn("kensei2-sandbox-", url)
        self.assertIn(".kensei2.svc.cluster.local:18789", url)
...
@tagged("post_install", "-at_install")
class TestSandboxPortAllocation(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestSandboxJSONL(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestSandboxTrajectoryBuilding(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestSandboxAutoProcess(Kensei2TestCase):
