# -*- coding: utf-8 -*-
"""Comprehensive tests for Kensei2, Kensei2Turn, Kensei2Taxonomy models
and utility functions in models/kensei2.py.
"""
import json
...
# ═══════════════════════════════════════════════════════════════════════
# 1. Kensei2 record creation
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestKensei2Creation(Kensei2TestCase):
    """Tests for Kensei2 record creation, ensure_sandboxes, and field defaults."""
...
@tagged("post_install", "-at_install")
class TestKensei2ComputedFields(Kensei2TestCase):
    """Tests for computed/related fields on the Kensei2 model."""
    ...
    def test_is_kensei2_admin_not_admin(self):
        """is_kensei2_admin is False for non-admin users."""
        ...
        with patch.object(type(self.env.user), "has_group", return_value=False):
            self.task.invalidate_recordset(["is_kensei2_admin"])
            self.task._compute_is_kensei2_admin()
            self.assertFalse(self.task.is_kensei2_admin)

    def test_is_kensei2_admin_admin(self):
        """is_kensei2_admin is True for quality lead users."""
        with patch.object(type(self.env.user), "has_group", return_value=True):
            self.task.invalidate_recordset(["is_kensei2_admin"])
            self.task._compute_is_kensei2_admin()
            self.assertTrue(self.task.is_kensei2_admin)
...
@tagged("post_install", "-at_install")
class TestKensei2Trajectory(Kensei2TestCase):
    ...
    def test_export_and_clear_turns(self):
        ...
        self.assertEqual(attachment.res_model, "kensei2.kensei2")
...
@tagged("post_install", "-at_install")
class TestKensei2Actions(Kensei2TestCase):
    """Tests for action methods on the Kensei2 model."""
    ...
    def test_action_view_turns(self):
        ...
        self.assertEqual(result["res_model"], "kensei2.turn")
    ...
    def test_action_generate_golden_no_claude_trajectory(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import _GOLDEN_GENERATING, _GOLDEN_LOCK
...
@tagged("post_install", "-at_install")
class TestKensei2AutoProcess(Kensei2TestCase):
    ...
    def test_publish_auto_process_filters_ineligible(self):
        ...
        with patch(
            "odoo.addons.kensei2.services.rabbitmq_service.batch_publish_auto_process_tasks"
        ) as mock_publish:
...
@tagged("post_install", "-at_install")
class TestKensei2Turn(Kensei2TestCase):
    """Tests for Kensei2Turn model: defaults, ordering, computed, cascade."""
    ...
    def test_related_kensei2_id(self):
        """kensei2_id is related to sandbox_id.kensei2_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.kensei2_id, self.task)

    def test_related_employee_id(self):
        """employee_id is related through kensei2_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.employee_id, self.task.employee_id)


# ═══════════════════════════════════════════════════════════════════════
# 7. Kensei2Taxonomy model
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestKensei2Taxonomy(Kensei2TestCase):
    """Tests for Kensei2Taxonomy model."""
...
@tagged("post_install", "-at_install")
class TestKensei2Utilities(Kensei2TestCase):
    """Tests for module-level utility functions in models/kensei2.py."""

    def test_load_dotenv_returns_dict(self):
        """_load_dotenv returns a dict containing os.environ keys."""
        from odoo.addons.kensei2.models.kensei2 import _load_dotenv
        result = _load_dotenv()
    ...
    def test_load_dotenv_parses_env_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(env_path, "w") as f:
                f.write("# comment\n")
                f.write("TEST_KENSEI2_VAR=hello123\n")
                f.write("\n")
                f.write("ANOTHER_VAR=world\n")
                f.write("MALFORMED_LINE\n")

            with patch("odoo.addons.kensei2.models.kensei2.odoo_config") as mock_config:
                mock_config.rcfile = os.path.join(tmpdir, "odoo.conf")
                result = _load_dotenv()
                self.assertEqual(result.get("TEST_KENSEI2_VAR"), "hello123")
    ...
    def test_is_degenerate_output_empty(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import _is_degenerate_output
    ...
    def test_wrap_trajectory_message_user_passthrough(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import _wrap_trajectory_message
    ...
    def test_wrap_messages_with_turn_feedback_empty_turns(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import _wrap_messages_with_turn_feedback
        messages = [
    ...
    def test_format_tool_result_none(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import _format_tool_result
    ...
    def test_generate_task_description_sync_missing_creds(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import generate_task_description_sync
        with patch("odoo.addons.kensei2.models.kensei2._load_dotenv", return_value={}):
    ...
    def test_generate_task_description_sync_degenerate_discarded(self):
        ...
        from odoo.addons.kensei2.models.kensei2 import generate_task_description_sync
        self._set_param("kensei2.bedrock_inference_arn", "arn:test")
        dotenv = {"AWS_BEARER_TOKEN_BEDROCK": "fake-key"}
        with patch("odoo.addons.kensei2.models.kensei2._load_dotenv", return_value=dotenv):
            with patch("odoo.addons.kensei2.models.kensei2._get_taskdesc_prompt", return_value="prompt"):
                with patch(
                    "odoo.addons.kensei2.controllers.llm_assisst_qc._call_bedrock_converse",
...
# ═══════════════════════════════════════════════════════════════════════
# 9. _search_is_kensei2_admin
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestAdminSearch(Kensei2TestCase):

    def test_search_admin_eq_true_admin(self):
        with patch.object(type(self.env.user), "has_group", return_value=True):
            domain = self.task._search_is_kensei2_admin("=", True)
    ...
    def test_search_admin_unsupported_operator(self):
        with self.assertRaises(ValueError):
            self.task._search_is_kensei2_admin(">", True)

    def test_search_admin_neq_false_admin(self):
        with patch.object(type(self.env.user), "has_group", return_value=True):
            domain = self.task._search_is_kensei2_admin("!=", False)
            self.assertEqual(domain, [])
...
@tagged("post_install", "-at_install")
class TestGenerateActions(Kensei2TestCase):
    ...
    def test_generate_taskdesc_already_generating(self):
        from odoo.addons.kensei2.models.kensei2 import _TASKDESC_GENERATING, _TASKDESC_LOCK
...
@tagged("post_install", "-at_install")
class TestDeleteTrajectoryEntry(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestAutoProcessExtended(Kensei2TestCase):
...
@tagged("post_install", "-at_install")
class TestGetAllTurns(Kensei2TestCase):
