...
function renderTrajectoryHtml(trajectory) {
    if (typeof trajectory === "string") {
        return `<pre class="kensei2-json-block"><code>${escapeHtml(trajectory)}</code></pre>`;
    }
    const pretty = JSON.stringify(trajectory, null, 2);
    return `<pre class="kensei2-json-block"><code>${syntaxHighlightJsonToHtml(pretty)}</code></pre>`;
}

export class Kensei2JsonField extends Component {
    static template = "kensei2.JsonField";
    static props = {
        ...standardFieldProps,
    ...
    _saveScrollPositions() {
        ...
        if (!root) return;
        const positions = [];
        const bodies = root.querySelectorAll(".kensei2-json-entry-body");
    ...
    async _handleGoldenStatusChanged(payload) {
        ...
        try {
            ...
            console.warn("[Kensei2JsonField] Failed to reload after golden status change:", e);
        }
    }
    ...
    onToggleEntry(index) {
        ...
        this.env.bus.dispatchEvent(
            new CustomEvent("KENSEI2:TRAJECTORY_EXPANDED", {
    ...
    get isKensei2Admin() {
        return !!this.props.record.data.is_kensei2_admin;
    }
    ...
    async onQcEntry(index) {
        ...
        try {
            const resp = await rpc("/kensei2/trajectory_qc", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
            });
    ...
    async onGenerateTaskDesc(index) {
        ...
        try {
            const resp = await rpc("/kensei2/generate_task_description", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
            });
    ...
    async onAbortQc(index) {
        ...
        try {
            await rpc("/kensei2/abort_trajectory_action", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
                action_type: "qc",
...
export const kensei2JsonField = {
    component: Kensei2JsonField,
...
registry.category("fields").add("kensei2_json", kensei2JsonField);
