...
export class TaskProgressWidget extends Component {
    static template = "kensei2.TaskProgressWidget";
    ...
    stepClass(step) {
        return `o_kensei2_progress_step o_kensei2_progress_step_${step.state}`;
    }

    connectorClass(conn) {
        return `o_kensei2_progress_connector ${conn.filled ? "o_kensei2_progress_connector_filled" : ""}`;
    }
}

export const taskProgressWidgetDef = { component: TaskProgressWidget };
registry.category("view_widgets").add("kensei2_task_progress", taskProgressWidgetDef);
