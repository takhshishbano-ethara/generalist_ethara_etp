...
class Kensei2TrackerBulkAllocation(models.TransientModel):
    ...
    tasker_line_ids = fields.One2many(
        'kensei2.tracker.bulk.allocation.line', 'wizard_id', string='Taskers')
    ...
    def default_get(self, fields_list):
        ...
        Member = self.env['kensei2.tracker.team.member']
    ...
    def _unassigned_persona_domain(self):
        ...
        ``kensei2.persona.assignment_status`` is a STORED, INDEXED compute kept in
    ...
    def action_start(self):
        ...
        Alloc = self.env['kensei2.tracker.allocation'].with_context(kensei2_skip_toast=True)
        if assignments:
            Alloc.create([
...
class Kensei2TrackerBulkAllocationLine(models.TransientModel):
    _name = 'kensei2.tracker.bulk.allocation.line'
    _description = 'Bulk Allocation — Tasker Line'
    _order = 'current_count, id'

    wizard_id = fields.Many2one(
        'kensei2.tracker.bulk.allocation', required=True, ondelete='cascade')
