...
class Kensei2TrackerStageHandoff(models.TransientModel):
    ...
    kensei2.tracker.allocation.stage_no / parent_id). This wizard creates the next
    ...
    tasker_member_id = fields.Many2one(
        'kensei2.tracker.team.member', string='Next Tasker', required=True,
    ...
    def _default_tasker(self):
        alloc = self.env['kensei2.tracker.allocation'].browse(
    ...
    def action_confirm(self):
        ...
        if not (self.env.su
                or self.env.user.has_group('kensei2.group_kensei2_ql')
        ...
        nxt = self.env['kensei2.tracker.allocation'].create({
