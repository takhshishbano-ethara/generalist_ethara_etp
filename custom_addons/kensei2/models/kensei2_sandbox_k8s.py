...
from .kensei2 import _DEFAULT_LITELLM_CONFIG, _load_dotenv
from .kensei2_sandbox import MODEL_DEFAULTS
...
WS_ROUTER_NAME = "kensei2-ws-router"
...
SANDBOX_SERVICE_ACCOUNT = "kensei2-sandbox"
...
server {
    ...
    location ~ ^/sandbox/(\\d+)(?:/(.*))? {
        set $task_id $1;
        set $path $2;
        set $backend kensei2-sandbox-$task_id.%(namespace)s.svc.cluster.local:18789;

        # CORS for cross-origin /v1/responses requests from Odoo frontend
    ...
    location ~ ^/litellm/(\\d+)(?:/(.*))? {
        set $task_id $1;
        set $path $2;
        set $backend kensei2-sandbox-$task_id.%(namespace)s.svc.cluster.local:4000;
        proxy_pass http://$backend/$path$is_args$args;
        proxy_http_version 1.1;
        proxy_set_header Host localhost;
...
WS_ROUTER_LABELS = {
    "platform": "kensei2",
    "component": "ws-router",
    "app.kubernetes.io/name": "kensei2-ws-router",
    "app.kubernetes.io/managed-by": "kensei2-odoo",
}
...
def _sandbox_labels(sandbox_record):
    return {
        ...
        "app.kubernetes.io/name": "kensei2-sandbox",
        "app.kubernetes.io/managed-by": "kensei2-odoo",
    }


def _resource_name(sandbox_record):
    return "kensei2-sandbox-%s" % sandbox_record.id
...
def _build_prestop_script(task_id, persona_name, s3_bucket=None, s3_prefix=None):
    ...
    prefix = s3_prefix or S3_KENSEI2_PREFIX
    ...
    return (
        "RUN_ID=$(cat /home/node/.openclaw/.kensei2-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
        'echo "[kensei2] preStop: run=$RUN_ID backing up session data to S3..." && '
        ...
        "--no-progress --quiet 2>/dev/null || true && "
        "echo '[kensei2] preStop: backup complete, waiting for connections to drain...' && "
        "sleep 10"
...
class Kensei2SandboxK8s(models.AbstractModel):
    _name = "kensei2.sandbox.k8s"
    _description = "Kensei2 K8s Sandbox Deployer"
    ...
    def deploy_sandbox(self, sandbox_record):
        ...
        task_id = sandbox_id
        persona = sandbox_record.kensei2_id.persona_id
        if not persona:
            raise UserError(
                "No persona selected on task '%s'. Please select a persona and save before starting."
                % (sandbox_record.kensei2_id.display_name or sandbox_record.kensei2_id.id)
            )
        ...
        aws_region = (env.get("KENSEI2_AWS_REGION") or env.get("AWS_REGION", "ap-south-1")).strip()
        bedrock_arn = (env.get("KENSEI2_BEDROCK_MODEL_ARN") or env.get("BEDROCK_MODEL_ARN", "")).strip()
        litellm_master_key = (env.get("KENSEI2_LITELLM_MASTER_KEY") or env.get("LITELLM_MASTER_KEY", "")).strip()
        if not litellm_master_key:
            litellm_master_key = "sk-kensei2-%s" % secrets.token_hex(8)
        litellm_db_password = (env.get("KENSEI2_LITELLM_DB_PASSWORD") or env.get("LITELLM_DB_PASSWORD", "")).strip()
        if not litellm_db_password:
            litellm_db_password = secrets.token_hex(16)
        llama_api_key = (env.get("KENSEI2_LLAMA_API_KEY") or env.get("LLAMA_API_KEY", "")).strip()
        moonshot_api_key = (env.get("KENSEI2_MOONSHOT_API_KEY") or env.get("MOONSHOT_API_KEY", "")).strip()
        openai_api_key = (env.get("KENSEI2_OPENAI_API_KEY") or env.get("OPENAI_API_KEY", "")).strip()

        openclaw_image = self._get_config_param(
            "kensei2.openclaw_image",
            "426628337772.dkr.ecr.ap-south-1.amazonaws.com/kensei2-q1-coding:v1.0.0",
        )
        litellm_image = self._get_config_param(
            "kensei2.litellm_image", "ghcr.io/berriai/litellm:main-stable"
        )
        postgres_image = self._get_config_param("kensei2.postgres_image", "postgres:16")
        aws_cli_image = self._get_config_param(
            "kensei2.aws_cli_image", "amazon/aws-cli:latest"
        )
        s3_bucket = self._get_config_param("kensei2.s3_bucket", S3_BUCKET)
        s3_prefix = self._get_config_param("kensei2.s3_prefix", S3_KENSEI2_PREFIX)
    ...
    def _create_secret(
        ...
        secret = client.V1Secret(
            ...
            metadata=client.V1ObjectMeta(
                name="kensei2-sandbox-creds-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            string_data={
    ...
    def _create_persona_configmap(self, core_v1, task_id, labels, persona):
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei2-sandbox-persona-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            data=data,
    ...
    def _create_gog_secret(self, core_v1, task_id, labels, sandbox_record):
        ...
        gog_auth_raw = sandbox_record.kensei2_id.gog_auth or ""
        if gog_auth_raw:
            try:
        ...
        gog_auth_token_raw = sandbox_record.kensei2_id.gog_auth_token or ""
        ...
        gog_kp = sandbox_record.kensei2_id.password or ""
        gog_account = sandbox_record.kensei2_id.email or ""
        ...
        if _has_token_files and not gog_kp:
            _logger.warning(
                ...
                "decrypt JWE tokens. Ensure kensei2_id.password is set.",
                task_id,
        ...
        secret = client.V1Secret(
            ...
            kind="Secret",
            metadata=client.V1ObjectMeta(
                name="kensei2-sandbox-gog-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_openclaw_config_configmap(
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei2-sandbox-openclaw-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_litellm_configmap(self, core_v1, task_id, labels, litellm_yaml):
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei2-litellm-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_skills_configmap(self, core_v1, task_id, labels, sandbox_record):
        ...
        cm_name = "kensei2-sandbox-skills-%s" % task_id
        cm = client.V1ConfigMap(
            api_version="v1",
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
    ...
    def _create_mock_data_configmaps(self, core_v1, task_id, labels):
        ...
        for entry in sorted(os.listdir(env_dir)):
            ...
            cm_name = "kensei2-mock-data-%s-%s" % (entry, task_id)
            cm = client.V1ConfigMap(
                api_version="v1",
                kind="ConfigMap",
                metadata=client.V1ObjectMeta(
    ...
    def _load_environment_services(self):
        ...
        from odoo.modules.module import get_module_path
        from .kensei2_sandbox import _parse_service_toml_fallback
        ...
        registry_prefix = ICP.get_param("kensei2.ecr_registry", "")
        image_tag = ICP.get_param("kensei2.mock_image_tag", "latest")
    ...
    def _create_deployment(
        ...
        secret_name = "kensei2-sandbox-creds-%s" % task_id
        gog_secret_name = "kensei2-sandbox-gog-%s" % task_id
        persona_cm = "kensei2-sandbox-persona-%s" % task_id
        openclaw_config_cm = "kensei2-sandbox-openclaw-config-%s" % task_id
        litellm_config_cm = "kensei2-litellm-config-%s" % task_id
        ...
        session_restore = client.V1Container(
                ...
                command=[
                    ...
                    "echo $RUN_ID > /data/session/.kensei2-run-id && "
        ...
        s3_init_tail.append(
            client.V1Container(
                ...
                command=[
                    "sh",
                    "-c",
                    "RUN_ID=$(cat /data/.kensei2-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
                    "echo '[kensei2] snapshot-start: saving fresh state as run-'$RUN_ID && "
        ...
        s3_init_tail.append(
            client.V1Container(
                ...
                command=[
                    ...
                    "chmod +x /gog-bin/gog && "
                    "ls -la /gog-bin/ || "
                    "echo '[kensei2] gog binary not found in image, skipping'",
                ],
                volume_mounts=[
                    client.V1VolumeMount(
                        name="gog-bin",
                ...
                pre_stop=client.V1LifecycleHandler(
                    _exec=client.V1ExecAction(
                        command=[
                            "sh",
                            "-c",
                            'echo "[kensei2] session-backup preStop: final sync..." && '
                            "aws s3 sync /data/session/ %s "
                            "--no-progress --quiet 2>/dev/null || true && "
                            "aws s3 sync /data/browser-profiles/ %s "
                            "--no-progress --quiet 2>/dev/null || true && "
                            'echo "[kensei2] session-backup preStop: done"'
    ...
    def _ensure_ws_router(self, core_v1, apps_v1, networking_v1, ws_host, nginx_image):
        ...
        cm_body = client.V1ConfigMap(
            metadata=client.V1ObjectMeta(
                name="kensei2-ws-router-conf",
                namespace=NAMESPACE,
                labels=WS_ROUTER_LABELS,
                annotations={"kensei2.ethara.ai/conf-hash": desired_hash},
            ),
        ...
        try:
            existing_cm = core_v1.read_namespaced_config_map(
                name="kensei2-ws-router-conf", namespace=NAMESPACE
            )
                ...
                patch = {
                    "spec": {
                        "template": {
                            "metadata": {
                                "annotations": {
                                    "kensei2.ethara.ai/conf-hash": desired_hash,
                                    "kensei2.ethara.ai/restarted-at": _dt.datetime.utcnow().isoformat(),
                                }
                            }
                        }
                    }
        ...
        if ws_host:
            # "kensei2-ws.example.com" → host only;
            # "projects.example.com/kensei2-sandbox" → host + path prefix
            if "/" in ws_host:
    ...
    def destroy_sandbox(self, sandbox_record):
        ...
        self._delete_resource(
            ...
            "kensei2-sandbox-creds-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_secret,
            "kensei2-sandbox-gog-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei2-sandbox-persona-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei2-sandbox-openclaw-config-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei2-litellm-config-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei2-sandbox-skills-%s" % task_id,
            NAMESPACE,
        )
        for svc_dir in self._list_mock_service_dirs():
            self._delete_resource(
                core_v1.delete_namespaced_config_map,
                "kensei2-mock-data-%s-%s" % (svc_dir, task_id),
                NAMESPACE,
            )
    ...
    def reconcile_sandboxes(self):
        ...
        NOTE: Prefer using kensei2.sandbox._cron_reconcile() (called via the
