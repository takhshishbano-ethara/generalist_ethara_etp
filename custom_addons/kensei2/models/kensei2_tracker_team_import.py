...
# normalised status spellings -> stored value (see kensei2.tracker.team.member)
_STATUS_MAP = {
    'active': 'active',
...
class Kensei2TrackerTeamImport(models.TransientModel):
    """Bulk-add existing Odoo users to the Kensei2 Tracker team roster.
    ...
    def action_import(self):
        ...
        self.ensure_one()
        entries = self._classify()
        Member = self.env['kensei2.tracker.team.member'].with_context(
            kensei2_skip_toast=True)
