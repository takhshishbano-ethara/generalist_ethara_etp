# Gohan 19.0.2.0.0 Deployment Runbook
...
| `<NAMESPACE>` | `ethara` |
| `<DEPLOYMENT>` | `etp-be` |
| `<LAMBDA_FN>` | `gohan-extraction` |
| `<ECR_REPO>` | `<ACCOUNT_ID>.dkr.ecr.<REGION>.amazonaws.com/gohan-extraction` |
...
### PF-2. Confirm `GOHAN_WEBHOOK_TOKEN` matches between Lambda and Odoo

```bash
aws lambda get-function-configuration --region <REGION> \
  --function-name <LAMBDA_FN> \
  --query 'Environment.Variables.GOHAN_WEBHOOK_TOKEN' --output text
```
...
```bash
kubectl -n <NAMESPACE> get deployment -l app=etp-gohan-consumer
kubectl -n <NAMESPACE> get configmap | grep -i gohan
kubectl -n <NAMESPACE> get secret | grep -i gohan
```
...
```bash
cd ethara-etp
git tag pre-gohan-19.0.2.0.0 origin/stage
git push origin pre-gohan-19.0.2.0.0
...
# Build
docker build -t gohan-extraction:<NEW_TAG> .

# Tag for ECR
docker tag gohan-extraction:<NEW_TAG> <ECR_REPO>:<NEW_TAG>
docker tag gohan-extraction:<NEW_TAG> <ECR_REPO>:latest
...
aws ecr describe-images --region <REGION> \
  --repository-name gohan-extraction \
...
# Expect:
#   Image:   .../gohan-extraction:<NEW_TAG>
#   Storage: 4096
...
```bash
# 3a. Create policy
cat > /tmp/gohan-odoo-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [{
...
aws iam create-policy \
  --policy-name gohan-odoo-invoke-policy \
  --policy-document file:///tmp/gohan-odoo-policy.json \
...
eksctl create iamserviceaccount \
  ...
  --namespace <NAMESPACE> \
  --cluster <CLUSTER> \
  --attach-policy-arn arn:aws:iam::<ACCOUNT_ID>:policy/gohan-odoo-invoke-policy \
  --approve \
...
# Expect a non-empty role ARN ending in "gohan-odoo-..."
...
**Settings → Gohan → Extraction AWS Credentials**. The IAM user attached to those keys must have `lambda:InvokeFunction` on the Lambda ARN.

**Rollback:**

```bash
eksctl delete iamserviceaccount --name <DEPLOYMENT> --namespace <NAMESPACE> --cluster <CLUSTER> --region <REGION>
aws iam delete-policy --policy-arn arn:aws:iam::<ACCOUNT_ID>:policy/gohan-odoo-invoke-policy
```
...
**What:** Roll the Odoo deployment to the v19.0.2.0.0 image, then upgrade the `gohan` addon. The addon's pre-migration script auto-handles DB changes.
...
kubectl -n <NAMESPACE> exec deploy/<DEPLOYMENT> -- \
  ...
  -d <DB_NAME> -u gohan --stop-after-init
```
...
Or via UI: **Settings → Gohan → Extraction Lambda → Lambda Function** → paste the ARN → Save.
...
kubectl -n <NAMESPACE> exec deploy/<DEPLOYMENT> -- \
  ...
  "SELECT column_name FROM information_schema.columns
     WHERE table_name='gohan_job' AND column_name IN ('via_rabbitmq', 'via_batch');"
# Expect: only via_batch (NOT via_rabbitmq)

# 5f. Confirm the new config seeded
kubectl -n <NAMESPACE> exec deploy/<DEPLOYMENT> -- \
  psql -h <PG_HOST> -U <USER> -d <DB_NAME> -c \
  "SELECT key, value FROM ir_config_parameter WHERE key LIKE 'gohan.%' ORDER BY key;"
...
kubectl -n <NAMESPACE> exec deploy/<DEPLOYMENT> -- \
  ...
  "SELECT name FROM ir_model_data
    WHERE module='gohan' AND name LIKE '%rabbitmq%';"
# Expect: 0 rows.
```
...
ALTER TABLE gohan_job ADD COLUMN via_rabbitmq BOOLEAN DEFAULT FALSE;
UPDATE gohan_job SET via_rabbitmq = via_batch;
ALTER TABLE gohan_job DROP COLUMN via_batch;
EOF
...
kubectl -n <NAMESPACE> delete deployment etp-gohan-consumer 2>/dev/null || echo "no consumer deployment"
kubectl -n <NAMESPACE> delete configmap gohan-consumer-env 2>/dev/null || echo "no consumer configmap"
kubectl -n <NAMESPACE> delete secret gohan-consumer-secrets 2>/dev/null || echo "no consumer secret"
```

**Verify:**

```bash
kubectl -n <NAMESPACE> get all -l app=etp-gohan-consumer
...
#   ... phases ...
#   "Callback to https://<your-odoo>/api/v1/gohan/webhook/extraction-complete returned 200"
```

#### 7b. Batch dispatch (the actual 250-concurrent test)

1. **Gohan → Configuration → Import URLs** → upload a CSV with 5 URLs.
2. **Gohan → All Tasks** → filter "Not Assigned" → select all 5.
...
| Lambda `Throttles` | CloudWatch | 0 sustained | Bump account quota; reduce `gohan.batch_concurrency` |
...
| Batch fails with `ResourceNotFoundException` | Wrong function name in settings | Set `gohan.lambda_function_name` to full ARN |
...
| Lambdas finish but jobs stay in `extracting` | Webhook auth failing | Confirm `GOHAN_WEBHOOK_TOKEN` matches (PF-2) |
...
kubectl apply -f /path/to/old/etp-gohan-consumer.yaml
...
- [ ] Step 5: Odoo on new image; migration ran (no `via_rabbitmq` column); `gohan.lambda_function_name` set
