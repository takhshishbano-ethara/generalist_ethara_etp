...
from `odoo.addons.gohan.*` and persists them to `gohan.job.log` for
...
def _resolve_db_name():
    # Odoo pins dbname on each thread for HTTP requests; cron / startup
    # paths fall back to the env var set in K8s manifests.
    th = threading.current_thread()
    return getattr(th, "dbname", None) or os.environ.get("ODOO_DB") or None


class GohanJobLogHandler(logging.Handler):
    """Filters records named ``odoo.addons.gohan*`` whose message carries
    ``[job=<id>]`` and writes them to ``gohan_job_log`` via direct SQL
    ...
    def emit(self, record):
        try:
            if not record.name.startswith("odoo.addons.gohan"):
                return
    ...
    def _resolve_job_id(self, match):
        ...
        if not prefix:
            ...
            try:
                ...
                return None
        # `[job=LEV-00012]` - look up by name with a small cache.
        ...
        try:
            ...
            with Registry(db_name).cursor() as cr:
                cr.execute("SELECT id FROM gohan_job WHERE name = %s LIMIT 1", (name,))
                row = cr.fetchone()
                if row:
    ...
    def _write_log(self, db_name, job_id, level, message, created):
        ...
        try:
            with Registry(db_name).cursor() as cr:
                # FK on job_id needs KEY SHARE on gohan_job; the parent
...
def install_handler():
    """Attach the handler to the root `odoo.addons.gohan` logger exactly once.
    Safe to call multiple times (idempotent via module-level guard)."""
    global _INSTALLED
    if _INSTALLED:
        return
    handler = GohanJobLogHandler()
    handler.setLevel(logging.INFO)
    target = logging.getLogger("odoo.addons.gohan")
    target.addHandler(handler)
    _INSTALLED = True
    _logger.info("[gohan] log handler installed (source=%s, hostname=%s)", _SOURCE, _HOSTNAME)
