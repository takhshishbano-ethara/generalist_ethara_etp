...
This file is intentionally NON-FUNCTIONAL. The Gohan pipeline no longer
...
1. In Odoo, select N rows in the Gohan list view.
...
The old ``consumer.py`` EKS Deployment YAML must be **deleted**. Specifically:
    ...
    kubectl -n ethara delete deployment etp-gohan-consumer  # if it exists
    kubectl -n ethara delete configmap gohan-consumer-env  # if it exists
...
def main() -> int:
    sys.stderr.write(
        "consumer.py was removed in gohan 19.0.2.0.0.\n"
