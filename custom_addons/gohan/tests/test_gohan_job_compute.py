...
class TestScoreDisplay(GohanTestCase):
...
@tagged("post_install", "-at_install", "gohan")
class TestStageProgress(GohanTestCase):
...
@tagged("post_install", "-at_install", "gohan")
class TestScoreReportHtml(GohanTestCase):
...
@tagged("post_install", "-at_install", "gohan")
class TestAssetPreviews(GohanTestCase):
    ...
    def test_screenshots_render_when_bucket_set(self):
        self._set_param("gohan.s3_bucket", "lev-bucket")
        self._set_param("gohan.s3_region", "us-east-1")
        job = self._create_job(
            screenshot_keys=["gohan/LEV-001/screenshots/01.png"],
        )
    ...
    def test_cdn_url_overrides_bucket(self):
        self._set_param("gohan.s3_bucket", "lev-bucket")
        self._set_param("gohan.s3_cdn_url", "https://cdn.example.com")
        job = self._create_job(
            screenshot_keys=["gohan/LEV-001/01.png"],
        )
        self.assertIn("https://cdn.example.com/gohan/", job.screenshot_urls_html)

    def test_asset_grouping(self):
        self._set_param("gohan.s3_bucket", "lev-bucket")
        job = self._create_job(asset_keys=[
            "gohan/LEV-001/deliverables/Page Assets/logo.svg",
            "gohan/LEV-001/deliverables/References/ref1.png",
            "gohan/LEV-001/deliverables/_unused/sample.jpg",
        ])
...
@tagged("post_install", "-at_install", "gohan")
class TestAssetScoreHtml(GohanTestCase):
...
@tagged("post_install", "-at_install", "gohan")
class TestIsAdmin(GohanTestCase):

    def test_admin_flag_for_admin_user(self):
        admin_group = self.env.ref("gohan.group_gohan_admin")
