...
@tagged("post_install", "-at_install", "gohan")
class TestStartTaskWizard(GohanTestCase):

    def test_confirm_without_category_picks_any(self):
        job = self._create_job()
        wiz = self.env["gohan.start.task.wizard"].create({})
...
@tagged("post_install", "-at_install", "gohan")
class TestBulkAssignWizard(GohanTestCase):

    def test_no_active_ids_raises(self):
        wiz = self.env["gohan.bulk.assign.wizard"].with_context(
            active_ids=[],
