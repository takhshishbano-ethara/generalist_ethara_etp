"""Vegeta Extraction Lambda — AWS Lambda handler.
...
Environment Variables:
    VEGETA_WEBHOOK_TOKEN — token sent to Odoo webhook (X-Vegeta-Token header)
...
logger = logging.getLogger("vegeta-extraction")
logger.setLevel(logging.INFO)

VEGETA_WEBHOOK_TOKEN = os.environ.get("VEGETA_WEBHOOK_TOKEN", "")
...
# Stop S3 uploads before Lambda hard-kill at 900s — leaves time for callback POST.
S3_UPLOAD_DEADLINE_SECONDS = 870

# Reused across warm-container invocations — re-creating per invoke burns connection pools at 250x concurrency.
_S3_CLIENT = None
_CALLBACK_HTTP_CLIENT: httpx.Client | None = None
_S3_UPLOAD_WORKERS = 8

_CALLBACK_MAX_ATTEMPTS = 5
_CALLBACK_BASE_BACKOFF_SECONDS = 2.0
_CALLBACK_PER_ATTEMPT_TIMEOUT = 30.0
_STARTED_PING_MAX_ATTEMPTS = 3
_STARTED_PING_TIMEOUT = 10.0

# Cap screenshots shipped to S3 per job — high-density categories may want this raised.
_MAX_SCREENSHOTS_UPLOADED = 10


def _is_private_or_reserved_host(host: str) -> bool:
    """Return True if hostname resolves to a non-publicly-routable address.

    Why: prevents SSRF where a malicious callback_url points at the AWS metadata
    endpoint (169.254.169.254), private VPC ranges, or IPv6 link-local — which
    would let the lambda exfiltrate IAM credentials via the callback body.
    Hostname literals plus DNS-resolved IPs are checked against the full
    private/loopback/link-local/reserved/multicast set.
    """
    if not host:
        return True
    ...
    for cand in candidates:
        ...
        if isinstance(ip, ipaddress.IPv6Address):
            mapped = ip.ipv4_mapped or ip.sixtofour or ip.teredo
...
def _get_s3_client():
    ...
    if _S3_CLIENT is None:
        ...
        _S3_CLIENT = boto3.client(
            ...
            config=Config(
                max_pool_connections=_S3_UPLOAD_WORKERS * 2,
...
def _get_callback_http_client() -> httpx.Client:
    global _CALLBACK_HTTP_CLIENT
    if _CALLBACK_HTTP_CLIENT is None:
        _CALLBACK_HTTP_CLIENT = httpx.Client(
            timeout=httpx.Timeout(60.0, connect=10.0),
            limits=httpx.Limits(max_keepalive_connections=4, max_connections=8),
        )
    return _CALLBACK_HTTP_CLIENT
...
def _handle_extract(event):
    ...
    try:
        ...
        return _cors_response(400, json.dumps({"error": "job_id must be an integer"}))

    try:
        cb_parsed = urlparse(callback_url)
    except ValueError:
        logger.warning("callback_url failed to parse: %r", callback_url)
        return _cors_response(400, json.dumps({"error": "callback_url is malformed"}))
    ...
    if _is_private_or_reserved_host(cb_host):
        logger.warning("callback_url points to private/reserved address: %s", cb_host)
        return _cors_response(400, json.dumps({"error": "callback_url must not be a private or reserved address"}))

    try:
        asyncio.run(_run_extraction(url, job_id_int, callback_url))
        return _cors_response(202, json.dumps({
            "success": True,
            "extraction_id": f"ext-{job_id_int}",
            "message": "Extraction complete, callback sent",
        }))
    except Exception as exc:
        logger.exception("Extraction failed for job %s", job_id_int)
...
async def _run_extraction(url: str, job_id: int, callback_url: str):
    ...
    from modules.api_doc_prober import probe_api_docs
    from modules.business_signals_extractor import extract_business_signals
    from modules.form_harvester import harvest_forms
    from modules.vegeta_classifier import classify_vegeta_category, compute_scrape_coverage
    from modules.page_typer import type_pages
    ...
    try:
        ...
        business_signals = {}
        vegeta_category = {}
        scrape_coverage = "marketing_only"
        observed_pages: list = []
        schema_org_types: list = []
        ...
        async with async_playwright() as pw:
            ...
            # ── API doc probe (OpenAPI/Swagger/GraphQL/robots/sitemap) ─
            try:
                api_doc_data = await probe_api_docs(page, url)
                if api_doc_data:
                    _save_json(api_doc_data, os.path.join(raw_dir, "api_doc_data.json"))
            except Exception as exc:
                logger.warning("[job=%s] API doc probe failed: %s", job_id, exc)
            ...
            # ── Form harvesting (signup/login/reset/mfa routes) ─────
            try:
                form_data = await harvest_forms(page, url, site_data.get("pages", []))
                if form_data:
                    _save_json(form_data, os.path.join(raw_dir, "form_data.json"))
            except Exception as exc:
                logger.warning("[job=%s] Form harvesting failed: %s", job_id, exc)

            # ── Business signals (pricing tiers / billing model) ────
            try:
                business_signals = await extract_business_signals(page, url, site_data.get("pages", []))
                if business_signals:
                    _save_json(business_signals, os.path.join(raw_dir, "business_signals.json"))
            except Exception as exc:
                logger.warning("[job=%s] Business signals extraction failed: %s", job_id, exc)
        ...
        # ── Schema.org types from JSON-LD (Tier 1 entity evidence) ──
        for node in (style_data or {}).get("seo", {}).get("json_ld", []) or []:
            if not isinstance(node, dict):
                continue
            graph_value = node.get("@graph")
            graph = graph_value if isinstance(graph_value, list) else [node]
            for item in graph:
                if not isinstance(item, dict):
                    continue
                t = item.get("@type")
                if isinstance(t, str):
                    schema_org_types.append(t)
                elif isinstance(t, list):
                    schema_org_types.extend(s for s in t if isinstance(s, str))
        schema_org_types = sorted({s for s in schema_org_types if s})

        # ── Page typing (landing/listing/detail/auth/dashboard/etc.) ─
        try:
            observed_pages = type_pages(
                site_data.get("pages", []),
                style_data=style_data,
                network_data=network_data,
            )
            if observed_pages:
                _save_json({"observed_pages": observed_pages}, os.path.join(raw_dir, "observed_pages.json"))
        except Exception as exc:
            logger.warning("[job=%s] Page typing failed: %s", job_id, exc)
            observed_pages = []

        # ── Vegeta business category classification (1 of 16) ───────
        try:
            vegeta_category = classify_vegeta_category(
                site_data=site_data,
                network_data=network_data,
                business_signals=business_signals,
                schema_org_types=schema_org_types,
                observed_routes=site_data.get("pages", []),
                api_doc_data=api_doc_data,
            )
            if vegeta_category:
                _save_json(vegeta_category, os.path.join(raw_dir, "vegeta_category.json"))
        except Exception as exc:
            logger.warning("[job=%s] Vegeta classifier failed: %s", job_id, exc)
            vegeta_category = {}

        # ── Scrape coverage (marketing_only/public/authenticated) ───
        try:
            scrape_coverage = compute_scrape_coverage(
                site_data=site_data,
                auth_data=auth_data,
                form_data=form_data,
                observed_routes_count=len(site_data.get("pages", []) or []),
                vegeta_category=(vegeta_category or {}).get("category"),
            )
        except Exception as exc:
            logger.warning("[job=%s] Scrape-coverage compute failed: %s", job_id, exc)
            scrape_coverage = "marketing_only"
        ...
        prd_prompt = build_prd_prompt(
            ...
            form_data=form_data,
            business_signals=business_signals,
            vegeta_category=vegeta_category,
        ...
        await _finalize_and_callback(
            ...
            api_doc_data=api_doc_data, form_data=form_data,
            business_signals=business_signals, vegeta_category=vegeta_category,
            scrape_coverage=scrape_coverage, observed_pages=observed_pages,
            schema_org_types=schema_org_types,
        )
...
async def _finalize_and_callback(
    ...
    api_doc_data=None, form_data=None, business_signals=None,
    vegeta_category=None, scrape_coverage=None,
    observed_pages=None, schema_org_types=None,
):
    ...
    if prd_prompt_override:
        ...
        try:
            prd_prompt = build_prd_prompt(
                ...
                dark_mode_data=dark_mode_data,
                output_dir=work_dir,
                api_doc_data=api_doc_data or {},
                form_data=form_data or {},
                business_signals=business_signals or {},
                vegeta_category=vegeta_category or {},
                scrape_coverage=scrape_coverage or "marketing_only",
                observed_pages=observed_pages or [],
                schema_org_types=schema_org_types or [],
            )
        except Exception as exc:
    ...
    if S3_BUCKET:
        try:
            screenshot_keys, asset_keys = _upload_to_s3(
                job_id, work_dir,
                hard_deadline=start_time + S3_UPLOAD_DEADLINE_SECONDS,
            )
        except Exception as exc:
    ...
    signals = {
        "vegeta_category": vegeta_category or {},
        "api_doc": _strip(api_doc_data or {}, {"probe_log"}),
        "business": _strip(business_signals or {}, {"probe_log", "raw_text_sample"}),
        "network": _strip(network_data or {}, set()),
        "auth": _strip(auth_data or {}, set()),
        "forms": _strip(form_data or {}, {"probe_log"}),
        "scrape_coverage": scrape_coverage or "marketing_only",
    }
    callback_payload["signals"] = signals
...
def _upload_to_s3(job_id: int, work_dir: str, hard_deadline: float = 0) -> tuple[list, list]:
    ...
    def _upload_one(fpath: str, key: str, content_type: str | None = None) -> str | None:
        try:
            extra = {"ContentType": content_type} if content_type else None
            if extra:
                client.upload_file(fpath, S3_BUCKET, key, ExtraArgs=extra)
            else:
    ...
    def _run_batch(jobs: list[tuple[str, str, str | None]]) -> list[str]:
        if not jobs:
            return []
        results: list[str] = []
        with ThreadPoolExecutor(max_workers=_S3_UPLOAD_WORKERS, thread_name_prefix=f"s3-{job_id}") as pool:
            futures = [pool.submit(_upload_one, fp, k, ct) for fp, k, ct in jobs]
    ...
    ss_dir = os.path.join(work_dir, "screenshots")
    ss_jobs: list[tuple[str, str, str | None]] = []
    if os.path.isdir(ss_dir):
        for fname in sorted(os.listdir(ss_dir))[:_MAX_SCREENSHOTS_UPLOADED]:
            if not _time_left():
            ...
            if (os.path.isfile(fpath) and os.path.getsize(fpath) > 0
                ...
                ss_jobs.append((fpath, key, content_type))
    screenshot_keys.extend(_run_batch(ss_jobs))

    assets_dir = os.path.join(work_dir, "assets")
    asset_jobs: list[tuple[str, str, str | None]] = []
    if os.path.isdir(assets_dir):
        for subdir in ["images", "svgs", "fonts", "videos", "logos", "json", "textures"]:
            ...
            for fname in sorted(os.listdir(sub_path)):
                ...
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 0:
                    asset_jobs.append((fpath, f"{prefix}/assets/{subdir}/{fname}", None))
    asset_keys.extend(_run_batch(asset_jobs))

    raw_dir = os.path.join(work_dir, "raw_data")
    if os.path.isdir(raw_dir) and _time_left():
        bundle = {}
            ...
            if os.path.isfile(fpath) and fname.endswith(".json"):
                try:
                    with open(fpath, "r") as f:
                        bundle[os.path.splitext(fname)[0]] = json.load(f)
                except Exception:
                    pass
        if bundle:
    ...
    dlv_jobs: list[tuple[str, str, str | None]] = []
    if os.path.isdir(dlv_dir):
        for root, _dirs, files in os.walk(dlv_dir):
            ...
            for fname in sorted(files):
                ...
                dlv_jobs.append((fpath, f"{prefix}/deliverables/{rel}", None))
    asset_keys.extend(_run_batch(dlv_jobs))
...
def _send_callback(callback_url: str, payload: dict):
    """POST results back to Odoo. 5xx and network failures retry with full
    jitter. 4xx is a permanent failure (token mismatch, wrong job_id) and is
    NOT retried — logged loudly so the operator sees it.
    """
    headers = {"Content-Type": "application/json"}
    if VEGETA_WEBHOOK_TOKEN:
        headers["X-Vegeta-Token"] = VEGETA_WEBHOOK_TOKEN

    client = _get_callback_http_client()
    for attempt in range(_CALLBACK_MAX_ATTEMPTS):
        try:
            resp = client.post(
                callback_url, json=payload, headers=headers,
                timeout=_CALLBACK_PER_ATTEMPT_TIMEOUT,
            )
            status = resp.status_code
            logger.info(
                "Callback to %s returned %d (attempt %d/%d)",
                callback_url, status, attempt + 1, _CALLBACK_MAX_ATTEMPTS,
            )
            if 200 <= status < 400:
                return
            if status != 429 and 400 <= status < 500:
                logger.error(
                    "Callback rejected with %d (permanent, not retrying): %s",
                    status, resp.text[:512],
                )
                return
        except Exception as exc:
            logger.warning(
                "Callback attempt %d/%d failed: %s",
                attempt + 1, _CALLBACK_MAX_ATTEMPTS, exc,
            )

        if attempt < _CALLBACK_MAX_ATTEMPTS - 1:
            wait = min(
                random.random() * _CALLBACK_BASE_BACKOFF_SECONDS * (2 ** attempt),
                30.0,
            )
            logger.info("Retrying callback in %.1fs...", wait)
            time.sleep(wait)

    logger.error(
        "All %d callback attempts failed for %s",
        _CALLBACK_MAX_ATTEMPTS, callback_url,
    )


def _send_started_ping(callback_url: str, job_id: int):
    """Fire-and-forget 'extraction started' heartbeat. Short retries so a single
    brief Odoo 5xx doesn't drop all 250 concurrent heartbeats and trip the
    watchdog. Never blocks extraction.
    """
    headers = {"Content-Type": "application/json"}
    if VEGETA_WEBHOOK_TOKEN:
        headers["X-Vegeta-Token"] = VEGETA_WEBHOOK_TOKEN

    body = {"job_id": job_id, "status": "started"}
    client = _get_callback_http_client()
    for attempt in range(_STARTED_PING_MAX_ATTEMPTS):
        try:
            resp = client.post(
                callback_url, json=body, headers=headers,
                timeout=_STARTED_PING_TIMEOUT,
            )
            logger.info(
                "[job=%s] Started ping attempt %d -> %d",
                job_id, attempt + 1, resp.status_code,
            )
            if resp.status_code < 500:
                return
        except Exception as exc:
            logger.warning(
                "[job=%s] Started ping attempt %d failed: %s",
                job_id, attempt + 1, exc,
            )
        if attempt < _STARTED_PING_MAX_ATTEMPTS - 1:
