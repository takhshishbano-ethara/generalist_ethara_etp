...
class TestScoreDisplay(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestStageProgress(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestScoreReportHtml(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestAssetPreviews(VegetaTestCase):
    ...
    def test_screenshots_render_when_bucket_set(self):
        self._set_param("vegeta.s3_bucket", "lev-bucket")
        self._set_param("vegeta.s3_region", "us-east-1")
        job = self._create_job(
            screenshot_keys=["vegeta/LEV-001/screenshots/01.png"],
        )
    ...
    def test_cdn_url_overrides_bucket(self):
        self._set_param("vegeta.s3_bucket", "lev-bucket")
        self._set_param("vegeta.s3_cdn_url", "https://cdn.example.com")
        job = self._create_job(
            screenshot_keys=["vegeta/LEV-001/01.png"],
        )
        self.assertIn("https://cdn.example.com/vegeta/", job.screenshot_urls_html)

    def test_asset_grouping(self):
        self._set_param("vegeta.s3_bucket", "lev-bucket")
        job = self._create_job(asset_keys=[
            "vegeta/LEV-001/deliverables/Page Assets/logo.svg",
            "vegeta/LEV-001/deliverables/References/ref1.png",
            "vegeta/LEV-001/deliverables/_unused/sample.jpg",
        ])
...
@tagged("post_install", "-at_install", "vegeta")
class TestAssetScoreHtml(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestIsAdmin(VegetaTestCase):

    def test_admin_flag_for_admin_user(self):
        admin_group = self.env.ref("vegeta.group_vegeta_admin")
