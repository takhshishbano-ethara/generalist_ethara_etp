...
class TestActionStartTask(VegetaTestCase):
    ...
    def test_bandwidth_limit_blocks_pickup(self):
        self._set_param("vegeta.max_jobs_per_user", "1")
    ...
    def test_bandwidth_zero_means_unlimited(self):
        self._set_param("vegeta.max_jobs_per_user", "0")
        for _ in range(3):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionReleaseTask(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionResetSelected(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionRun(VegetaTestCase):
    ...
    def test_opens_rerun_wizard_when_extraction_data_present(self):
        ...
        self.assertEqual(result["res_model"], "vegeta.rerun.wizard")
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionCancel(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionDiscard(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionReopen(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionRetry(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionMarkSubmitted(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionRunBatchConcurrent(VegetaTestCase):
    ...
    def test_requires_lambda_function_for_extract_path(self):
        job = self._create_job()
        self._set_param("vegeta.lambda_function_name", "")
        with self.assertRaises(UserError):
            job.action_run_batch_concurrent()

    def test_splits_extract_vs_generate(self):
        self._set_param("vegeta.lambda_function_name", "lev-extract")
        a = self._create_job()
...
@tagged("post_install", "-at_install", "vegeta")
class TestWatchdogCron(VegetaTestCase):
...
@tagged("post_install", "-at_install", "vegeta")
class TestActionDownloadZip(VegetaTestCase):
