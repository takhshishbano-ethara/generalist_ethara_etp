"""
Asyncio-based WebSocket client for OpenClaw gateways in Kensei sandbox containers.
...
logger = logging.getLogger("kensei.ws_client")
...
class OpenClawClient:
    ...
    def _next_id(self):
        self._msg_counter += 1
        ts = int(time.time())
        return f"kensei-{self._msg_counter}-{ts:x}"
    ...
    async def _handshake(self, ws):
        ...
        connect_msg = {
            ...
            "params": {
                ...
                "userAgent": "kensei-auto-process/1.0",
                "locale": "en-US",
            },
        }
