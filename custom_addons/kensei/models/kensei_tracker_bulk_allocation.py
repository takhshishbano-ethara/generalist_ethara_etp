...
class KenseiTrackerBulkAllocation(models.TransientModel):
    _name = 'kensei.tracker.bulk.allocation'
    ...
    tasker_line_ids = fields.One2many(
        'kensei.tracker.bulk.allocation.line', 'wizard_id', string='Taskers')
    ...
    def _persona_entries(self, create_missing):
        ...
        if self.source_mode == 'unassigned':
            personas = self.env['kensei.persona'].search(
    ...
    def action_start(self):
        ...
        Alloc = self.env['kensei.tracker.allocation'].with_context(kensei_skip_toast=True)
        if assignments:
            Alloc.create([
...
class KenseiTrackerBulkAllocationLine(models.TransientModel):
    _name = 'kensei.tracker.bulk.allocation.line'
    _description = 'Bulk Allocation — Tasker Line'
    _order = 'current_count, id'

    wizard_id = fields.Many2one(
        'kensei.tracker.bulk.allocation', required=True, ondelete='cascade')
