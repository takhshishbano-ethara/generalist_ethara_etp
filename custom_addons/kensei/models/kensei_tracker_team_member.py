...
class KenseiTrackerTeamMember(models.Model):
    """A curated Kensei Tracker team roster.
    ...
    """
    _name = 'kensei.tracker.team.member'
    ...
    def _compute_role(self):
        ...
        ladder = [
            ...
            ('tasker', {gid('etp_user_roles.group_tasker'), gid('kensei.group_kensei_tasker')}),
        ]
    ...
    #  has its own result summary (pass context ``kensei_skip_toast``).
    # ------------------------------------------------------------------ #
    @api.model_create_multi
    def create(self, vals_list):
        records = super().create(vals_list)
        if not self.env.context.get('kensei_skip_toast'):
