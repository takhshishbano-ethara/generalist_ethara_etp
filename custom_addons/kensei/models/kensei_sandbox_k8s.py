...
from .kensei import _DEFAULT_LITELLM_CONFIG, _load_dotenv
from .kensei_sandbox import MODEL_DEFAULTS
...
server {
    ...
    location ~ ^/litellm/(\\d+)(?:/(.*))? {
        set $task_id $1;
        set $path $2;
        set $backend kensei-sandbox-$task_id.%(namespace)s.svc.cluster.local:4000;
        proxy_pass http://$backend/$path$is_args$args;
        proxy_http_version 1.1;
        proxy_set_header Host localhost;
...
WS_ROUTER_LABELS = {
    "platform": "kensei",
    "component": "ws-router",
    "app.kubernetes.io/name": "kensei-ws-router",
    "app.kubernetes.io/managed-by": "kensei-odoo",
}
...
def _sandbox_labels(sandbox_record):
    return {
        ...
        "app.kubernetes.io/name": "kensei-sandbox",
        "app.kubernetes.io/managed-by": "kensei-odoo",
    }


def _resource_name(sandbox_record):
    return "kensei-sandbox-%s" % sandbox_record.id
...
def _build_prestop_script(task_id, persona_name):
    ...
    return (
        "RUN_ID=$(cat /home/node/.openclaw/.kensei-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
        'echo "[kensei] preStop: run=$RUN_ID backing up session data to S3..." && '
        ...
        "--no-progress --quiet 2>/dev/null || true && "
        "echo '[kensei] preStop: backup complete, waiting for connections to drain...' && "
        "sleep 10"
...
class KenseiSandboxK8s(models.AbstractModel):
    _name = "kensei.sandbox.k8s"
    _description = "Kensei K8s Sandbox Deployer"
    ...
    def deploy_sandbox(self, sandbox_record):
        ...
        task_id = sandbox_id
        persona = sandbox_record.kensei_id.persona_id
        if not persona:
            raise UserError(
                "No persona selected on task '%s'. Please select a persona and save before starting."
                % (sandbox_record.kensei_id.display_name or sandbox_record.kensei_id.id)
            )
        ...
        openclaw_image = self._get_config_param(
            ...
            "426628337772.dkr.ecr.ap-south-1.amazonaws.com/kensei-q1-coding:v1.0.0",
        )
        litellm_image = self._get_config_param(
            "kensei.litellm_image", "ghcr.io/berriai/litellm:main-stable"
        )
        postgres_image = self._get_config_param("kensei.postgres_image", "postgres:16")
        aws_cli_image = self._get_config_param(
            "kensei.aws_cli_image", "amazon/aws-cli:latest"
        )
        s3_bucket = self._get_config_param("kensei.s3_bucket", S3_BUCKET)
        s3_prefix = self._get_config_param("kensei.s3_prefix", S3_KENSEI_PREFIX)
        ...
        kensei_ws_host = self._get_config_param("kensei.ws_router_host", "")
        nginx_image = self._get_config_param("kensei.nginx_image", "nginx:alpine")
        self._ensure_ws_router(
            core_v1, apps_v1, networking_v1, kensei_ws_host, nginx_image
        )
    ...
    def _create_secret(
        ...
        secret = client.V1Secret(
            ...
            metadata=client.V1ObjectMeta(
                name="kensei-sandbox-creds-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            string_data={
    ...
    def _create_persona_configmap(self, core_v1, task_id, labels, persona):
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei-sandbox-persona-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
            data=data,
    ...
    def _create_gog_secret(self, core_v1, task_id, labels, sandbox_record):
        ...
        gog_auth_raw = sandbox_record.kensei_id.gog_auth or ""
        if gog_auth_raw:
            try:
        ...
        gog_auth_token_raw = sandbox_record.kensei_id.gog_auth_token or ""
        ...
        gog_kp = sandbox_record.kensei_id.password or ""
        gog_account = sandbox_record.kensei_id.email or ""
        ...
        if _has_token_files and not gog_kp:
            _logger.warning(
                ...
                "decrypt JWE tokens. Ensure kensei_id.password is set.",
                task_id,
        ...
        secret = client.V1Secret(
            ...
            kind="Secret",
            metadata=client.V1ObjectMeta(
                name="kensei-sandbox-gog-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_openclaw_config_configmap(
        ...
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei-sandbox-openclaw-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_litellm_configmap(self, core_v1, task_id, labels, litellm_yaml):
        cm = client.V1ConfigMap(
            ...
            kind="ConfigMap",
            metadata=client.V1ObjectMeta(
                name="kensei-litellm-config-%s" % task_id,
                namespace=NAMESPACE,
                labels=labels,
            ),
    ...
    def _create_deployment(
        ...
        gog_secret_name = "kensei-sandbox-gog-%s" % task_id
        persona_cm = "kensei-sandbox-persona-%s" % task_id
        openclaw_config_cm = "kensei-sandbox-openclaw-config-%s" % task_id
        litellm_config_cm = "kensei-litellm-config-%s" % task_id
        ...
        init_containers = [
            client.V1Container(
                ...
                command=[
                    ...
                    "echo $RUN_ID > /data/session/.kensei-run-id && "
        ...
        init_containers.extend([
            client.V1Container(
                ...
                command=[
                    "sh",
                    "-c",
                    "RUN_ID=$(cat /data/.kensei-run-id 2>/dev/null || TZ=Asia/Kolkata date +%%Y-%%m-%%d_%%H-%%M-%%S-IST) && "
                    "echo '[kensei] snapshot-start: saving fresh state as run-'$RUN_ID && "
            ...
            client.V1Container(
                ...
                command=[
                    ...
                    "chmod +x /gog-bin/gog && "
                    "ls -la /gog-bin/ || "
                    "echo '[kensei] gog binary not found in image, skipping'",
                ],
                volume_mounts=[
                    client.V1VolumeMount(
                        name="gog-bin",
    ...
    def _ensure_ws_router(self, core_v1, apps_v1, networking_v1, ws_host, nginx_image):
        ...
        cm_body = client.V1ConfigMap(
            metadata=client.V1ObjectMeta(
                name="kensei-ws-router-conf",
                namespace=NAMESPACE,
                labels=WS_ROUTER_LABELS,
                annotations={"kensei.ethara.ai/conf-hash": desired_hash},
            ),
        ...
        try:
            existing_cm = core_v1.read_namespaced_config_map(
                name="kensei-ws-router-conf", namespace=NAMESPACE
            )
                ...
                patch = {
                    "spec": {
                        "template": {
                            "metadata": {
                                "annotations": {
                                    "kensei.ethara.ai/conf-hash": desired_hash,
                                    "kensei.ethara.ai/restarted-at": _dt.datetime.utcnow().isoformat(),
                                }
                            }
                        }
                    }
    ...
    def destroy_sandbox(self, sandbox_record):
        ...
        self._delete_resource(
            ...
            "kensei-sandbox-creds-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_secret,
            "kensei-sandbox-gog-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei-sandbox-persona-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
            "kensei-sandbox-openclaw-config-%s" % task_id,
            NAMESPACE,
        )
        self._delete_resource(
            core_v1.delete_namespaced_config_map,
    ...
    def reconcile_sandboxes(self):
        ...
        NOTE: Prefer using kensei.sandbox._cron_reconcile() (called via the
