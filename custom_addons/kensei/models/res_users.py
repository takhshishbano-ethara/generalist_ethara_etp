...
class ResUsers(models.Model):
    ...
    @api.depends_context('kensei_show_email')
    def _compute_display_name(self):
        """Show the email address (not the name) when a field explicitly asks
        for it via ``context={'kensei_show_email': True}``.

        Odoo's ``res.users`` display is just the name; only ``res.partner``
        honours the stock ``show_email`` key. The Kensei Team-Management "Email"
        ...
        if not self.env.context.get('kensei_show_email'):
            return
        for user in self:
