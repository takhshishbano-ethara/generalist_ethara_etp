    ...
    for attempt in range(max_retries):
        try:
            ...
            with Registry(db_name).cursor() as cr:
                ...
                turn = env["kensei.turn"].browse(turn_id)
                if not turn.exists():
...
class AutoHintController(http.Controller):
    ...
    @http.route("/kensei/auto_hint_eval", type="json", auth="user")
