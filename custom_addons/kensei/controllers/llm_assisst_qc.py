...
from ..models.kensei import _load_dotenv
...
def _update_qc_entry(env, record_id, field_name, entry_index, qc_status, qc_result):
    ...
    for attempt in range(max_retries):
        try:
            task = env["kensei.kensei"].browse(record_id)
            if not task.exists():
                return
...
class LlmAssistQc(http.Controller):
    @http.route("/kensei/qc", type="json", auth="user")
    def qc_prompt(
        self, prompt="", system_prompt="", max_tokens=4096, temperature=0.3, **kw
    ):
        ICP = request.env["ir.config_parameter"].sudo()
        if ICP.get_param("kensei.disable_prompt_qc", "False").lower() == "true":
        ...
        if not inference_arn:
            return {"error": "Bedrock Inference ARN not configured in Settings > Kensei"}
    ...
    @http.route("/kensei/trajectory_qc", type="json", auth="user")
    def trajectory_qc(self, record_id=0, field_name="", entry_index=-1, **kw):
        ICP = request.env["ir.config_parameter"].sudo()
        if ICP.get_param("kensei.disable_trajectory_qc", "False").lower() == "true":
        ...
        task = request.env["kensei.kensei"].browse(record_id)
        if not task.exists():
    ...
    @http.route("/kensei/generate_task_description", type="json", auth="user")
    def generate_task_description(
        ...
        from ..models.kensei import _TASKDESC_POOL
        from ..models.kensei_sandbox import _inject_task_description_bg
    ...
    @http.route("/kensei/abort_trajectory_action", type="json", auth="user")
