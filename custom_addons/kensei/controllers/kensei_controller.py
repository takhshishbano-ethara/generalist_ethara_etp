...
class Kensei(http.Controller):
    ...
    def method_get_jsonl_data(self, **params):
        try:
            ...
            KenseiModel = request.env['kensei.kensei'].sudo()
            PersonaModel = request.env['kensei.persona'].sudo()
            DomainModel = request.env['kensei.domain'].sudo()
            TaxonomyModel = request.env['kensei.taxonomy'].sudo()
            ...
            for item in data:
                ...
                # Skip duplicates
                if task_id and KenseiModel.search([('task_id', '=', task_id)], limit=1):
                ...
                record = KenseiModel.create(vals)
