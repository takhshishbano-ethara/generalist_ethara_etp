...
class KenseiController(http.Controller):
    ...
    @http.route('/kensei/harbor/download', type='http', auth='user', methods=['GET'])
    def download_harbor_bundle(self, **kwargs):
        ...
        is_ql = user.has_group('kensei.group_kensei_ql')
        is_pl = user.has_group('kensei.group_kensei_pl')
        ...
        prefix = icp.get_param("kensei.s3_prefix") or "Kensei"

        if not bucket:
            return http.Response(
                json.dumps({'error': 'S3 bucket not configured. Go to Settings → Kensei.'}),
                content_type='application/json', status=400
            )
