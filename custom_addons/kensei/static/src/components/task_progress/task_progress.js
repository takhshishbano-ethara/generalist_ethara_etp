...
export class TaskProgressWidget extends Component {
    static template = "kensei.TaskProgressWidget";
    ...
    stepClass(step) {
        return `o_kensei_progress_step o_kensei_progress_step_${step.state}`;
    }

    connectorClass(conn) {
        return `o_kensei_progress_connector ${conn.filled ? "o_kensei_progress_connector_filled" : ""}`;
    }
}

export const taskProgressWidgetDef = { component: TaskProgressWidget };
registry.category("view_widgets").add("kensei_task_progress", taskProgressWidgetDef);
