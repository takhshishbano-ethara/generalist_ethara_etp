...
function renderTrajectoryHtml(trajectory) {
    if (typeof trajectory === "string") {
        return `<pre class="kensei-json-block"><code>${escapeHtml(trajectory)}</code></pre>`;
    }
    const pretty = JSON.stringify(trajectory, null, 2);
    return `<pre class="kensei-json-block"><code>${syntaxHighlightJsonToHtml(pretty)}</code></pre>`;
}

export class KenseiJsonField extends Component {
    static template = "kensei.JsonField";
    static props = {
        ...standardFieldProps,
    ...
    _saveScrollPositions() {
        ...
        if (!root) return;
        const positions = [];
        const bodies = root.querySelectorAll(".kensei-json-entry-body");
    ...
    async _handleGoldenStatusChanged(payload) {
        ...
        try {
            ...
            console.warn("[KenseiJsonField] Failed to reload after golden status change:", e);
        }
    }
    ...
    get isKenseiAdmin() {
        return !!this.props.record.data.is_kensei_admin;
    }
    ...
    async onQcEntry(index) {
        ...
        try {
            const resp = await rpc("/kensei/trajectory_qc", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
            });
    ...
    async onGenerateTaskDesc(index) {
        ...
        try {
            const resp = await rpc("/kensei/generate_task_description", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
            });
    ...
    async onAbortQc(index) {
        ...
        try {
            await rpc("/kensei/abort_trajectory_action", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
                action_type: "qc",
...
export const kenseiJsonField = {
    component: KenseiJsonField,
...
registry.category("fields").add("kensei_json", kenseiJsonField);
