...
class TestKenseiSettings(KenseiTestCase):

    def test_default_deployment_mode_is_local(self):
        settings = self.env["res.config.settings"].create({})
        self.assertIn(settings.kensei_deployment_mode, ("local", False))

    def test_default_bedrock_region(self):
        settings = self.env["res.config.settings"].create({})
        region = settings.kensei_bedrock_region or "ap-south-1"
    ...
    def test_docker_available_true_when_docker_returns_0(self):
        ...
        with mock_result as mock_run:
            ...
            settings = self.env["res.config.settings"].create({})
            settings._compute_kensei_docker_available()
            self.assertTrue(settings.kensei_docker_available)
    ...
    def test_docker_available_false_when_docker_returns_nonzero(self):
        with patch("subprocess.run") as mock_run:
            ...
            settings = self.env["res.config.settings"].create({})
            settings._compute_kensei_docker_available()
            self.assertFalse(settings.kensei_docker_available)
    ...
    def test_config_params_persist(self):
        self._set_param("kensei.bedrock_inference_arn", "arn:aws:bedrock:test:12345:inference-profile/test")
        self._set_param("kensei.bedrock_region", "us-west-2")
        self._set_param("kensei.deployment_mode", "k8s")

        self.assertEqual(
            self.ICP.get_param("kensei.bedrock_inference_arn"),
            "arn:aws:bedrock:test:12345:inference-profile/test",
        )
        self.assertEqual(self.ICP.get_param("kensei.bedrock_region"), "us-west-2")
        self.assertEqual(self.ICP.get_param("kensei.deployment_mode"), "k8s")
