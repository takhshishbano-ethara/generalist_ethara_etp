...
class TestCheckLocalStatus(KenseiTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.sandbox.write({
            "docker_compose_project": "kensei-test-proj",
    ...
    def _mock_compose_ps(self, stdout="", returncode=0, side_effect=None):
        if side_effect:
            return patch(
                "odoo.addons.kensei.models.kensei_sandbox.subprocess.run",
    ...
    def test_skip_when_sandbox_starting(self):
        from odoo.addons.kensei.models.kensei_sandbox import (
            _SANDBOX_LOCK,
            _SANDBOX_STARTING,
        )
    ...
    def test_container_exited(self):
        ...
        with self._mock_compose_ps(stdout=ps_json):
            with patch(
                "odoo.addons.kensei.models.kensei_sandbox._compose_cmd",
...
@tagged("post_install", "-at_install")
class TestWaitForHealth(KenseiTestCase):
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.time.sleep")
    @patch("odoo.addons.kensei.models.kensei_sandbox.time.monotonic")
    def test_success(self, mock_mono, mock_sleep):
        ...
        with patch(
            ...
            with patch(
                "odoo.addons.kensei.models.kensei_sandbox.urllib.request.urlopen"
            ) as mock_urlopen:
...
@tagged("post_install", "-at_install")
class TestBuildComposeEnv(KenseiTestCase):
    ...
    def _patch_dotenv(self, env_dict=None):
        return patch(
            "odoo.addons.kensei.models.kensei_sandbox._load_dotenv",
    ...
    def test_generates_litellm_key(self):
        with self._patch_dotenv({}):
            env = self.sandbox._build_compose_env("tok-123")
        self.assertTrue(env["LITELLM_MASTER_KEY"].startswith("sk-kensei-"))
...
@tagged("post_install", "-at_install")
class TestPrepareWorkdir(KenseiTestCase):
    ...
    def _base_patches(self, source_dir="/fake/sandbox"):
        ...
        self._mock_module_dir = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox._module_sandbox_dir",
            return_value=source_dir,
        ))
        self._mock_isdir = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.os.path.isdir",
            return_value=True,
        ))
        self._mock_isfile = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.os.path.isfile",
            return_value=False,
        ))
        self._mock_exists = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.os.path.exists",
            return_value=False,
        ))
        self._mock_makedirs = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.os.makedirs",
        ))
        self._mock_rmtree = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.shutil.rmtree",
        ))
        self._mock_copy2 = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.shutil.copy2",
        ))
        self._written_files = {}
        ...
        self._mock_json_dump = stack.enter_context(patch(
            "odoo.addons.kensei.models.kensei_sandbox.json.dump",
        ))
        return stack

    def test_no_sandbox_dir_raises(self):
        with patch(
            "odoo.addons.kensei.models.kensei_sandbox._load_dotenv", return_value={}
        ):
            with patch(
                "odoo.addons.kensei.models.kensei_sandbox._module_sandbox_dir",
                return_value=None,
