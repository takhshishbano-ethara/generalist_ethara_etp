# -*- coding: utf-8 -*-
"""Tests for models/kensei_sandbox_k8s.py — the K8s deployer AbstractModel."""
import json
...
_K8S_MOD = "odoo.addons.kensei.models.kensei_sandbox_k8s"
...
class TestK8sHelperFunctions(KenseiTestCase):
    """Tests for module-level helper functions."""

    def test_sandbox_labels(self):
        """_sandbox_labels returns correct label dict with task-id."""
        from odoo.addons.kensei.models.kensei_sandbox_k8s import _sandbox_labels
        labels = _sandbox_labels(self.claude_sandbox)
        self.assertEqual(labels["platform"], "kensei")
        ...
        self.assertEqual(labels["app.kubernetes.io/managed-by"], "kensei-odoo")

    def test_resource_name(self):
        """_resource_name returns 'kensei-sandbox-{id}'."""
        from odoo.addons.kensei.models.kensei_sandbox_k8s import _resource_name
        name = _resource_name(self.claude_sandbox)
        self.assertEqual(name, "kensei-sandbox-%s" % self.claude_sandbox.id)

    def test_s3_session_path(self):
        """_s3_session_path returns correct S3 URI format."""
        from odoo.addons.kensei.models.kensei_sandbox_k8s import (
            _s3_session_path, S3_BUCKET, S3_KENSEI_PREFIX,
        )
        path = _s3_session_path(self.claude_sandbox)
        expected = "s3://%s/%s/tasks/%s/sessions/" % (
            S3_BUCKET, S3_KENSEI_PREFIX, self.claude_sandbox.id,
        )
        self.assertEqual(path, expected)

    def test_build_prestop_script(self):
        """_build_prestop_script contains task_id and persona."""
        from odoo.addons.kensei.models.kensei_sandbox_k8s import _build_prestop_script
    ...
    def test_build_openclaw_config_structure(self):
        ...
        from odoo.addons.kensei.models.kensei_sandbox_k8s import _build_openclaw_config
    ...
    def test_build_openclaw_config_model_defaults(self):
        ...
        from odoo.addons.kensei.models.kensei_sandbox_k8s import _build_openclaw_config
        from odoo.addons.kensei.models.kensei_sandbox import MODEL_DEFAULTS
...
@tagged("post_install", "-at_install")
class TestK8sDeploy(KenseiTestCase):
    """Tests for KenseiSandboxK8s.deploy_sandbox — all K8s calls mocked."""

    def _get_deployer(self):
        return self.env["kensei.sandbox.k8s"]
    ...
    def test_deploy_creates_secret(self, _dotenv, mock_client, _config):
        ...
        for call in mock_core.create_namespaced_secret.call_args_list:
            self.assertEqual(call[1]["namespace"], "kensei")
...
@tagged("post_install", "-at_install")
class TestK8sDestroy(KenseiTestCase):
    """Tests for KenseiSandboxK8s.destroy_sandbox."""
...
@tagged("post_install", "-at_install")
class TestK8sStatus(KenseiTestCase):
    """Tests for KenseiSandboxK8s.get_sandbox_status."""
