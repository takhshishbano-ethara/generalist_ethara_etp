...
class TestReadJsonlLocal(KenseiTestCase):
    """Tests for KenseiSandbox._read_jsonl_local."""
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.os.path.isdir")
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.os.listdir", return_value=[])
    @patch("odoo.addons.kensei.models.kensei_sandbox.os.path.isdir", return_value=True)
    def test_read_jsonl_local_no_jsonl_files(self, _isdir, _listdir):
        """Empty sessions dir → returns []."""
        sb = self._sandbox(docker_workdir="/tmp/fake")
        self.assertEqual(sb._read_jsonl_local(), [])

    @patch("odoo.addons.kensei.models.kensei_sandbox.os.path.getmtime", return_value=1.0)
    @patch("odoo.addons.kensei.models.kensei_sandbox.os.listdir",
    ...
    def test_read_jsonl_local_uses_persona_name(self, mock_isdir, _listdir, _getmtime):
        ...
        persona_name = sb.kensei_id.persona_id.name
...
@tagged("post_install", "-at_install")
class TestReadJsonlK8s(KenseiTestCase):
    """Tests for KenseiSandbox._read_jsonl_k8s."""
    ...
    def test_read_jsonl_k8s_no_k8s(self):
        ...
        sb = self._sandbox()
        with patch(
            "odoo.addons.kensei.models.kensei_sandbox.KenseiSandbox._read_jsonl_k8s",
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.subprocess.run")
    ...
    def test_read_jsonl_k8s_namespace_from_config(self, mock_run):
        ...
        sb = self._sandbox()
        self._set_param("kensei.k8s_namespace", "custom-ns")
        mock_run.return_value = MagicMock(
            returncode=0,
...
@tagged("post_install", "-at_install")
class TestQueryLitellmSpend(KenseiTestCase):
    """Tests for KenseiSandbox._query_litellm_spend."""
    ...
    def test_query_spend_no_port(self):
        ...
        self._set_param("kensei.deployment_mode", "local")
        sb = self._sandbox(docker_litellm_port=0)
        self.assertEqual(sb._query_litellm_spend(), (0, 0))

    @patch("odoo.addons.kensei.models.kensei_sandbox._load_dotenv", return_value={})
    def test_query_spend_no_litellm_key(self, _dotenv):
        """No LITELLM_MASTER_KEY → returns (0, 0)."""
        self._set_param("kensei.deployment_mode", "local")
        sb = self._sandbox(docker_litellm_port=16001)
        self.assertEqual(sb._query_litellm_spend(), (0, 0))

    @patch("odoo.addons.kensei.models.kensei_sandbox._load_dotenv",
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.urllib.request.urlopen")
    @patch("odoo.addons.kensei.models.kensei_sandbox._load_dotenv",
           return_value={"LITELLM_MASTER_KEY": "sk-test"})
    def test_query_spend_http_error(self, _dotenv, mock_urlopen):
        ...
        mock_urlopen.side_effect = urllib.error.HTTPError(
            url="http://localhost:16001/spend/logs",
            code=500,
    ...
    def test_query_spend_k8s_mode_url(self, _dotenv, mock_urlopen):
        ...
        self._set_param("kensei.ws_router_host", "ws.example.com")
        sb = self._sandbox(docker_litellm_port=16001)
    ...
    def test_query_spend_k8s_fallback_key(self, _dotenv, mock_urlopen):
        """Falls back to sk-kensei-<token[:16]> when no master key."""
        self._set_param("kensei.deployment_mode", "k8s")
        self._set_param("kensei.ws_router_host", "ws.example.com")
        ...
        self.assertIn("sk-kensei-" + token[:16], auth_header)
...
@tagged("post_install", "-at_install")
class TestExportTrajectoryToTask(KenseiTestCase):
    """Tests for KenseiSandbox._export_trajectory_to_task."""
    ...
    @patch("odoo.addons.kensei.models.kensei_sandbox.KenseiSandbox._query_litellm_spend",
           return_value=(0, 0))
    @patch("odoo.addons.kensei.models.kensei_sandbox.KenseiSandbox._read_session_jsonl",
           return_value=[])
