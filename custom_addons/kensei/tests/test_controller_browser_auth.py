...
_BROWSER_MOD = "odoo.addons.kensei.controllers.browser_auth"


@tagged("post_install", "-at_install")
class TestBrowserApiBaseUrl(KenseiTestCase):

    def _ctrl(self):
        from odoo.addons.kensei.controllers.browser_auth import KenseiBrowserAuthController
        return KenseiBrowserAuthController
    ...
    def test_url_k8s_mode_with_ws_host(self):
        ctrl_cls = self._ctrl()
        self._set_param("kensei.ws_router_host", "ws.ethara.ai")
        with patch.object(
    ...
    def test_url_k8s_mode_no_ws_host(self):
        ...
        with patch.object(
            ...
            url = ctrl_cls._browser_api_base(self.claude_sandbox)
        svc_name = "kensei-sandbox-%s" % self.claude_sandbox.id
        expected = "http://%s.kensei.svc.cluster.local:18789/browser-api" % svc_name
        self.assertEqual(url, expected)


@tagged("post_install", "-at_install")
class TestBrowserScreenshot(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserInjectCookies(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserStatus(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserScreenshotErrorPaths(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserInjectCookiesErrorPaths(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserStatusErrorPaths(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestBrowserValidateAndApiBaseEdgeCases(KenseiTestCase):
