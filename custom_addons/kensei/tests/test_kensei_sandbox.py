...
class TestKenseiSandboxCreate(KenseiTestCase):
    ...
    def test_unique_constraint_task_model(self):
        """Cannot create two sandboxes with the same (kensei_id, model_type)."""
        with self.assertRaises(Exception):
            self.Sandbox.create({
                "kensei_id": self.task.id,
                "model_type": "claude",
            })
    ...
    def test_ordering_by_model_type(self):
        ...
        sandboxes = self.Sandbox.search([("kensei_id", "=", task2.id)])
        types = sandboxes.mapped("model_type")
        self.assertEqual(types, sorted(types))


@tagged("post_install", "-at_install")
class TestSandboxComputedFields(KenseiTestCase):
    """Tests for dashboard_url, ws_url, and _get_gateway_ws_url."""

    def _make_running(self, sandbox, port=21042, token="abc123"):
        sandbox.write({
    ...
    def test_dashboard_url_running_local(self):
        ...
        self.assertIn("localhost:21042", url)
    ...
    def test_dashboard_url_running_k8s_no_ws_host(self):
        self._set_param("kensei.deployment_mode", "k8s")
        self._set_param("kensei.ws_router_host", "")
        self._make_running(self.claude_sandbox)
        url = self.claude_sandbox.docker_dashboard_url
        self.assertIn("kensei-sandbox-", url)
        self.assertIn(".kensei.svc.cluster.local:18789", url)
    ...
    def test_ws_url_running_local(self):
        ...
        self._make_running(self.claude_sandbox)
        self.assertEqual(
            self.claude_sandbox.docker_ws_url, "ws://localhost:21042"
        )
    ...
    def test_get_gateway_ws_url_local(self):
        ...
        self.assertEqual(url, "ws://localhost:21042")
...
@tagged("post_install", "-at_install")
class TestSandboxPortAllocation(KenseiTestCase):
    ...
    def test_uses_id_mod_5000(self):
        ...
        self.assertEqual(gw, 21000 + offset)
        self.assertEqual(ll, 16000 + offset)
        self.assertEqual(db, 17432 + offset)
    ...
    def test_correct_base_ranges(self):
        ...
        self.assertEqual(GATEWAY_PORT_BASE, 21000)
        self.assertEqual(LITELLM_PORT_BASE, 16000)
        self.assertEqual(DB_PORT_BASE, 17432)


@tagged("post_install", "-at_install")
class TestSandboxJSONL(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestSandboxTrajectoryBuilding(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestSandboxAutoProcess(KenseiTestCase):
    ...
    def test_auto_process_get_ws_info_running(self):
        ...
        self.assertEqual(result["ws_url"], "ws://localhost:21100")
