...
_QC_MOD = "odoo.addons.kensei.controllers.llm_assisst_qc"


@tagged("post_install", "-at_install")
class TestCallBedrockConverse(KenseiTestCase):

    @patch(_QC_MOD + ".httpx")
    def test_call_bedrock_success(self, mock_httpx):
        from odoo.addons.kensei.controllers.llm_assisst_qc import _call_bedrock_converse
...
@tagged("post_install", "-at_install")
class TestParseHelpers(KenseiTestCase):

    def test_parse_json_response_code_block(self):
        from odoo.addons.kensei.controllers.llm_assisst_qc import _parse_json_response
    ...
    def test_parse_qc_verdict_valid(self):
        from odoo.addons.kensei.controllers.llm_assisst_qc import _parse_qc_verdict
    ...
    def test_is_degenerate_normal_text(self):
        from odoo.addons.kensei.controllers.llm_assisst_qc import _is_degenerate
...
@tagged("post_install", "-at_install")
class TestQCEndpoint(KenseiTestCase):
    ...
    def test_qc_success(self, _sys, _dotenv, mock_bedrock):
        ...
        self._set_param("kensei.bedrock_region", "us-east-1")

        ctrl = self.env["ir.http"]
        from odoo.addons.kensei.controllers.llm_assisst_qc import (
    ...
    def test_qc_missing_prompt(self):
        from odoo.addons.kensei.controllers.llm_assisst_qc import LlmAssistQc
...
@tagged("post_install", "-at_install")
class TestTrajectoryQC(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestQCToggle(KenseiTestCase):

    def test_qc_prompt_skipped_when_disabled(self):
        from odoo.addons.kensei.controllers.llm_assisst_qc import LlmAssistQc

        self._set_param("kensei.disable_prompt_qc", "True")
        ctrl = LlmAssistQc()
        with patch("odoo.http.request") as mock_req:
            mock_req.env = self.env
            result = ctrl.qc_prompt(prompt="test")
    ...
    def test_qc_prompt_runs_when_not_disabled(self, _sys, _dotenv, mock_bedrock):
        ...
        self._set_param("kensei.disable_prompt_qc", "False")
        self._set_param("kensei.bedrock_inference_arn", "arn:test")
        self._set_param("kensei.bedrock_region", "us-east-1")
        ctrl = LlmAssistQc()
        with patch("odoo.http.request") as mock_req:
    ...
    def test_trajectory_qc_skipped_when_disabled(self):
        ...
        self._set_param("kensei.disable_trajectory_qc", "True")
        ctrl = LlmAssistQc()
        with patch("odoo.http.request") as mock_req:
            mock_req.env = self.env
            result = ctrl.trajectory_qc(
    ...
    def test_trajectory_qc_runs_when_not_disabled(self):
        ...
        traj = json.dumps([self._make_session_entry()])
        self.task.write({"claude_trajectory": traj})
        self._set_param("kensei.disable_trajectory_qc", "False")
        ctrl = LlmAssistQc()
        with patch("odoo.http.request") as mock_req:
            mock_req.env = self.env
            mock_req.env.cr = self.env.cr
...
@tagged("post_install", "-at_install")
class TestGenerateTaskDescription(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestAbortTrajectoryAction(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestLegacyQCEndpoint(KenseiTestCase):
    ...
    def test_legacy_qc_missing_inference_arn(self, _dotenv):
        ...
        self._set_param("kensei.bedrock_inference_arn", "")
        ctrl = LlmAssistQc()
        with patch("odoo.http.request") as mock_req:
            mock_req.env = self.env
            mock_req.get_json_data.return_value = {"prompt": "test"}
