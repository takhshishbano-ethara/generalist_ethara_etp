# -*- coding: utf-8 -*-
"""Cross-cutting integration tests for the Kensei module.
...
from ..models.kensei_sandbox import TRAJECTORY_FIELD_MAP
...
class TestFullLifecycle(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestCascadingDeletes(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestTokenAggregation(KenseiTestCase):
...
@tagged("post_install", "-at_install")
class TestPersonaPropagation(KenseiTestCase):
    """Verify persona data flows through task → sandbox chain."""

    def test_persona_fields_available_for_sandbox(self):
        """Sandbox can reach persona fields through kensei_id.persona_id."""
        task = self._create_task(seed_prompt="Persona propagation test")
        sandbox = task.claude_sandbox_id

        persona = sandbox.kensei_id.persona_id
        self.assertTrue(persona, "Sandbox should reach persona via kensei_id")
