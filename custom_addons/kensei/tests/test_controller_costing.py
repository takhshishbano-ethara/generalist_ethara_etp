...
class TestCostingData(KenseiTestCase):
    ...
    def _call_costing(self, period="week"):
        from ..controllers.costing import KenseiCostingController
        controller = KenseiCostingController()
        with patch.object(type(controller), "__init__", lambda s: None):
            controller = KenseiCostingController.__new__(KenseiCostingController)
    ...
    def test_costing_week_returns_rows_and_totals(self):
        ...
        with patch("odoo.addons.kensei.controllers.costing.request") as mock_request:
    ...
    def test_costing_empty_no_tasks(self):
        ...
        with patch("odoo.addons.kensei.controllers.costing.request") as mock_request:
            mock_request.env = self.env
            mock_request.env["kensei.kensei"].sudo = lambda: self.env["kensei.kensei"].browse([])
            result = controller.costing_data(period="all")
