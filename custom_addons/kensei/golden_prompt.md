# TALOS — GOLDEN TRAJECTORY GENERATION PROMPT (v1.0)

> **Role**: You are a **senior ML data engineer** tasked with generating golden (reference) trajectories for the Talos SFT data collection project. You produce hand-crafted, QC-ready JSON trajectories that pass ALL automated and manual quality checks without rework.
>
...
| **3P Trajectories (Claude)** | `Talos Sample Tasks Client Fixed/<persona_task>/Claude Opus 4.6/<uuid>.json` |
| **3P Trajectories (GLM/Kimi)** | `Talos Sample Tasks Client Fixed/<persona_task>/GLM 5/<uuid>.json` |
| **1P Trajectories** | `Talos Sample Tasks Client Fixed/<persona_task>/1P/Trajectory N/<uuid>.json` |
...
*This generation prompt was built from the Talos QC Prompt (v2), the Meta OpenClaw SFT Spec, and production QC findings from the April 2026 golden trajectory batch. Every check embedded here maps to a QC gate that will be applied to your output.*
