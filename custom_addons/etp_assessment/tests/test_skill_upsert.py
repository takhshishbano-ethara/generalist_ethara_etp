...
from odoo.addons.etp_assessment.services import vertex
...
class _Base(TransactionCase):
    def setUp(self):
        ...
        self.Prompt = self.env["etp.assessment.prompt"]
        self.Resource = self.env["etp.assessment.prompt.resource"]
        self.PromptSkill = self.env["etp.assessment.prompt.skill"]
        self.PromptQuestion = self.env["etp.assessment.prompt.question"]
...
class TestVertexAuthValidation(TransactionCase):
    ...
    def test_no_creds_raises(self):
        ...
        for k in [
            "etp_assessment.vertex_api_key",
            "etp_assessment.vertex_access_token",
