...
# normalised status spellings -> stored value (see project.tracker.team.member)
_STATUS_MAP = {
    'active': 'active',
...
class Kensei2TrackerTeamImport(models.TransientModel):
    """Bulk-add existing Odoo users to the Project Tracker team roster.
    ...
    """
    _name = 'project.tracker.team.import'
    _description = 'Project Tracker — Team Bulk Import'
    ...
    def action_import(self):
        ...
        self.ensure_one()
        entries = self._classify()
        Member = self.env['project.tracker.team.member'].with_context(
            project_tracker_skip_toast=True)
