...
class Kensei2TrackerStageHandoff(models.TransientModel):
    ...
    project.tracker.allocation.stage_no / parent_id). This wizard creates the next
    ...
    """
    _name = 'project.tracker.stage.handoff'
    _description = 'Project Tracker — Hand off to Next Stage'

    allocation_id = fields.Many2one(
        'project.tracker.allocation', string='Completed Stage',
    ...
    tasker_member_id = fields.Many2one(
        'project.tracker.team.member', string='Next Tasker', required=True,
    ...
    def _default_tasker(self):
        alloc = self.env['project.tracker.allocation'].browse(
    ...
    def action_confirm(self):
        ...
        if not (self.env.su
                or self.env.user.has_group('project_tracker.group_project_tracker_ql')
        ...
        nxt = self.env['project.tracker.allocation'].create({
