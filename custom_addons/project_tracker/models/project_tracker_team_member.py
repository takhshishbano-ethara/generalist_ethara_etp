...
class Kensei2TrackerTeamMember(models.Model):
    """A curated Project Tracker team roster.
    ...
    """
    _name = 'project.tracker.team.member'
    _description = 'Project Tracker Team Member'
    ...
    assigned_pl_id = fields.Many2one(
        ...
        help='Project Lead this member reports to for Project Tracker work.')
    ...
    def _compute_role(self):
        ...
        ladder = [
            ('admin', {gid('base.group_system'), gid('project_tracker.group_project_tracker_admin'),
                       gid('etp_user_roles.group_it_admin'),
                       gid('etp_user_roles.group_hr_admin'), gid('etp_user_roles.group_cto')}),
            ('pl', {gid('etp_user_roles.group_project_lead'), gid('project_tracker.group_project_tracker_pl')}),
            ('ql', {gid('etp_user_roles.group_quality_lead'), gid('project_tracker.group_project_tracker_ql')}),
            ('tasker', {gid('etp_user_roles.group_tasker'), gid('project_tracker.group_project_tracker_tasker')}),
        ]
    ...
    def action_view_performance(self):
        ...
        return {
            ...
            'tag': 'project_tracker_tasker_dashboard',
    ...
    #  has its own result summary (pass context ``project_tracker_skip_toast``).
    # ------------------------------------------------------------------ #
    @api.model_create_multi
    def create(self, vals_list):
        records = super().create(vals_list)
        if not self.env.context.get('project_tracker_skip_toast'):
    ...
    def _notify_added(self):
        ...
        for rec in self:
            ...
            self.env['bus.bus']._sendone(
                partner,
                'project_tracker/team_member_added',
                {
    ...
    def _notify_updated(self):
        ...
        self.env['bus.bus']._sendone(
            partner,
            'project_tracker/team_member_updated',
            {
