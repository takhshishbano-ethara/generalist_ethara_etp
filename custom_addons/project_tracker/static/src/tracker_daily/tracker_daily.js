...
import { todayIso, daysAgoIso } from "@project_tracker/tracker_common";

export class Kensei2TrackerDaily extends Component {
    static template = "project_tracker.TrackerDaily";
    ...
    async _loadFilters() {
        try {
            this.state.opts = await rpc("/project_tracker/daily/filters", {});
        } catch (e) {
    ...
    async _load() {
        ...
        try {
            const res = await rpc("/project_tracker/daily/data", this._params());
            if (isStale()) {
                return;
            }
    ...
    async onExport() {
        try {
            const res = await rpc("/project_tracker/daily/data", this._params({ export: true, page_size: 100000 }));
...
registry.category("actions").add("project_tracker_daily", Kensei2TrackerDaily);
