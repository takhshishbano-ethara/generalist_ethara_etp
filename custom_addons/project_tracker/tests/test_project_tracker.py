# -*- coding: utf-8 -*-
"""Project Tracker — allocation pipeline, access control and stage hand-off.
...
from odoo.addons.project_tracker.controllers import tracker
...
class Kensei2TrackerCommon(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.Alloc = cls.env["project.tracker.allocation"]
        cls.Member = cls.env["project.tracker.team.member"]
        ...
        cls.g_tasker = cls.env.ref("project_tracker.group_project_tracker_tasker")
        cls.g_ql = cls.env.ref("project_tracker.group_project_tracker_ql")
        cls.g_pl = cls.env.ref("project_tracker.group_project_tracker_pl")
        cls.g_admin = cls.env.ref("project_tracker.group_project_tracker_admin")
    ...
    def _make_member(cls, user, pl=None, ql=None):
        ...
        return cls.Member.with_context(project_tracker_skip_toast=True).create(vals)
...
class TestTrackerAccessControl(Kensei2TrackerCommon):
    ...
    def test_etp_only_user_cannot_see_other_taskers(self):
        ...
        self.assertFalse(
            self.user_etp.has_group("project_tracker.group_project_tracker_tasker"),
    ...
    def test_tasker_cannot_unlock_via_context(self):
        """SEC-4: `project_tracker_reopen` was honoured on the context key alone.
        ...
        with self.assertRaises(UserError):
            alloc.with_user(self.user_tasker).with_context(
                project_tracker_reopen=True).write({"rubric_score": 99.0})
...
class TestTrackerVisibilityTiers(Kensei2TrackerCommon):
    ...
    def test_admin_sees_every_team(self):
        """Guards a trap: group_project_tracker_admin IMPLIES group_project_tracker_pl, so without a
...
class TestTrackerTaskUuid(Kensei2TrackerCommon):
    ...
    def test_stages_of_the_same_task_share_the_uuid(self):
        ...
        self._complete_stage1(alloc)
        self.env["project.tracker.stage.handoff"].with_user(self.user_ql).create({
...
class TestTrackerStageHandoff(Kensei2TrackerCommon):
    ...
    def test_handoff_creates_next_stage_clean(self):
        ...
        wizard = self.env["project.tracker.stage.handoff"].with_user(
            self.user_ql).create({
                "allocation_id": alloc.id,
                "tasker_member_id": self.member_other.id,
            })
    ...
    def test_every_locked_input_is_force_save(self):
        ...
        """
        import re
        from odoo.addons.project_tracker.models.project_tracker_allocation import (
    ...
    def test_cannot_hand_off_twice(self):
        alloc = self._ready_stage1()
        Wiz = self.env["project.tracker.stage.handoff"].with_user(self.user_ql)
        Wiz.create({
            "allocation_id": alloc.id,
            "tasker_member_id": self.member_other.id,
        }).action_confirm()
    ...
    def test_tasker_cannot_hand_off(self):
        alloc = self._ready_stage1()
        with self.assertRaises(AccessError):
            self.env["project.tracker.stage.handoff"].with_user(
...
class TestTrackerDashboardDataAccess(Kensei2TrackerCommon):
    ...
    def test_progress_rows_loads_for_a_pl(self):
        ...
        from odoo.addons.project_tracker.controllers.tracker import (
    ...
    def test_daily_roster_loads_for_a_pl(self):
        ...
        from odoo.addons.project_tracker.controllers.tracker import _label
        Alloc = self.Alloc.with_user(self.user_pl)
...
class TestTrackerPersona(Kensei2TrackerCommon):
    ...
    def test_action_view_allocations_builds_an_action(self):
        ...
        action = persona.action_view_pt_allocations()
        self.assertEqual(action["res_model"], "project.tracker.allocation")
    ...
    def test_assignment_status_tracks_its_allocations(self):
        ...
        self.assertEqual(persona.pt_assignment_status, "unassigned")
        self.assertEqual(persona.pt_allocation_count, 0)

        alloc = self._make_alloc(suffix="persona-status", persona_id=persona.id)
        persona.invalidate_recordset()
        self.assertEqual(persona.pt_assignment_status, "assigned")
        self.assertEqual(persona.pt_allocation_count, 1)
        self.assertEqual(persona.pt_current_allocation_id, alloc)
        ...
        self.assertEqual(persona.pt_allocation_count, 0)
        self.assertFalse(persona.pt_current_allocation_id)
...
class TestTrackerListStats(Kensei2TrackerCommon):
    ...
    def _cards(self, model, domain=None, user=None):
        from odoo.addons.project_tracker.controllers.tracker import _LIST_STATS
    ...
    def test_allocation_cards(self):
        cards = self._cards("project.tracker.allocation")
    ...
    def test_team_cards(self):
        cards = self._cards("project.tracker.team.member")
    ...
    def test_an_average_carries_no_share(self):
        ...
        that does not exist."""
        cards = {c["key"]: c
                 for c in self._cards_raw("project.tracker.allocation")}
    ...
    def test_cards_follow_the_search_domain(self):
        ...
        everything = self._cards("project.tracker.allocation")
        filtered = self._cards(
            "project.tracker.allocation", [("task_id", "=", self.a1.task_id)])
    ...
    def test_cards_are_scoped_by_the_record_rules(self):
        ...
        as_pl = self._cards("project.tracker.allocation", user=self.user_pl)
        as_admin = self._cards("project.tracker.allocation", user=self.user_admin)
...
class TestTrackerListStatsEndpoint(HttpCase):
    ...
    def test_allocation_stats_load(self):
        ...
        resp = self.url_open(
            "/project_tracker/list_stats",
            data=json.dumps({"jsonrpc": "2.0", "method": "call",
                             "params": {"model": "project.tracker.allocation",
                                        "domain": []}}),
            headers={"Content-Type": "application/json"},
        )
        body = resp.json()
...
class TestTrackerDailyEndpoint(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.pl_user = cls.env["res.users"].with_context(
            etp_importing=True).create({
                ...
                "group_ids": [(6, 0, [
                    ...
                    cls.env.ref("project_tracker.group_project_tracker_pl").id,
                ])],
            })
        tasker = cls.env["res.users"].with_context(etp_importing=True).create({
            ...
            "group_ids": [(6, 0, [
                cls.env.ref("base.group_user").id,
                cls.env.ref("project_tracker.group_project_tracker_tasker").id,
            ])],
        })
        member = cls.env["project.tracker.team.member"].with_context(
            project_tracker_skip_toast=True).create({
                "user_id": tasker.id,
                "status": "active",
                "assigned_pl_id": cls.pl_user.id,
            })
        persona = cls.env["kensei2.persona"].create({"name": "daily-endpoint-persona"})
        cls.alloc = cls.env["project.tracker.allocation"].create({
    ...
    def test_daily_data_loads_for_a_pl(self):
        ...
        body = self._call("/project_tracker/daily/data")
        self.assertNotIn(
            "error", body,
    ...
    def test_daily_filters_load_for_a_pl(self):
        self.authenticate("daily.pl@ethara.ai", "daily.pl.pw")
        body = self._call("/project_tracker/daily/filters")
        self.assertNotIn(
            "error", body,
    ...
    def test_daily_stage_filter_partitions_the_completions(self):
        ...
        def _completed(stage):
            ...
            body = self._call("/project_tracker/daily/data", params)
    ...
    def test_daily_stage_filter_is_sent_on_export_too(self):
        ...
        js = tools.file_open(
            "project_tracker/static/src/tracker_daily/tracker_daily.js").read()
    ...
    def test_dashboard_loads_for_a_pl(self):
        ...
        self.authenticate("daily.pl@ethara.ai", "daily.pl.pw")
        body = self._call("/project_tracker/dashboard")
        self.assertNotIn(
            "error", body,
...
class TestTrackerBulkAllocation(Kensei2TrackerCommon):
    ...
    def test_unassigned_domain_uses_the_stored_index(self):
        ...
        domain = self.env["project.tracker.bulk.allocation"]._unassigned_persona_domain()
        self.assertEqual(domain, [("pt_assignment_status", "=", "unassigned")])

        fresh = self.Persona.create({"name": "bulk-unassigned-probe"})
        self.assertEqual(fresh.pt_assignment_status, "unassigned")
