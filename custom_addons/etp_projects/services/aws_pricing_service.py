...
def _client(env):
    ...
    if not access_key or not secret_key:
        raise UserError(_(
            ...
            "Access Key in Settings \u2192 ETP Projects \u2192 AWS Credentials, "
            "then click Save."
        ))
    region = (
        creds.region_name
        or env["ir.config_parameter"].sudo().get_param("aws_pricing.region", "us-east-1")
    )
    return boto3.client(
        "pricing",
        region_name=region,
...
def action_sync_all(env, triggered_by="cron", user_id=None):
    Log = env["etp.aws.pricing.sync.log"].sudo()
    Infra = env["etp.infra.type"].sudo()
    Item = env["etp.aws.pricing.item"].sudo()
    ...
    try:
        ...
        for svc in services:
            ...
            try:
                ...
                _logger.warning("AWS Pricing sync: upsert service %s failed: %s", code, e)
...
def fetch_pricing_cached(env, service_code, item_name, region_label, force_refresh=False):
    Infra = env["etp.infra.type"].sudo()
    Item = env["etp.aws.pricing.item"].sudo()
    Sku = env["etp.aws.pricing.sku"].sudo()
    ...
    if not force_refresh:
        ...
        if cached:
            return _skus_to_pricing_dict(cached, service_code, item_name, region_label, cached=True)
    ...
    ttl_hours = int(env["ir.config_parameter"].sudo().get_param("aws_pricing.cache_ttl_hours", "24"))
    ...
    return _skus_to_pricing_dict(sku_records, service_code, item_name, region_label, cached=False)
