...
def _batch_scope_domain(gen_scope_domain):
    """Translate a crowley.generation user-scope domain (on `user_id`) into
    the equivalent crowley.batch.delivery domain (on the m2m `job_ids.user_id`).
    An empty generation scope (the "full" role) yields an empty batch scope.
    """
    out = []
...
def _build_batch_domain(params):
    ...
    if search:
        domain += [
            "|", "|",
            ("name", "ilike", search),
            ("job_ids.video_s3_url", "ilike", search),
...
def _serialize_trajectory(job, category_labels):
    category_label = category_labels.get(job.category, "") if job.category else ""
    model = job.model_name or ""
    if category_label and model:
        repo_model = f"{category_label} × {model}"
    else:
        repo_model = category_label or model
    return {
        "id": job.name or "",
        "repo_model": repo_model,
        "cost": _money(job.total_cost_usd or job.cost_usd),
        "status": job.review_state or job.state or "",
    }
...
def _added_by_options(env, batch_scope):
    owners = env["crowley.batch.delivery"].sudo().search(batch_scope).mapped(
        "user_id"
    )
...
def _expanded():
    return {
        ...
        "columns": [
            ...
            {"key": "repo_model", "label": "Repo × Model", "type": "string",
             "flex": 1},
...
def _batch_payload(env, batch_scope, params):
    Batch = env["crowley.batch.delivery"].sudo()
...
class CrowleyBatchDashboardController(http.Controller):

    @http.route(
        "/api/v1/crowley_ext/batch_dashboard",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def crowley_ext_batch_dashboard(self, **kwargs):
        env = request.env
        role_tag = _user_role_tag(env)
        if role_tag is None:
            return return_Response(
                message="You are not allowed to access Crowley batch data.",
                status=403,
            )

        params = request.params or {}
        gen_scope = _user_scope_domain(env, role_tag)
