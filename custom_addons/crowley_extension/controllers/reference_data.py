...
class CrowleyReferenceDataController(http.Controller):

    @http.route(
        "/api/v1/crowley_ext/categories",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def crowley_ext_list_categories(self, **kwargs):
        env = request.env
    ...
    @validate_token
    def crowley_ext_list_taskers(self, **kwargs):
        env = request.env
        if _user_role_tag(env) is None:
            return return_Response(
                message="You are not allowed to access Crowley data.",
        ...
        tag, gen_domain, projects = _scope(env)
        if tag == "tasker":
