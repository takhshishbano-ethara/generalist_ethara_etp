...
class CrowleyBatchDelivery(models.Model):
    _name = "crowley.batch.delivery"
    _description = "Crowley Batch Delivery"
    ...
    job_ids = fields.Many2many(
        "crowley.generation",
        relation="crowley_batch_delivery_job_rel",
        column1="batch_id",
        column2="job_id",
        string="Generations",
    )
    job_count = fields.Integer(
        string="Generation Count", compute="_compute_job_count", store=True
    )
    column_ids = fields.Many2many(
        "ir.model.fields",
        relation="crowley_batch_delivery_column_rel",
        column1="batch_id",
        column2="field_id",
        string="CSV Columns",
        domain=[("model", "=", "crowley.generation")],
    ...
    def action_export_csv(self):
        self.ensure_one()
        if not self.job_ids:
            raise UserError(_("Nothing to export — batch has no generations."))
