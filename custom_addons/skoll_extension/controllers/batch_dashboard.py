...
from .task_view_dashboard import _user_scope_domain
...
# Per-task token columns on skoll.skoll, summed into a single "tokens" cost.
TOKEN_FIELDS = (
...
def _batch_scope_domain(task_scope_domain):
    """Translate a skoll.skoll user-scope domain (on the task record) into the
    equivalent skoll.batch.delivery domain (on the m2m `job_ids`).
    An empty task scope (full/ql roles) yields an empty batch scope.
    """
    out = []
...
def _added_by_options(env, batch_scope):
    owners = env["skoll.batch.delivery"].sudo().search(batch_scope).mapped(
        "user_id"
    )
...
def _batch_payload(env, batch_scope, params):
    Batch = env["skoll.batch.delivery"].sudo()
...
class SkollBatchDashboardController(http.Controller):

    @http.route(
        "/api/v1/skoll_ext/batch_dashboard",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def skoll_ext_batch_dashboard(self, **kwargs):
        env = request.env
        role_tag = _user_role_tag(env)
        if role_tag is None:
            return return_Response(
                message="You are not allowed to access Skoll batch data.",
                status=403,
            )
