...
class ResConfigSettings(models.TransientModel):
    ...
    video_editor_s3_youtube_ec2_enabled = fields.Boolean(
        string="Run YouTube ingest on EC2 service",
        config_parameter="video_editor_s3.youtube_ec2_enabled",
        help=(
            "When enabled, YouTube ingest jobs are dispatched to the external "
            "EC2 FastAPI service instead of running yt-dlp inside the Odoo "
            "worker. EC2 performs the download and S3 upload, then posts the "
            "result back to /video_editor_s3/callback/youtube_ec2."
        ),
    )
    video_editor_s3_youtube_ec2_url = fields.Char(
        string="YouTube EC2 Service URL",
        config_parameter="video_editor_s3.youtube_ec2_url",
        help=(
            "Public base URL of the EC2 FastAPI YouTube extractor, "
            "e.g. http://1.2.3.4:8000. Odoo POSTs jobs to {url}/download."
        ),
    )
    video_editor_s3_youtube_ec2_callback_base_url = fields.Char(
        string="YouTube EC2 Callback Base URL",
        config_parameter="video_editor_s3.youtube_ec2_callback_base_url",
        help=(
            "Public HTTPS URL of this Odoo instance that the EC2 service can "
            "reach, e.g. https://odoo.example.com. EC2 appends "
            "/video_editor_s3/callback/youtube_ec2 to it."
        ),
    )
    ...
    def set_values(self):
        ...
        self._validate_youtube_ec2_urls()
        ICP = self.env["ir.config_parameter"].sudo()
    ...
    def _validate_youtube_ec2_urls(self):
        enabled = bool(self.video_editor_s3_youtube_ec2_enabled)
        ec2_url = (self.video_editor_s3_youtube_ec2_url or "").strip()
        cb_url = (self.video_editor_s3_youtube_ec2_callback_base_url or "").strip()
        if enabled and (not ec2_url or not cb_url):
            raise ValidationError(_(
                "YouTube EC2 ingest is enabled but the EC2 Service URL "
                "or the Callback Base URL is empty. Set both, or disable "
                "the toggle."
            ))
        for label, url in (
            ("YouTube EC2 Service URL", ec2_url),
            ("YouTube EC2 Callback Base URL", cb_url),
        ):
            if url and not url.startswith(("http://", "https://")):
                raise ValidationError(_(
                    "%(label)s must start with http:// or https:// (got %(url)s)."
                ) % {"label": label, "url": url[:120]})
