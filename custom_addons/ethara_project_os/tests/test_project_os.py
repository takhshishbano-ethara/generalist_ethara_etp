...
class ProjectOsCommon(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_pm, cls.emp_pm = cls._person('Gita PM', 'pm@test.epo')
        cls.user_pl, cls.emp_pl = cls._person('Piyush PL', 'pl@test.epo')
        cls.user_tasker, cls.emp_tasker = cls._person('Mira Tasker', 'tasker@test.epo')
        cls.user_tasker2, cls.emp_tasker2 = cls._person('Noor Tasker', 'tasker2@test.epo')
        ...
        Role.create({'employee_id': cls.emp_tasker.id, 'role': 'tasker'})
        Role.create({'employee_id': cls.emp_tasker2.id, 'role': 'tasker'})
...
class TestRoles(ProjectOsCommon):
    ...
    def test_grant_drives_the_login_level(self):
        ...
        self.assertEqual(self.emp_pm.epo_role, 'pm')
        self.assertTrue(self.user_pm.has_group('ethara_project_os.group_epo_pm'))
        self.assertEqual(self.user_pm._epo_role(), 'pm')

    def test_ladder_implies_lower_roles(self):
        """A PM can do everything a Pod Lead can, without a second grant."""
        self.assertTrue(self.user_pm.has_group('ethara_project_os.group_epo_pod_lead'))
        self.assertTrue(self.user_pm.has_group('ethara_project_os.group_epo_tasker'))

    def test_revoking_is_a_date_not_a_delete(self):
        """Access ends, the record of having had it does not."""
        grant = self.emp_tasker.epo_role_assignment_ids[0]
        grant.action_revoke()
    ...
    def test_same_role_twice_over_overlapping_dates(self):
        ...
        with self.assertRaises(psycopg2.IntegrityError), self.cr.savepoint():
            self.env['epo.role.assignment'].create({
                'employee_id': self.emp_tasker.id, 'role': 'tasker'})

    def test_scope_by_role(self):
        """A Tasker sees themselves; a lead sees their pod; a PM sees everyone."""
        self.assertEqual(self.user_tasker._epo_scope_employee_ids(), self.emp_tasker.ids)
        self.assertIn(self.emp_tasker.id, self.user_pl._epo_scope_employee_ids())
        self.assertIn(self.emp_tasker.id, self.user_pm._epo_scope_employee_ids())
...
class TestKickoffGate(ProjectOsCommon):
    ...
    def test_knowledge_link_must_be_http(self):
        """A javascript: URL rendered into a PM's browser is an escalation."""
    ...
    def test_archiving_with_people_still_on_it(self):
        ...
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker.id})
        with self.assertRaises(UserError):
...
class TestFormEngine(ProjectOsCommon):
    ...
    def test_choices_can_be_written_one_per_line(self):
        """The backend edits choices as lines; the API still reads JSON."""
        project = self.env['project.project'].create({'is_project_os': True, 'name': 'Lines'})
        template = self.env['epo.form.template'].create({
            'project_id': project.id, 'name': 'Draft'})
        field = self.env['epo.form.field'].create({
            'template_id': template.id, 'name': 'Sentiment', 'field_type': 'dropdown',
            'options_text': 'Positive\nNeutral\n\n  Negative  \n'})
        # Blank lines dropped, each line trimmed — a stray newline must not become a
        # choice nobody can read.
        self.assertEqual(json.loads(field.options_json),
                         ['Positive', 'Neutral', 'Negative'])
        field.grid_rows_text = 'Response A\nResponse B'
        self.assertEqual(json.loads(field.grid_rows_json), ['Response A', 'Response B'])
        # And the mirror reads back from whatever the API wrote.
        field.options_json = json.dumps(['Good', 'Bad'])
        field.invalidate_recordset()
        self.assertEqual(field.options_text, 'Good\nBad')

    def test_build_form_reuses_the_open_draft(self):
        """Clicking Build twice must not make a second draft — the unique index on
        (project, form_type, version) would reject it and the PM would lose the tab."""
        project = self.env['project.project'].create({'is_project_os': True, 'name': 'Build'})
        first = project.with_context(epo_form_type='stagelist').action_build_form()
        second = project.with_context(epo_form_type='stagelist').action_build_form()
        self.assertEqual(first['res_id'], second['res_id'])
        template = self.env['epo.form.template'].browse(first['res_id'])
        self.assertEqual(template.project_id, project)
        self.assertEqual(template.form_type, 'stagelist')
        # A form with no section has nowhere to hang its first field.
        self.assertTrue(template.section_ids)
        # The feedback form is a different record, not the same one retyped.
        feedback = project.with_context(epo_form_type='feedback').action_build_form()
        self.assertNotEqual(feedback['res_id'], first['res_id'])
...
class TestSubmissionGuard(ProjectOsCommon):
    ...
    def setUp(self):
        ...
        self.allocation = self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id})

    def test_cannot_submit_before_onboarding(self):
        with self.assertRaises(ValidationError):
            self.env['epo.form.entry'].submit_answers(
                self.template.id, self.emp_tasker.id,
    ...
    def test_cannot_submit_without_being_allocated(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['epo.form.entry'].submit_answers(
                self.template.id, self.emp_tasker2.id,
                [{'field_id': self.field.id, 'text': 'T-1'}])

    def test_required_field_blank(self):
        self._cleared(self.project, self.emp_tasker)
        with self.assertRaises(UserError):
            self.env['epo.form.entry'].submit_answers(
                self.template.id, self.emp_tasker.id, [])
    ...
    def test_submitted_entry_is_evidence(self):
        ...
        entry = self.env['epo.form.entry'].submit_answers(
            ...
            [{'field_id': self.field.id, 'text': 'T-1'}])
        with self.assertRaises(UserError):
            entry.write({'employee_id': self.emp_tasker2.id})
        with self.assertRaises(UserError):
            entry.unlink()
    ...
    def test_choice_answer_must_be_a_declared_choice(self):
        ...
        self.env['epo.document'].create({
            ...
            'name': 'SOP', 'doc_type': 'link', 'url': 'https://docs.test/sop'})
        template.action_publish()
        # emp_tasker is already on this class's project at 100%; use a second person so the
        # capacity guard is not what this test ends up exercising.
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker2.id})
        self._cleared(project, self.emp_tasker2)
        entry = self.env['epo.form.entry'].create({
            'template_id': template.id, 'employee_id': self.emp_tasker2.id})
        with self.assertRaises(ValidationError):
...
class TestAllocationAndPhases(ProjectOsCommon):
    ...
    def test_capacity_over_100_percent(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['epo.allocation'].create({
                'project_id': other.id, 'employee_id': self.emp_tasker.id,
                'allocation_pct': 60})
    ...
    def test_cannot_allocate_to_a_project_in_setup(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['epo.allocation'].create({
                'project_id': draft.id, 'employee_id': self.emp_tasker.id})
    ...
    def test_roster_accumulates_time_per_phase(self):
        ...
        Roster.upsert(self.emp_tasker.id, self.today - timedelta(days=2),
                      {'tasking_status': 'training', 'project_id': self.project.id})
        Roster.upsert(self.emp_tasker.id, self.today,
    ...
    def test_backfilling_a_past_day_does_not_rewrite_today(self):
        ...
        self.env['epo.roster.day'].upsert(
            self.emp_tasker.id, self.today - timedelta(days=3),
            {'tasking_status': 'training', 'project_id': self.project.id})
        allocation.invalidate_recordset()
        self.assertEqual(allocation.current_phase, 'tasking')
        self.assertGreaterEqual(allocation.days_training, 1)
...
class TestRoster(ProjectOsCommon):
    ...
    def test_one_row_per_person_per_day(self):
        Roster = self.env['epo.roster.day']
        first = Roster.upsert(self.emp_tasker.id, self.today, {'tasking_status': 'onboarding'})
        second = Roster.upsert(self.emp_tasker.id, self.today, {'tasking_status': 'training'})
        self.assertEqual(first, second)
    ...
    def test_no_rostering_onto_an_unallocated_project(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['epo.roster.day'].upsert(
                self.emp_tasker2.id, self.today,
                {'tasking_status': 'tasking', 'project_id': self.project.id})

    def test_locked_day_refuses_edits(self):
        row = self.env['epo.roster.day'].upsert(
            self.emp_tasker.id, self.today, {'tasking_status': 'onboarding'})
        row.locked = True
        with self.assertRaises(UserError):
...
class TestHistory(ProjectOsCommon):
    ...
    def test_one_stream_two_readings(self):
        ...
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker.id})
        self._cleared(project, self.emp_tasker)
        self.env['epo.form.entry'].submit_answers(
            template.id, self.emp_tasker.id, [{'field_id': field.id, 'text': 'T-1'}])

        Timeline = self.env['epo.timeline.event']
        by_person = Timeline.search([('employee_id', '=', self.emp_tasker.id)])
...
class TestAssessment(ProjectOsCommon):
    ...
    def test_mandatory_assessment_blocks_the_gate(self):
        ...
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker.id})
        onboarding = self._cleared(project, self.emp_tasker)
    ...
    def test_a_stage_with_no_content_auto_passes(self):
        """The gate must never block on a stage the PM never set up."""
        project, _t, _f = self._live_project('No Extras')
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker.id})
        onboarding = self.env['epo.onboarding']._get(self.emp_tasker.id, project.id)
        onboarding.action_mark_sop()
    ...
    def test_waiver_needs_a_reason_and_is_audited(self):
        ...
        self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker.id})
        onboarding = self.env['epo.onboarding']._get(self.emp_tasker.id, project.id)
        as_gpm = onboarding.with_user(self.user_pm)
        as_pm = onboarding.with_user(self.user_tasker)
        with self.assertRaises(UserError):
            # No reason given.
            as_gpm.action_waive()
        with self.assertRaises(UserError):
            # A Tasker cannot clear their own gate.
...
class TestProjectCode(ProjectOsCommon):
    ...
    def test_code_is_generated_and_unique(self):
        ...
        self.assertTrue(first.code.startswith('EPR/'), first.code)
...
class TestFolders(ProjectOsCommon):
    ...
    def test_every_project_gets_the_same_cabinet(self):
        """A PM must never face an empty screen, and a Tasker must always know
        where to look."""
    ...
    def test_s3_prefix_mirrors_the_visible_path(self):
        """An operator browsing the bucket should see the same shape as a PM browsing
        the UI."""
...
class TestDocumentAccess(ProjectOsCommon):
    ...
    def test_management_is_invisible_to_a_pod_member(self):
        """Client contracts are not a Tasker's business, and the honest way to say
        ...
        as_member = self.env['epo.document'].with_user(self.user_tasker)
        ...
        as_gpm = self.env['epo.document'].with_user(self.user_pm)
    ...
    def test_folder_tree_hides_management_from_a_pod_member(self):
        ...
        tree = self.env['epo.folder'].tree_for(project, user=self.user_tasker)
        self.assertEqual([node['slug'] for node in tree], ['knowledge'])
        full = self.env['epo.folder'].tree_for(project, user=self.user_pm)
...
class TestCrossProjectIsolation(ProjectOsCommon):
    """A Tasker must not be able to read a project they were never put on.
    ...
    def setUp(self):
        super().setUp()
        # emp_tasker is on `mine`; nobody has put them anywhere near `theirs`.
        self.mine, _t, _f = self._live_project('Mine')
        self.env['epo.allocation'].create({
            'project_id': self.mine.id, 'employee_id': self.emp_tasker.id})
    ...
    def test_cannot_read_another_projects_knowledge(self):
        ...
        visible = self.env['epo.document'].with_user(self.user_tasker).search([
            ('project_id', '=', self.theirs.id)])
        self.assertFalse(visible)

    def test_cannot_read_another_projects_folders(self):
        visible = self.env['epo.folder'].with_user(self.user_tasker).search([
            ('project_id', '=', self.theirs.id)])
        self.assertFalse(visible)

    def test_cannot_read_another_projects_form(self):
        """The questions on a stagelist are project IP."""
        visible = self.env['epo.form.template'].with_user(self.user_tasker).search([
    ...
    def test_cannot_read_someone_elses_role_grant(self):
        ...
        visible = self.env['epo.role.assignment'].with_user(self.user_tasker).search([
            ('id', '=', grant.id)])
        self.assertFalse(visible)

    def test_can_still_read_their_own_project(self):
        """The rules must narrow, not break: the day job has to keep working."""
        own_docs = self.env['epo.document'].with_user(self.user_tasker).search([
            ('project_id', '=', self.mine.id)])
        self.assertTrue(own_docs, 'a Tasker lost access to their own SOP')
        own_folders = self.env['epo.folder'].with_user(self.user_tasker).search([
            ('project_id', '=', self.mine.id)])
        self.assertTrue(own_folders)
        own_grant = self.env['epo.role.assignment'].with_user(self.user_tasker).search([
            ('employee_id', '=', self.emp_tasker.id)])
...
class TestPhaseWindow(ProjectOsCommon):
    ...
    def test_backfill_splits_rather_than_rewriting_the_present(self):
        ...
        self.assertEqual(allocation.current_phase, 'tasking')
        self.env['epo.roster.day'].upsert(
            self.emp_tasker.id, self.today - timedelta(days=4),
            {'tasking_status': 'training', 'project_id': self.project.id})
        allocation.invalidate_recordset()
        self.assertEqual(allocation.current_phase, 'tasking')
...
class TestMinimumScore(ProjectOsCommon):
    ...
    def _grade(self, employee, score, project=None):
        ...
        v2 4.6.3: Odoo does not grade, so a score exists only because a Tasker read it
    ...
    def test_below_the_bar_is_refused(self):
        self._grade(self.emp_tasker, 72.0)
    ...
    def test_at_or_above_the_bar_is_allowed(self):
        self._grade(self.emp_tasker, 88.0)
        allocation = self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id})
    ...
    def test_no_score_at_all_is_refused_but_says_so(self):
        ...
        with self.assertRaises(ValidationError) as caught:
            self.env['epo.allocation'].create({
                'project_id': self.project.id, 'employee_id': self.emp_tasker2.id})
    ...
    def test_raising_the_bar_does_not_unseat_people_already_working(self):
        ...
        self._grade(self.emp_tasker, 85.0)
        allocation = self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id})
    ...
    def test_no_bar_means_anybody(self):
        ...
        self.assertTrue(self.env['epo.allocation'].create({
            'project_id': open_project.id, 'employee_id': self.emp_tasker2.id}))

    def test_the_candidate_list_explains_every_exclusion(self):
        """A staffing screen that silently omits people cannot tell the PM the
        difference between "nobody qualifies" and "the filter is broken"."""
        self._grade(self.emp_tasker, 88.0)
        self._grade(self.emp_tasker2, 40.0)
        # candidates() returns only people at or above the bar; the cut ones come
        # back with their reason under include_below_bar.
        rows = {r['employee_id']: r
                for r in self.project.candidates(include_below_bar=True)}

        self.assertTrue(rows[self.emp_tasker.id]['eligible'])
        self.assertEqual(rows[self.emp_tasker.id]['best_score'], 88.0)

        self.assertFalse(rows[self.emp_tasker2.id]['eligible'])
        self.assertIn('40', rows[self.emp_tasker2.id]['reason'])
        self.assertIn('80', rows[self.emp_tasker2.id]['reason'])

    def test_already_allocated_people_are_shown_as_such(self):
        self._grade(self.emp_tasker, 90.0)
        self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id})
        # Already-allocated people are an exclusion like any other, so they only come
        # back under include_below_bar — candidates() is the shortlist, not the roll.
        row = next(r for r in self.project.candidates(include_below_bar=True)
                   if r['employee_id'] == self.emp_tasker.id)
...
class TestAllocationNotice(ProjectOsCommon):
    ...
    def test_joining_a_project_emails_the_pod_member(self):
        ...
        self.assertIn(self.emp_tasker.work_email, mail.email_to or '')
    ...
    def test_a_missing_work_email_does_not_block_the_allocation(self):
        ...
        self.emp_tasker2.work_email = False
        allocation = self.env['epo.allocation'].create({
            'project_id': project.id, 'employee_id': self.emp_tasker2.id})
        self.assertTrue(allocation.exists())
...
class TestEdgeCases(ProjectOsCommon):
    ...
    def test_a_new_version_carries_the_whole_form(self):
        """Odoo does not copy one2many relations, so this used to hand the PM an empty
    ...
    def test_carry_forward_survives_somebody_being_released(self):
        ...
        for employee in (self.emp_tasker, self.emp_tasker2):
        ...
        released_row = Roster.search([
            ('employee_id', '=', self.emp_tasker.id), ('business_date', '=', self.today)])
        ...
        kept_row = Roster.search([
            ('employee_id', '=', self.emp_tasker2.id), ('business_date', '=', self.today)])
    ...
    def test_bulk_allocation_reports_who_it_skipped(self):
        """A PM picking twelve people off a list that went stale while they read it
        ...
        # emp_tasker cleared the bar on an earlier project; emp_tasker2 has never been assessed.
        # The bar reads the best score ANYWHERE, which is what makes this a real test of
        # the skip path rather than of one project's own paperwork.
        prior = self._live_project('Prior Paper')[0]
        self.env['epo.onboarding']._get(self.emp_tasker.id, prior.id).write({
            'assessment_passed': True,
        ...
        created, skipped = self.env['epo.allocation'].allocate_many(
            project.id, [self.emp_tasker.id, self.emp_tasker2.id])

        self.assertEqual(len(created), 1, 'the qualifying person was not allocated')
        self.assertEqual(created.employee_id, self.emp_tasker)
        self.assertEqual(len(skipped), 1)
        self.assertIn(self.emp_tasker2.name, skipped[0])
    ...
    def test_an_answer_for_another_forms_field_is_not_stored(self):
        ...
        self._cleared(project, self.emp_tasker)
        entry = self.env['epo.form.entry'].submit_answers(
            template.id, self.emp_tasker.id,
...
class TestAssessmentOpensTheGate(ProjectOsCommon):
    ...
    none asserted it opened. A Tasker could pass and stay locked out for good.
    """
    ...
    def setUp(self):
        ...
        self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id})
        self.onboarding = self._cleared(self.project, self.emp_tasker)

    def _grade(self, score):
        """Record what a Tasker read in the external application.

        v2 4.6.3: Odoo does not grade, so `passed` is the verifier's judgement — here
        taken as "did it clear the paper's own pass mark", which is what a Tasker reading
        that screen would conclude.
        """
        self.onboarding.write({
            'assessment_passed': score >= self.link.pass_score,
            'assessment_score': score,
            'assessment_verified_by': self.env.uid,
            'assessment_verified_at': fields.Datetime.now(),
        })
    ...
    def test_correcting_a_fail_to_a_pass_opens_it(self):
        """A verdict recorded by hand has to be correctable, and the correction has to
        open the gate.

        Replaces an older test about retakes: v2 4.6.2 removed attempts entirely, so
        `max_attempts` no longer exists. What survives — and matters more — is that a Tasker
        who recorded the wrong result can fix it and the person is not left locked out.
        """
        self._grade(40.0)
        self.onboarding.invalidate_recordset()
        self.assertFalse(self.onboarding.unlocked)
        self._grade(90.0)
        self.onboarding.invalidate_recordset()
        self.assertTrue(self.onboarding.unlocked,
                        'the corrected verdict did not open the gate')

    def test_the_gate_opening_moves_the_allocation_into_tasking(self):
        self._grade(75.0)
        allocation = self.emp_tasker.epo_allocation_ids.filtered(
...
class TestBackfillDoesNotEndTheCurrentStretch(ProjectOsCommon):
    ...
    def setUp(self):
        ...
        self.allocation = self.env['epo.allocation'].create({
            'project_id': self.project.id, 'employee_id': self.emp_tasker.id,
            'date_from': self.today - timedelta(days=10)})
        self._cleared(self.project, self.emp_tasker)      # opens a stretch dated today

    def test_a_past_leave_day_leaves_today_alone(self):
        self.env['epo.roster.day'].upsert(
            self.emp_tasker.id, self.today - timedelta(days=6),
    ...
    def test_working_a_week_gives_contiguous_stretches(self):
        ...
        for offset in range(6, -1, -1):
            Roster.upsert(self.emp_tasker.id, self.today - timedelta(days=offset),
                          {'tasking_status': 'tasking', 'project_id': self.project.id})
        self.allocation.invalidate_recordset()
