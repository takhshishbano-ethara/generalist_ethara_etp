...
PR #2 adds the human-override fields to t2av.video.review and introduces
...
def migrate(cr, version):
    ...
    _logger.info(
        "t2av 19.0.1.14.0: backfilled effective_verdict on %d rows", affected,
    )

    cr.execute(
        """
        SELECT COUNT(*) FROM t2av_video_review
    ...
    if overridden:
        _logger.warning(
            "t2av 19.0.1.14.0: %d row(s) already have human_verdict set "
    ...
    cr.execute(
        ...
        UPDATE ir_sequence
        SET padding = 7
        WHERE code LIKE 't2av.attempt.%%'
          AND padding < 7
        """
    )
    bumped = cr.rowcount or 0
    _logger.info(
        "t2av 19.0.1.14.0: bumped padding on %d per-category attempt sequence(s) "
        "from 6 to 7", bumped,
    )
