...
from ..models.t2av_generation import CATEGORY_SELECTION

_VALID_DURATIONS = {"4", "5", "6", "7", "8", "9", "10", "12", "15"}
_VALID_RESOLUTIONS = {"480p", "720p", "1080p"}
_PIXEL_TO_RESOLUTION = {
    "854x480": "480p",
    "1280x720": "720p",
    "1920x1080": "1080p",
}
...
class T2AVImportWizard(models.TransientModel):
    _name = "t2av.import.wizard"
    _description = "T2AV CSV Import Wizard"
    ...
    def _validate_row(self, row_num, row, header_map, category_lookup):
        ...
        if resolution:
            normalized = _PIXEL_TO_RESOLUTION.get(
                resolution.lower(), resolution.lower(),
            )
            if normalized not in _VALID_RESOLUTIONS:
                errors.append(_(
                    "Row %(row)d: resolution '%(val)s' invalid. "
                    "Allowed: 480p, 720p, 1080p, 854x480, 1280x720, 1920x1080."
