...
``t2av.s3.storage`` AbstractModel (which talks to boto3 directly and gives
...
``env["s3.connector"]`` and ``env["t2av.s3.storage"]``) so it is NOT pure,
...
from ..models.t2av_s3_storage import S3StorageError, S3VerificationError
    ...
    try:
        ...
        if tmp_path and os.path.exists(tmp_path):
            try:
                ...
                _logger.warning("T2AV: failed to delete temp file %s", tmp_path)
...
def _download_to_temp(url: str, *, auth_bearer: str | None = None) -> tuple[str, int, str]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            last_exc = e
            _logger.warning(
                "T2AV: download attempt %d/%d failed: %s",
...
def _upload_with_retry(storage, connector_id: int, key: str, tmp_path: str) -> dict:
    """Upload local file via ``t2av.s3.storage.upload_with_integrity``.
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            last_exc = e
            _logger.warning(
                "T2AV: S3 upload attempt %d/%d failed: %s",
                attempt + 1, MAX_RETRIES, e,
            )
            if attempt < MAX_RETRIES - 1:
                time.sleep(RETRY_BACKOFF[attempt])
