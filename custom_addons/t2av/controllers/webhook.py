"""T2AV webhook controller — receives OpenRouter generation callbacks.

Inactive by default: returns 503 unless ``t2av.webhook_secret`` is set in
...
dispatches to ``t2av.attempt._handle_webhook_event``.
"""
import json
import logging
...
class T2AVWebhookController(http.Controller):
    ...
    def openrouter_webhook(self, **kwargs):
        ...
        tolerance = int(ICP.get_param("t2av.webhook_signature_tolerance", "300"))

        if not secret:
            _logger.warning("T2AV webhook: no secret configured, rejecting")
            return request.make_json_response({"error": "not_configured"}, status=503)

        verifier = request.env["t2av.webhook.verifier"].sudo()
        if not verifier.verify(raw_body, sig_header, secret, tolerance):
            _logger.warning("T2AV webhook: invalid signature")
            return request.make_json_response({"error": "invalid_signature"}, status=401)

        try:
            payload = json.loads(raw_body.decode("utf-8") if raw_body else "{}")
        except (ValueError, UnicodeDecodeError) as e:
            _logger.warning("T2AV webhook: invalid JSON: %s", e)
        ...
        Attempt = request.env["t2av.attempt"].sudo()
        attempt = Attempt.search(
            [("openrouter_job_id", "=", openrouter_job_id)], limit=1,
        )
        if not attempt:
            _logger.warning(
                "T2AV webhook: unknown openrouter_job_id %s", openrouter_job_id,
            )
        ...
        request.env.cr.execute(
            "SELECT webhook_idempotency_key FROM t2av_attempt "
        ...
        if idempotency_key and existing_key == idempotency_key:
            _logger.info(
                "T2AV webhook: already processed (idempotency key %s)",
                idempotency_key,
            )
        ...
        try:
            ...
            _logger.exception(
                "T2AV webhook: handler failed for attempt %s", attempt.id,
            )
