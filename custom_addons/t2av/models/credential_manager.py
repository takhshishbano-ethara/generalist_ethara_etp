...
ENCRYPTED_PARAMS = frozenset({
    "t2av.openrouter_api_key",
    "t2av.aws_access_key",
    "t2av.aws_secret_key",
    "t2av.bedrock_api_key",
    "t2av.webhook_secret",
})
...
def _get_or_create_key(ICP) -> bytes:
    ...
    env_key = os.environ.get("T2AV_ENCRYPTION_KEY", "").strip()
    if env_key:
        return env_key.encode()
    with _key_cache_lock:
        if _cached_fernet_key is not None:
            return _cached_fernet_key
    key_str = ICP.get_param("t2av.encryption_key", "")
    if key_str:
        _logger.error(
            "T2AV: encryption key loaded from database — NOT SAFE FOR PRODUCTION. "
            "Set T2AV_ENCRYPTION_KEY environment variable."
        )
    ...
    ICP.set_param("t2av.encryption_key", key.decode())
    _logger.warning(
        "T2AV: generated new Fernet encryption key and stored in database. "
        "For production, set T2AV_ENCRYPTION_KEY environment variable."
    )
    with _key_cache_lock:
...
def _get_previous_key(ICP) -> bytes | None:
    env_prev = os.environ.get("T2AV_ENCRYPTION_KEY_PREVIOUS", "").strip()
    if env_prev:
        return env_prev.encode()
    prev_str = ICP.get_param("t2av.encryption_key_previous", "")
    if prev_str:
...
def _get_or_create_key_raw(cr) -> bytes:
    ...
    if row and row[0]:
        _logger.error(
            "T2AV: encryption key loaded from database (background thread) — "
            "NOT SAFE FOR PRODUCTION. Set T2AV_ENCRYPTION_KEY environment variable."
        )
        result = row[0].encode()
        with _key_cache_lock:
    ...
    cr.execute(
        ...
        """,
        ("t2av.encryption_key", key.decode()),
    )
    cr.commit()
    _logger.info("T2AV: generated new Fernet encryption key (raw SQL).")
    with _key_cache_lock:
...
def decrypt_value(ICP, stored: str) -> str:
    ...
    try:
        ...
        _logger.error(
            "T2AV: failed to decrypt config value. "
...
def decrypt_value_raw(cr, stored: str) -> str:
    ...
    try:
        ...
        _logger.error(
            "T2AV: failed to decrypt config value (raw). "
            "Key may have changed or data is corrupted. Returning empty string."
        )
        return ""
...
def get_encrypted_param(env, key: str, default: str = "") -> str:
    ...
    if key in ENCRYPTED_PARAMS:
        ...
        if not decrypted and stored:
            _logger.warning(
                "T2AV: encrypted param '%s' is set but decryption returned empty. "
                "Check T2AV_ENCRYPTION_KEY matches the key used to encrypt.",
                key,
            )
        return decrypted
    return stored
...
def get_openrouter_api_key(env):
    return get_encrypted_param(env, "t2av.openrouter_api_key")


def get_openrouter_api_key_raw(cr):
    return get_encrypted_param_raw(cr, "t2av.openrouter_api_key")


def get_aws_access_key(env):
    return get_encrypted_param(env, "t2av.aws_access_key")


def get_aws_access_key_raw(cr):
    return get_encrypted_param_raw(cr, "t2av.aws_access_key")


def get_aws_secret_key(env):
    return get_encrypted_param(env, "t2av.aws_secret_key")


def get_aws_secret_key_raw(cr):
    return get_encrypted_param_raw(cr, "t2av.aws_secret_key")


def get_webhook_secret(env):
    return get_encrypted_param(env, "t2av.webhook_secret")


def get_webhook_secret_raw(cr):
    return get_encrypted_param_raw(cr, "t2av.webhook_secret")


def get_bedrock_api_key(env):
    return get_encrypted_param(env, "t2av.bedrock_api_key")


def get_bedrock_api_key_raw(cr):
    return get_encrypted_param_raw(cr, "t2av.bedrock_api_key")
