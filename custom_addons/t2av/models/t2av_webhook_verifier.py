"""T2AV webhook HMAC verifier (HMAC-SHA256 with timestamp tolerance).

Used by ``/t2av/webhook`` to verify that incoming OpenRouter callbacks
are genuine. Stays inert until the ``t2av.webhook_secret`` ir.config_parameter
is set.
"""
import hashlib
import hmac
...
class T2AVWebhookVerifier(models.AbstractModel):
    _name = "t2av.webhook.verifier"
    ...
    def verify(self, raw_body, sig_header, secret, tolerance_seconds=300):
        ...
        if abs(time.time() - ts_val) > tolerance_seconds:
            _logger.warning(
                "T2AV webhook: timestamp out of tolerance (%s vs %s)",
                ts_val, time.time(),
            )
            return False
