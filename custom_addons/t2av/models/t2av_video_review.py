...
class T2AVVideoReview(models.Model):
    _name = "t2av.video.review"
    _description = "T2AV Video QC Review"
    _order = "attempt_id, create_date desc"
    _rec_name = "display_name"

    attempt_id = fields.Many2one(
        "t2av.attempt", string="Attempt",
        required=True, ondelete="cascade", index=True,
    )
    job_id = fields.Many2one(
        "t2av.generation", related="attempt_id.job_id",
        ...
        for rec in stuck:
            with self.env.cr.savepoint():
                try:
                    ...
                    _logger.exception("T2AV video-review watchdog: failed on id=%s", rec.id)
        if stuck:
            _logger.info("T2AV video-review watchdog: reset %d stuck row(s)", len(stuck))
        ...
        def _run():
            try:
                ...
                with registry.cursor() as cr:
                    ...
                    rec = env["t2av.video.review"].browse(rec_id).exists()
                    if rec:
                        getattr(rec, callback_name)(*args)
            except Exception:
                _logger.exception(
                    "T2AV video-review deferred task failed (id=%s, cb=%s)",
                ...
                try:
                    ...
                    with registry.cursor() as cr2:
                        ...
                        rec = env2["t2av.video.review"].browse(rec_id).exists()
                    ...
                    _logger.exception(
                        "T2AV: failed to mark video-review %s as error", rec_id,
                    )
    ...
    def _run_review(self):
        ...
        if not self.env.cr.fetchone():
            _logger.info(
                "T2AV video-review %s: already claimed by another worker; skipping",
                self.id,
            )
            return
        ...
        try:
            ...
            _logger.exception("T2AV video-review: unexpected error in _run_review")
    ...
    def _run_review_body(self):
        ...
        provider = (icp.get_param(
            "t2av.review_provider", review_client.DEFAULT_PROVIDER,
        ...
        if provider == "bedrock":
            ...
            if not openrouter_api_key:
                ...
                return
            model_id = icp.get_param(
                "t2av.openrouter_review_model_id",
        ...
        region = icp.get_param(
            "t2av.bedrock_region", review_client.DEFAULT_REGION,
        )
        try:
            num_frames = int(icp.get_param("t2av.review_num_frames", "20"))
        ...
        try:
            ...
            _logger.exception("T2AV video-review: unexpected error")
    ...
    def _build_review_video_url(self, attempt):
        ...
        try:
            ttl = int(icp.get_param(
                "t2av.review_video_url_ttl_seconds",
        ...
        if attempt.video_s3_key and connector_id:
            try:
                return self.env["t2av.s3.storage"].presigned_get_url(
                    connector_id,
                ...
                _logger.exception(
                    "T2AV video-review: failed to presign URL for attempt %s",
                    attempt.id,
                )
    ...
    def _maybe_create_sheet_row(self, attempt, verdict_norm):
        ...
        try:
            ...
            _logger.exception(
                "T2AV: failed to create sequence-sheet row for review %s (type=%s)",
                self.id, sheet_type,
            )
    ...
    def _reevaluate_sheet_row(self):
        ...
        Row = self.env["t2av.sequence.sheet.row"].sudo()
        ...
        if desired_type:
            try:
                ...
                _logger.exception(
                    "T2AV: failed to re-evaluate sheet row for review %s "
    ...
    def _maybe_spawn_retry(self, attempt, verdict_norm):
        ...
        if existing >= max_attempts:
            ...
            return
        new_review = self.env["t2av.video.review"].create({
            "attempt_id": attempt.id,
        })
        if job:
            job.message_post(body=_(
    ...
    def _cron_run_queued_reviews(self):
        ...
        _logger.info(
            "T2AV video-review cron: dispatched %d review(s) (in-flight=%d, max=%d)",
    ...
    def _open_override_wizard(self, mode):
        ...
        return {
            ...
            "res_model": "t2av.review.override.wizard",
            "view_mode": "form",
            "target": "new",
            "context": {
