...
The Leviathan pipeline now uses **direct async Lambda invocation**
...
``rabbitmq_service``. Replace any call site with the new entry point:
    ...
    batch_publish_leviathan_tasks(record_ids)
...
class LeviathanQueueRemovedError(RuntimeError):
    """Raised when deprecated RabbitMQ entry points are called."""


_MIGRATION_MSG = (
    "RabbitMQ orchestration was removed in leviathan 19.0.2.0.0. "
    "Use leviathan.job.action_run_batch_concurrent() instead. "
    "See custom_addons/leviathan/docs/EKS_DEPLOYMENT.md for the new "
    "async-Lambda architecture (250 concurrent without a queue)."
)


def publish_leviathan_task(record_id, action="run_pipeline"):
    raise LeviathanQueueRemovedError(_MIGRATION_MSG)
