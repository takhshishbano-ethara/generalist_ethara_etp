...
def run_qc(
    ...
    """
    _logger.info(
        "[leviathan] QC start: url=%s category=%s prd=%dB screenshots=%d",
    ...
    _logger.info(
        "[leviathan] QC structural checks done: %d issue(s) — calling "
    ...
    _logger.info(
        "[leviathan] QC verdict=%s — issues critical=%d high=%d medium=%d "
