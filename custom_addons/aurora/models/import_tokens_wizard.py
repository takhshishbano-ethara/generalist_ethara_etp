...
from .credential_manager import encrypt_value
...
class AuroraImportTokensWizard(models.TransientModel):
    _name = "aurora.import.tokens.wizard"
    ...
    def action_import(self):
        ...
        self.env.cr.execute("""
            SELECT token_hash FROM aurora_github_token
        ...
        existing_count = self.env["aurora.github.token"].search_count([])
        ...
        for start in range(0, len(vals_list), _BATCH_SIZE):
            ...
            self.env["aurora.github.token"].create(batch)
            created += len(batch)
        ...
        msg_parts.append(f"Tokens are in 'draft' state pending health check verification")
