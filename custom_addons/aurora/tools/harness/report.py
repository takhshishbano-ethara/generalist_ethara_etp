...
from .pull_request import Base, PullRequest, PullRequestBase
from .test_result import Test, TestResult, TestStatus
...
class Report(PullRequestBase):
    ...
    run_result: Optional[TestResult] = None
    test_patch_result: Optional[TestResult] = None
    fix_patch_result: Optional[TestResult] = None
    ...
    def check(self, force: bool = False) -> Tuple[bool, str]:
        ...
        if self.fix_patch_result.all_count == 0:
            self.valid = False
            self.error_msg = (
                f"After applying the fix patch, no test results were captured "
                f"when executing the test command. A brief summary is as follows: "
                f"{self.short_report()}"
            )
            return (self.valid, self.error_msg)
        ...
        for name, test in self._tests.items():
            if test.test == TestStatus.PASS and test.fix == TestStatus.FAIL:
                self.valid = False
                self.error_msg = (
                    f"Before applying the fix patch, the test passed; however, "
                    f"after applying the fix patch, the test failed. A brief summary "
                    f"is as follows: {self.short_report()}. `{name}`: {test}"
                )
                return (self.valid, self.error_msg)
        ...
        if not fix_something:
            self.valid = False
            self.error_msg = (
                f"After applying the fix patch, no test cases transitioned from "
                f"failed to passed. A brief summary is as follows: {self.short_report()}"
            )
            return (self.valid, self.error_msg)
        ...
        for name, test in self._tests.items():
            if (
                ...
                self.valid = False
                self.error_msg = (
                    f"By comparing the test results before and after applying the fix "
                    f"patch, an anomalous pattern was detected. A brief summary is as "
                    f"follows: {self.short_report()}. `{name}`: {test}"
                )
                return (self.valid, self.error_msg)
    ...
    def short_report(self) -> str:
        return (
            ...
            "Each stage is reported as (pass, fail, skip), representing the number of tests "
            "that passed, failed, or were skipped, respectively.\n\n"
            f"Results:\n"
...
class ReportTask(PullRequestBase):
    ...
    def test_patch_run_log(self) -> str:
        path = self.instance_dir / TEST_PATCH_RUN_LOG_FILE
        if not path.exists():
            raise FileNotFoundError(f"Test patch run log file not found: {path}")
        with open(path, "r", encoding="utf-8") as f:
            return f.read()

    @property
    def fix_patch_run_log(self) -> str:
        path = self.instance_dir / FIX_PATCH_RUN_LOG_FILE
        if not path.exists():
            raise FileNotFoundError(f"Fix patch run log file not found: {path}")
        with open(path, "r", encoding="utf-8") as f:
            return f.read()
        ...
        if not regen:
            ...
            if report_path.exists():
                with open(report_path, "r", encoding="utf-8") as f:
                    return Report.from_json(f.read())
