...
CGO_ENABLED=0 go test -v -tags kqueue -count=1 ./... || true
...
for pkg in $PKGS; do
  CGO_ENABLED=0 go test -v -tags kqueue -count=1 -timeout 15m "$pkg" || true
done
...
@Instance.register("minio", "minio_go1_15")
class Minio_go1_15(Instance):
