...
class UrfaveCliImageBase(Image):
    ...
    def _go_version(self) -> str:
        if self._pr.base.ref in ("v1", "v1-maint", "master"):
            return "1.14"
        return "latest"
...
class UrfaveCliImageDefault(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Image | None:
        return UrfaveCliImageBase(self.pr, self.config)
...
@Instance.register("urfave", "cli")
class UrfaveCli(Instance):
    ...
    def dependency(self) -> Optional[Image]:
        return UrfaveCliImageDefault(self.pr, self._config)
