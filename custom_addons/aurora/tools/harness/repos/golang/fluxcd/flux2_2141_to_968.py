"""fluxcd/flux2 config for PRs 968-2141 (Go 1.16 era, Debian Bullseye)."""
...
class Flux2ImageBaseGo116(Image):
...
class Flux2ImageDefaultGo116(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Union[str, Image]:
        return Flux2ImageBaseGo116(self.pr, self.config)
...
""".format(repo=self.pr.repo),
    ...
    def dockerfile(self) -> str:
        ...
        if isinstance(image, str):
            raise ValueError("Flux2ImageDefaultGo116 dependency must be an Image")
...
@Instance.register("fluxcd", "flux2_2141_to_968")
class FLUX2_2141_TO_968(Instance):
    ...
    def dependency(self) -> Image:
        return Flux2ImageDefaultGo116(self.pr, self._config)
