"""fluxcd/flux2 config for PRs 4042-4532 (Go 1.20 era, Debian Bookworm)."""
...
class Flux2ImageBaseGo120(Image):
...
class Flux2ImageDefaultGo120(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Union[str, Image]:
        return Flux2ImageBaseGo120(self.pr, self.config)
...
""".format(repo=self.pr.repo),
    ...
    def dockerfile(self) -> str:
        ...
        if isinstance(image, str):
            raise ValueError("Flux2ImageDefaultGo120 dependency must be an Image")
...
@Instance.register("fluxcd", "flux2_4532_to_4042")
class FLUX2_4532_TO_4042(Instance):
    ...
    def dependency(self) -> Image:
        return Flux2ImageDefaultGo120(self.pr, self._config)
