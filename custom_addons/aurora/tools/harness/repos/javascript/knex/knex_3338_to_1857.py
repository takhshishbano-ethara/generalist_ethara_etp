...
class knexImageBase_era1(Image):
...
class knexImageDefault_era1(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Image | None:
        return knexImageBase_era1(self.pr, self._config)
...
cd node_modules/sqlite3 2>/dev/null && npx node-gyp rebuild 2>/dev/null && cd /home/{repo_dir} || true

mkdir -p node_modules/oracledb
echo 'module.exports = {{}};' > node_modules/oracledb/index.js

npm run build 2>/dev/null || npx babel src --out-dir lib 2>/dev/null || true
...
@Instance.register("knex", "knex_3338_to_1857")
class KNEX_3338_TO_1857(Instance):
    ...
    def dependency(self) -> Optional[Image]:
        return knexImageDefault_era1(self.pr, self._config)
    ...
    def parse_log(self, test_log: str) -> TestResult:
        ...
        # Mocha patterns:
        # Pass: "  ✓ test name" or "  ✔ test name" (with optional duration)
        ...
        # Remove overlap: same test name can pass for one DB dialect and fail for another
