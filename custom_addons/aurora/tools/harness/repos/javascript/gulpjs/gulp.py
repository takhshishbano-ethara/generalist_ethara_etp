...
class ImageDefault(Image):
    ...
    def files(self) -> list[File]:
        mocha_cmd = """\
TEST_CMD=$(node -p "try{require('./package.json').scripts.test}catch(e){''}")

if echo "$TEST_CMD" | grep -q "\\-\\-async-only"; then
    npx mocha --reporter json --async-only
else
...
MOCHA_VER=$(node -p "try{{JSON.parse(require('fs').readFileSync('package.json','utf8')).devDependencies.mocha||''}}catch(e){{''}}")
MOCHA_MAJOR=$(echo "$MOCHA_VER" | grep -oP "\\d+" | head -1)

if [ -n "$MOCHA_MAJOR" ] && [ "$MOCHA_MAJOR" -lt 3 ]; then
    echo "prepare.sh: mocha $MOCHA_VER (major $MOCHA_MAJOR) detected, installing Node 10"
...
@Instance.register("gulpjs", "gulp")
class GulpjsGulp(Instance):
