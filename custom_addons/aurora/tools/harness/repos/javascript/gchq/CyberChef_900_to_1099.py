...
class CyberChefTransitionalImageBase(Image):
...
class CyberChefTransitionalImageDefault(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Image | None:
        return CyberChefTransitionalImageBase(self.pr, self._config)
...
node --experimental-modules --experimental-specifier-resolution=node --no-warnings --no-deprecation tests/node/index.mjs || true
node --experimental-modules --experimental-specifier-resolution=node --no-warnings --no-deprecation tests/operations/index.mjs
""".format(pr=self.pr),
            ),
            File(
                ".",
...
@Instance.register("gchq", "CyberChef_900_to_1099")
class CYBERCHEF_900_TO_1099(Instance):
    ...
    def dependency(self) -> Optional[Image]:
        return CyberChefTransitionalImageDefault(self.pr, self._config)
    ...
    def parse_log(self, test_log: str) -> TestResult:
        ...
        # Same CyberChef TestRegister output as other eras:
        # ✔️️ / ❌ / 🔥 per test, PASSING/FAILING/TOTAL summary
