...
RUN echo 'deb http://archive.debian.org/debian buster main' > /etc/apt/sources.list && \
    ...
    apt-get install -y --no-install-recommends --allow-unauthenticated git libwebkit2gtk-4.0-dev && \
...
RUN git clone --bare https://github.com/rust-lang/crates.io-index.git /opt/crates-index.git && \
    ...
    head -12 mi/ni/miniz_oxide > mi/ni/miniz_oxide.tmp && mv mi/ni/miniz_oxide.tmp mi/ni/miniz_oxide && \
...
class ImageDefault839to80(Image):
    def __init__(self, pr: PullRequest, config: Config):
        self._pr = pr
        self._config = config
    ...
    def dependency(self) -> Image | None:
        return ImageBase839to80(self.pr, self.config)
...
@Instance.register("tauri-apps", "tauri_839_to_80")
class Tauri839to80(Instance):
    ...
    def dependency(self) -> Optional[Image]:
        return ImageDefault839to80(self.pr, self._config)
