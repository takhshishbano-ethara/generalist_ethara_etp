...
###ACTION_DELIMITER###
echo 'python3 -m pytest tests/ --ignore=tests/test_memleaks.py -v --no-header -rA --tb=no --continue-on-collection-errors -o addopts= --timeout=300 --timeout-method=thread' > test_commands.sh
###ACTION_DELIMITER###
...
export CI=true
cd /home/{pr.repo}
python3 -m pytest tests/ --ignore=tests/test_memleaks.py -v --no-header -rA --tb=no --continue-on-collection-errors -o addopts=
...
@Instance.register("giampaolo", "psutil_2700_to_2700")
class PSUTIL_2700_TO_2700(Instance):
    ...
    def parse_log(self, log: str) -> TestResult:
        ...
        # "tests/test_system.py::TestCPU::test_count PASSED [  2%]"
        pattern1 = re.compile(
            r"^(tests/[\w\/\.\-::\[\]]+)\s+"
        ...
        # "PASSED tests/test_system.py::TestCPU::test_count"
        pattern2 = re.compile(
            r"(passed|failed|skipped|xfailed|xfail) "
            r"(tests/[\w\/\.\-::\[\]]+)",
            re.MULTILINE | re.IGNORECASE,
