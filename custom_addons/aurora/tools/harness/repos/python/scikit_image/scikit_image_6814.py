...
###ACTION_DELIMITER###
pip install ninja meson 'meson-python>=0.13' 'Cython>=0.29.26,<3.0' wheel 'setuptools>=67' packaging 'numpy<2' pythran 'lazy_loader>=0.2'
###ACTION_DELIMITER###
...
""".format(pr=self.pr),
            ...
            File(
                ...
                """#!/bin/bash
cd /home/{pr.repo}
if ! git -C /home/{pr.repo} apply --whitespace=nowarn --exclude='*.inv' --exclude='*.png'  /home/test.patch /home/fix.patch; then
    echo "Error: git apply failed" >&2
    exit 1  
fi
python -m pytest -v --pyargs skimage --continue-on-collection-errors
...
RUN pip install ninja meson 'meson-python>=0.13' 'Cython>=0.29.26,<3.0' wheel 'setuptools>=67' packaging 'numpy<2' pythran 'lazy_loader>=0.2'
...
@Instance.register("scikit-image", "scikit_image_6814")
class SCIKIT_IMAGE_6814(Instance):
