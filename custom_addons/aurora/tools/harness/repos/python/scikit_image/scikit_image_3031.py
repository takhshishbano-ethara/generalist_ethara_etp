...
###ACTION_DELIMITER###
pip install ninja meson-python meson 'Cython==3.0.10' pythran 'numpy<2' 'setuptools>=67' packaging wheel lazy_loader
###ACTION_DELIMITER###
...
""".format(pr=self.pr),
            ...
            File(
                ...
                """#!/bin/bash
cd /home/{pr.repo}
rm -f /home/{pr.repo}/skimage/data/gray_morph_output.npz
if ! git -C /home/{pr.repo} apply --whitespace=nowarn --exclude='*gray_morph_output.npz' /home/test.patch /home/fix.patch; then
    echo "Error: git apply failed" >&2
    exit 1  
fi
python -m pytest -v --pyargs skimage --continue-on-collection-errors
...
RUN pip install ninja meson-python meson 'Cython==3.0.10' pythran 'numpy<2' 'setuptools>=67' packaging wheel lazy_loader
RUN pip install --no-build-isolation -e .
RUN pip install pytest numpydoc
"""
...
@Instance.register("scikit-image", "scikit_image_3031")
class SCIKIT_IMAGE_3031(Instance):
