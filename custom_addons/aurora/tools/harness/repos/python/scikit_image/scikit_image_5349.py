...
###ACTION_DELIMITER###
pip install ninja meson 'meson-python>=0.13' 'Cython>=0.29.24,<3.0' wheel 'setuptools<=65.5' packaging 'numpy<2' pythran lazy_loader
###ACTION_DELIMITER###
...
""".format(pr=self.pr),
            ...
            File(
                ...
                """#!/bin/bash
cd /home/{pr.repo}
if ! git -C /home/{pr.repo} apply --whitespace=nowarn --exclude='*.inv' --exclude='*.png' --exclude='*.ico' /home/test.patch; then
...
""".format(pr=self.pr),
            ...
            File(
                ...
                """#!/bin/bash
cd /home/{pr.repo}
if ! git -C /home/{pr.repo} apply --whitespace=nowarn --exclude='*.inv' --exclude='*.png' --exclude='*.ico' /home/test.patch /home/fix.patch; then
    echo "Error: git apply failed" >&2
    exit 1  
fi
python -m pytest -v --pyargs skimage --continue-on-collection-errors
...
RUN pip install ninja meson 'meson-python>=0.13' 'Cython>=0.29.24,<3.0' wheel 'setuptools<=65.5' packaging 'numpy<2' pythran lazy_loader
RUN pip install --no-build-isolation -e .
RUN pip install pytest scipy Pillow imageio tifffile networkx PyWavelets
"""
...
@Instance.register("scikit-image", "scikit_image_5349")
class SCIKIT_IMAGE_5349(Instance):
