    ...
    def files(self) -> list[File]:
        return [
            ...
            File(
                ...
                """ls
###ACTION_DELIMITER###
pip install 'setuptools<60' 'numpy==1.21.6' 'Cython>=0.29.21,<3.0' wheel
###ACTION_DELIMITER###
apt-get update && apt-get install -y --no-install-recommends build-essential
###ACTION_DELIMITER###
pip install -r requirements/default.txt -r requirements/test.txt
###ACTION_DELIMITER###
CFLAGS='-Wno-error=implicit-function-declaration' python setup.py build_ext --inplace
###ACTION_DELIMITER###
...
python -m pytest -v skimage -x --timeout=60 2>&1 | tail -30""",
            ...
            File(
                ...
                """#!/bin/bash
cd /home/{pr.repo}
python -m pytest -v skimage --ignore=skimage/viewer --ignore=skimage/io/_plugins --ignore=skimage/io/tests/test_mpl_imshow.py --continue-on-collection-errors
...
RUN pip install 'setuptools<60' 'numpy==1.21.6' 'Cython>=0.29.21,<3.0' wheel
RUN apt-get update && apt-get install -y --no-install-recommends build-essential && rm -rf /var/lib/apt/lists/*
RUN pip install -r requirements/default.txt -r requirements/test.txt
RUN pip install 'numpy==1.21.6'
RUN CFLAGS='-Wno-error=implicit-function-declaration' python setup.py build_ext --inplace
"""
...
@Instance.register("scikit-image", "scikit_image_4237")
class SCIKIT_IMAGE_4237(Instance):
