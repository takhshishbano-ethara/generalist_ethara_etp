...
# PRs 23968, 24148, 24305, 24742, 24814
# Python 3.9 | meson build | bookworm
# numpy 1.25.x-1.26.x requires Python >= 3.9 and uses meson build system
...
pip install --no-cache-dir "setuptools<69" 2>/dev/null || true
pip install --no-cache-dir pytest "hypothesis<6.31" typing_extensions "cython<3" 2>/dev/null || true
pip install --no-cache-dir meson-python meson ninja "cython<3" pybind11 2>/dev/null || true
pip install --no-cache-dir --no-build-isolation . 2>/dev/null || pip install --no-cache-dir -e . 2>/dev/null || python setup.py build_ext --inplace 2>/dev/null || true
...
RUN pip install --no-cache-dir "setuptools<69" 2>/dev/null || true
RUN pip install --no-cache-dir pytest "hypothesis<6.31" typing_extensions meson-python meson ninja "cython<3" pybind11 2>/dev/null || true
...
@Instance.register("numpy", "numpy_24814_to_23968")
class NUMPY_24814_TO_23968(Instance):
