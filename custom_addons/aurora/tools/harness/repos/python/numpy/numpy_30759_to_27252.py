...
# PRs 27252, 27333, 27512, 27935, 28050, 28185, 28333, 28545, 28778
# Python 3.10 | conda + meson | numpy 2.1.x-2.2.x
class ImageDefault(Image):
    ...
    def files(self) -> list[File]:
        ...
        # For meson (non-editable install), resolve test paths from installed package
        # numpy gets installed at site-packages, so use `python -c "import numpy; print(numpy.__path__[0])"` at runtime
        ...
        if test_files_filtered:
            # Convert source paths like "numpy/_core/tests/test_foo.py" to "$NUMPY_INSTALLED/_core/tests/test_foo.py"
            # Strip the leading "numpy/" from each path since $NUMPY_INSTALLED already points to numpy/
...
""".format(resolve_prefix=resolve_prefix, test_args=resolved_test_args),
    ...
    def dockerfile(self) -> str:
        ...
        return f"""FROM python:3.10-slim-bookworm{global_env_block}
...
@Instance.register("numpy", "numpy_30759_to_27252")
class NUMPY_30759_TO_27252(Instance):
