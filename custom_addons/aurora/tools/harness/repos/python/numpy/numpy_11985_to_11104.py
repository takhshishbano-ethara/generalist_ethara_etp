...
# PRs 11104, 11985
# Python 2.7 | setup.py | ubuntu-trusty
# Dependency signature: py:2.x||os:ubuntu-trusty||deps:setup.py
class ImageDefault(Image):
    ...
    def files(self) -> list[File]:
        ...
        test_files_filtered = [
            f for f in (test_files_from_patch or [])
            if "/tests/" in f and f.endswith(".py")
        ]
        test_files = (
            " ".join(test_files_filtered) if test_files_filtered else "numpy/"
        )
        return [
            File(
                ".",
...
set -uxo pipefail
cd /home/numpy
python -m pytest {test_files} -rA --tb=short -v --ignore=doc -p no:cacheprovider 2>&1
...
@Instance.register("numpy", "numpy_11985_to_11104")
class NUMPY_11985_TO_11104(Instance):
