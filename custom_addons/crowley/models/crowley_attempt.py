"""Crowley video generation attempt — per-attempt state under a parent job.

A *crowley.generation* job owns up to **three** attempts: attempt #1 (the
...
# ---------------------------------------------------------------------------
_BUS_CHANNEL = "crowley.generation"
_BUS_TYPE = "crowley.generation.update"
...
class CrowleyAttempt(models.Model):
    _name = "crowley.attempt"
    _description = "Crowley Video Generation Attempt"
    ...
    def _compute_video_play_url(self):
        ...
        ttl = int(icp.get_param("crowley.presigned_ttl_seconds", "300") or "300")
        try:
        ...
        def _run():
            try:
                ...
                with registry.cursor() as cr:
                    ...
                    attempt = env["crowley.attempt"].browse(rec_id).exists()
                    if attempt:
    ...
    def _handle_completion(self, resp):
        ...
        if not self.env.cr.fetchone():
            _logger.info(
                "Crowley attempt %s: already past polling, skipping duplicate completion.",
                self.id,
            )
            return
    ...
    def _run_download(self):
        ...
        try:
            ...
            _logger.exception("Crowley: unexpected error in _run_download")
    ...
    # ------------------------------------------------------------------
    def _push_bus(self):
        """Notify the OWL crowley_live_status widget that this attempt's state changed."""
        self.ensure_one()
        try:
