...
class CrowleySequenceSheetRow(models.Model):
    _name = "crowley.sequence.sheet.row"
    _description = "Crowley Sequence Sheet Row (passed / failed)"
    ...
    review_id = fields.Many2one(
        "crowley.video.review", required=True, readonly=True,
        ondelete="cascade", index=True, copy=False,
    )
    attempt_id = fields.Many2one(
        "crowley.attempt", related="review_id.attempt_id",
        store=True, readonly=True, index=True,
    )
    job_id = fields.Many2one(
        "crowley.generation", related="review_id.job_id",
        store=True, readonly=True, index=True,
    )
    ...
    def _allocate_failure_sequence(self, category):
        code = f"crowley.attempt.{category}_fail"
    ...
    def create_from_review(self, review, sheet_type):
        ...
        if not attempt or not job:
            _logger.warning(
                "Crowley sheet-row: review %s missing attempt/job, skipping",
                review.id,
            )
            return self.browse()
