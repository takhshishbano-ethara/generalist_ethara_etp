"""Crowley v1.2 dataset-naming tests.
...
class TestDatasetNaming(TransactionCase):
    ...
    def _spawn_done_attempt(self, job, attempt_number, category, seq_n, **overrides):
        ...
        self.env.cr.execute(
            "UPDATE crowley_attempt SET state='done' WHERE id=%s",
            (attempt.id,),
        )
    ...
    def test_per_category_sequences_are_independent(self):
        ...
        seq_a = Sequence.search([("code", "=", f"crowley.attempt.{self.CATEGORY_A}")])
        seq_b = Sequence.search([("code", "=", f"crowley.attempt.{self.CATEGORY_B}")])
        ...
        consumed_a = [
            Sequence.next_by_code(f"crowley.attempt.{self.CATEGORY_A}")
            for _ in range(3)
        ]
        self.env.flush_all()
    ...
    def test_category_locked_false_when_only_failed_attempts(self):
        ...
        self.env.cr.execute(
            "UPDATE crowley_attempt SET state='failed', error_code='test' WHERE id=%s",
            (attempt.id,),
        )
        attempt.invalidate_recordset()
        job.invalidate_recordset()
    ...
    def test_legacy_record_without_category_cannot_be_submitted(self):
        ...
        job = self._make_job(category=self.CATEGORY_A)
        self.env.cr.execute(
            "UPDATE crowley_generation SET category=NULL WHERE id=%s",
            (job.id,),
        )
        job.invalidate_recordset()
    ...
    def test_retry_inherits_locked_category(self):
        ...
        with patch.object(type(job.env["crowley.attempt"]), "_defer", return_value=None), \
