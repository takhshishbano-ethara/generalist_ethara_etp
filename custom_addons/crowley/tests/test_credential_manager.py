...
from odoo.addons.crowley.models import credential_manager
from odoo.addons.crowley.models.credential_manager import (
    decrypt_value,
    encrypt_value,
    get_encrypted_param,
...
class TestCredentialManager(TransactionCase):
    ...
    def test_encrypt_decrypt_roundtrip(self):
        ...
        with mock.patch.dict(os.environ, {"CROWLEY_ENCRYPTION_KEY": key}, clear=False):
            credential_manager._cached_fernet_key = None
            ICP = self._icp()
    ...
    def test_get_encrypted_param_returns_decrypted(self):
        ...
        with mock.patch.dict(os.environ, {"CROWLEY_ENCRYPTION_KEY": key}, clear=False):
            credential_manager._cached_fernet_key = None
            set_encrypted_param(self.env, "crowley.openrouter_api_key", "my-key")
            # Raw value in DB must be encrypted (fernet:1: prefix).
            raw = self._icp().get_param("crowley.openrouter_api_key")
            self.assertTrue(raw.startswith("fernet:1:"))
            self.assertNotIn("my-key", raw)
            # Read-through getter decrypts.
            self.assertEqual(
                get_encrypted_param(self.env, "crowley.openrouter_api_key"),
                "my-key",
            )
    ...
    def test_get_openrouter_api_key_helper(self):
        ...
        with mock.patch.dict(os.environ, {"CROWLEY_ENCRYPTION_KEY": key}, clear=False):
            credential_manager._cached_fernet_key = None
            set_encrypted_param(self.env, "crowley.openrouter_api_key", "sk-or-xyz")
    ...
    def test_key_rotation_via_previous(self):
        ...
        with mock.patch.dict(os.environ, {"CROWLEY_ENCRYPTION_KEY": key1}, clear=False):
            credential_manager._cached_fernet_key = None
            ICP = self._icp()
        ...
        rotated_env = {
            "CROWLEY_ENCRYPTION_KEY": key2,
            "CROWLEY_ENCRYPTION_KEY_PREVIOUS": key1,
        }
