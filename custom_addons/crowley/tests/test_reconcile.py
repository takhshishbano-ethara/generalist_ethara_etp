"""Crowley v1.1 reconcile flow tests.
...
class TestReconcile(TransactionCase):
    ...
    def _make_job_with_attempt(self, attempt_state="processing", job_id="test_job_123"):
        job = self.env["crowley.generation"].create({
            "prompt": "test prompt",
            "duration": "5",
            "resolution": "720p",
            "aspect_ratio": "16:9",
        ...
        self.env.cr.execute(
            "UPDATE crowley_attempt SET state=%s, openrouter_job_id=%s WHERE id=%s",
    ...
    def test_reconcile_no_job_id_orphaned(self):
        ...
        self.env.cr.execute(
            "UPDATE crowley_attempt SET state='processing' WHERE id=%s",
            (attempt.id,),
        )
        attempt.invalidate_recordset()
        job.invalidate_recordset()
