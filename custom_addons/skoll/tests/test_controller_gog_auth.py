...
_GOG_MOD = "odoo.addons.skoll.controllers.gog_auth"


@tagged("post_install", "-at_install")
class TestGogStartAuth(SkollTestCase):

    def _ctrl(self):
        from odoo.addons.skoll.controllers.gog_auth import SkollGogAuthController
        return SkollGogAuthController()

    def _task_with_gog(self):
        task = self._create_task(
            task_id="GOG-START-%s" % self.env["ir.sequence"].next_by_code("skoll.skoll") or "001",
...
@tagged("post_install", "-at_install")
class TestExtractClientSecret(SkollTestCase):

    def _extract(self, raw):
        from odoo.addons.skoll.controllers.gog_auth import SkollGogAuthController
        return SkollGogAuthController._extract_client_secret(raw)
...
@tagged("post_install", "-at_install")
class TestGogExchangeToken(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestGogStatus(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestGogAuthErrorPaths(SkollTestCase):
    ...
    def _task_with_gog(self):
        task = self._create_task(
            task_id="GOG-ERR-%s" % (self.env["ir.sequence"].next_by_code("skoll.skoll") or "001"),
            email="user@example.com",
            password="keyring-pw",
            gog_auth=json.dumps({"installed": {"client_id": "abc"}}),
        )
...
@tagged("post_install", "-at_install")
class TestExtractClientSecretErrorPaths(SkollTestCase):
