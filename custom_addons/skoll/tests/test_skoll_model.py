# -*- coding: utf-8 -*-
"""Comprehensive tests for Skoll, SkollTurn, SkollTaxonomy models
and utility functions in models/skoll.py.
"""
import json
...
# ═══════════════════════════════════════════════════════════════════════
# 1. Skoll record creation
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestSkollCreation(SkollTestCase):
    """Tests for Skoll record creation, ensure_sandboxes, and field defaults."""
    ...
    def test_create_without_persona_raises(self):
        ...
        with self.assertRaises(Exception):
            self.Skoll.create({"task_id": "FAIL-001", "task_status": "NotSubmitted"})

    def test_batch_create(self):
        """model_create_multi handles batch creation properly."""
        records = self.Skoll.create([
...
@tagged("post_install", "-at_install")
class TestSkollComputedFields(SkollTestCase):
    """Tests for computed/related fields on the Skoll model."""
    ...
    def test_is_skoll_admin_not_admin(self):
        """is_skoll_admin is False for non-admin users."""
        ...
        with patch.object(type(self.env.user), "has_group", return_value=False):
            self.task.invalidate_recordset(["is_skoll_admin"])
            self.task._compute_is_skoll_admin()
            self.assertFalse(self.task.is_skoll_admin)

    def test_is_skoll_admin_admin(self):
        """is_skoll_admin is True for quality lead users."""
        with patch.object(type(self.env.user), "has_group", return_value=True):
            self.task.invalidate_recordset(["is_skoll_admin"])
            self.task._compute_is_skoll_admin()
            self.assertTrue(self.task.is_skoll_admin)
...
@tagged("post_install", "-at_install")
class TestSkollTrajectory(SkollTestCase):
    ...
    def test_build_trajectory_from_events(self):
        ...
        messages, counter, parent_id = self.Skoll._build_trajectory_from_events(
    ...
    def test_build_trajectory_from_events_trailing_text(self):
        ...
        messages, _, _ = self.Skoll._build_trajectory_from_events(
    ...
    def test_export_and_clear_turns(self):
        ...
        self.assertEqual(attachment.res_model, "skoll.skoll")
...
@tagged("post_install", "-at_install")
class TestSkollActions(SkollTestCase):
    """Tests for action methods on the Skoll model."""
    ...
    def test_action_view_turns(self):
        ...
        self.assertEqual(result["res_model"], "skoll.turn")
    ...
    def test_action_generate_golden_no_claude_trajectory(self):
        ...
        from odoo.addons.skoll.models.skoll import _GOLDEN_GENERATING, _GOLDEN_LOCK
...
@tagged("post_install", "-at_install")
class TestSkollAutoProcess(SkollTestCase):
    ...
    def test_claim_task_not_found(self):
        ...
        result = self.Skoll.auto_process_claim_task(999999)
    ...
    def test_claim_task_wrong_status(self):
        ...
        result = self.Skoll.auto_process_claim_task(self.task.id)
        self.assertTrue(result.get("skip"))
    ...
    def test_mark_done_success(self):
        ...
        result = self.Skoll.auto_process_mark_done(self.task.id, status="done")
        self.assertTrue(result)
        self.task.invalidate_recordset(["auto_process_status"])
    ...
    def test_mark_done_with_error(self):
        ...
        long_error = "x" * 3000
        self.Skoll.auto_process_mark_done(self.task.id, status="failed", error=long_error)
    ...
    def test_mark_done_nonexistent_task(self):
        ...
        result = self.Skoll.auto_process_mark_done(999999)
    ...
    def test_publish_auto_process_filters_ineligible(self):
        ...
        with patch(
            "odoo.addons.skoll.services.rabbitmq_service.batch_publish_auto_process_tasks"
        ) as mock_publish:
...
@tagged("post_install", "-at_install")
class TestSkollTurn(SkollTestCase):
    """Tests for SkollTurn model: defaults, ordering, computed, cascade."""
    ...
    def test_related_skoll_id(self):
        """skoll_id is related to sandbox_id.skoll_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.skoll_id, self.task)

    def test_related_employee_id(self):
        """employee_id is related through skoll_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.employee_id, self.task.employee_id)


# ═══════════════════════════════════════════════════════════════════════
# 7. SkollTaxonomy model
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestSkollTaxonomy(SkollTestCase):
    """Tests for SkollTaxonomy model."""
...
@tagged("post_install", "-at_install")
class TestSkollUtilities(SkollTestCase):
    """Tests for module-level utility functions in models/skoll.py."""

    def test_load_dotenv_returns_dict(self):
        """_load_dotenv returns a dict containing os.environ keys."""
        from odoo.addons.skoll.models.skoll import _load_dotenv
        result = _load_dotenv()
    ...
    def test_load_dotenv_parses_env_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(env_path, "w") as f:
                f.write("# comment\n")
                f.write("TEST_SKOLL_VAR=hello123\n")
                f.write("\n")
                f.write("ANOTHER_VAR=world\n")
                f.write("MALFORMED_LINE\n")

            with patch("odoo.addons.skoll.models.skoll.odoo_config") as mock_config:
                mock_config.rcfile = os.path.join(tmpdir, "odoo.conf")
                result = _load_dotenv()
                self.assertEqual(result.get("TEST_SKOLL_VAR"), "hello123")
    ...
    def test_is_degenerate_output_empty(self):
        ...
        from odoo.addons.skoll.models.skoll import _is_degenerate_output
    ...
    def test_wrap_trajectory_message_user_passthrough(self):
        ...
        from odoo.addons.skoll.models.skoll import _wrap_trajectory_message
    ...
    def test_wrap_messages_with_turn_feedback_empty_turns(self):
        ...
        from odoo.addons.skoll.models.skoll import _wrap_messages_with_turn_feedback
        messages = [
    ...
    def test_format_tool_result_none(self):
        ...
        from odoo.addons.skoll.models.skoll import _format_tool_result
    ...
    def test_generate_task_description_sync_missing_creds(self):
        ...
        from odoo.addons.skoll.models.skoll import generate_task_description_sync
        with patch("odoo.addons.skoll.models.skoll._load_dotenv", return_value={}):
    ...
    def test_generate_task_description_sync_degenerate_discarded(self):
        ...
        from odoo.addons.skoll.models.skoll import generate_task_description_sync
        self._set_param("skoll.bedrock_inference_arn", "arn:test")
        dotenv = {"AWS_BEARER_TOKEN_BEDROCK": "fake-key"}
        with patch("odoo.addons.skoll.models.skoll._load_dotenv", return_value=dotenv):
            with patch("odoo.addons.skoll.models.skoll._get_taskdesc_prompt", return_value="prompt"):
                with patch(
                    "odoo.addons.skoll.controllers.llm_assisst_qc._call_bedrock_converse",
...
# ═══════════════════════════════════════════════════════════════════════
# 9. _search_is_skoll_admin
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestAdminSearch(SkollTestCase):

    def test_search_admin_eq_true_admin(self):
        with patch.object(type(self.env.user), "has_group", return_value=True):
            domain = self.task._search_is_skoll_admin("=", True)
    ...
    def test_search_admin_unsupported_operator(self):
        with self.assertRaises(ValueError):
            self.task._search_is_skoll_admin(">", True)

    def test_search_admin_neq_false_admin(self):
        with patch.object(type(self.env.user), "has_group", return_value=True):
            domain = self.task._search_is_skoll_admin("!=", False)
            self.assertEqual(domain, [])
...
@tagged("post_install", "-at_install")
class TestGenerateActions(SkollTestCase):
    ...
    def test_generate_taskdesc_already_generating(self):
        from odoo.addons.skoll.models.skoll import _TASKDESC_GENERATING, _TASKDESC_LOCK
...
@tagged("post_install", "-at_install")
class TestDeleteTrajectoryEntry(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestAutoProcessExtended(SkollTestCase):
    ...
    def test_claim_task_success_returns_sandbox_info(self):
        ...
        result = self.Skoll.auto_process_claim_task(task.id)
        self.assertFalse(result.get("skip", False))
    ...
    def test_claim_task_already_claimed(self):
        ...
        r1 = self.Skoll.auto_process_claim_task(task.id)
        self.assertFalse(r1.get("skip", False))
        # Second claim: status is now 'processing', not 'queued'
        r2 = self.Skoll.auto_process_claim_task(task.id)
...
@tagged("post_install", "-at_install")
class TestGetAllTurns(SkollTestCase):
