# -*- coding: utf-8 -*-
"""Cross-cutting integration tests for the Skoll module.
...
from ..models.skoll_sandbox import TRAJECTORY_FIELD_MAP
...
class TestFullLifecycle(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestCascadingDeletes(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestTokenAggregation(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestPersonaPropagation(SkollTestCase):
    """Verify persona data flows through task → sandbox chain."""

    def test_persona_fields_available_for_sandbox(self):
        """Sandbox can reach persona fields through skoll_id.persona_id."""
        task = self._create_task(seed_prompt="Persona propagation test")
        sandbox = task.claude_sandbox_id

        persona = sandbox.skoll_id.persona_id
        self.assertTrue(persona, "Sandbox should reach persona via skoll_id")
