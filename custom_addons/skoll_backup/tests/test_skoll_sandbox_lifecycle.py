...
class TestSandboxStartLocal(SkollTestCase):

    def test_action_start_sets_starting_status(self):
        sandbox = self.claude_sandbox
        with patch("odoo.addons.skoll.models.skoll_sandbox._docker_available", return_value=True), \
             patch("odoo.addons.skoll.models.skoll_sandbox._compose_cmd", return_value=["docker", "compose"]), \
             patch("odoo.addons.skoll.models.skoll_sandbox._SANDBOX_POOL") as mock_pool:
    ...
    def test_action_start_allocates_ports(self):
        ...
        with patch("odoo.addons.skoll.models.skoll_sandbox._docker_available", return_value=True), \
             patch("odoo.addons.skoll.models.skoll_sandbox._compose_cmd", return_value=["docker", "compose"]), \
             patch("odoo.addons.skoll.models.skoll_sandbox._SANDBOX_POOL"):
            sandbox.action_start_sandbox()
    ...
    def test_action_start_docker_unavailable_raises(self):
        ...
        sandbox = self.claude_sandbox
        with patch("odoo.addons.skoll.models.skoll_sandbox._docker_available", return_value=False):
            with self.assertRaises(UserError):
                sandbox.action_start_sandbox()
    ...
    def test_action_start_compose_missing_raises(self):
        ...
        sandbox = self.claude_sandbox
        with patch("odoo.addons.skoll.models.skoll_sandbox._docker_available", return_value=True), \
             patch("odoo.addons.skoll.models.skoll_sandbox._compose_cmd", return_value=None):
            with self.assertRaises(UserError):
                sandbox.action_start_sandbox()

    def test_action_start_no_persona_raises(self):
        task_no_persona = self.Skoll.with_context(skip_ensure_sandboxes=True).create({
            "persona_id": self.persona.id,
            "task_status": "NotSubmitted",
        })
        task_no_persona.write({"persona_id": False})
        sandbox = self.Sandbox.search([("skoll_id", "=", task_no_persona.id)], limit=1)
        if sandbox:
            self._set_param("skoll.deployment_mode", "local")
            with patch("odoo.addons.skoll.models.skoll_sandbox._docker_available", return_value=True), \
                 patch("odoo.addons.skoll.models.skoll_sandbox._compose_cmd", return_value=["docker", "compose"]):
...
@tagged("post_install", "-at_install")
class TestSandboxStartK8s(SkollTestCase):
    ...
    def test_start_k8s_bg_calls_deploy(self):
        ...
        with patch("odoo.addons.skoll.models.skoll_sandbox_k8s.K8S_AVAILABLE", True), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.config"), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.client") as mock_k8s_client:
            mock_core = MagicMock()
            mock_apps = MagicMock()
            ...
            with patch.object(type(sandbox.env["skoll.sandbox.k8s"]), "get_sandbox_status", return_value="running"):
    ...
    def test_start_k8s_bg_deploy_failure_sets_error(self):
        ...
        with patch("odoo.addons.skoll.models.skoll_sandbox_k8s.K8S_AVAILABLE", True), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.config"), \
             patch.object(type(sandbox.env["skoll.sandbox.k8s"]), "deploy_sandbox", side_effect=Exception("K8s API error")):
            sandbox._start_k8s_bg()
...
@tagged("post_install", "-at_install")
class TestSandboxStop(SkollTestCase):
    ...
    def test_stop_local_calls_compose_down(self):
        ...
        sandbox.write({
            ...
            "docker_compose_project": "skoll-test-claude",
            "docker_workdir": "/tmp/skoll-test",
            "docker_port": 19001,
            "docker_litellm_port": 14001,
            "docker_gateway_token": "test-token",
        })
        self._set_param("skoll.deployment_mode", "local")

        with patch("odoo.addons.skoll.models.skoll_sandbox._compose_cmd", return_value=["docker", "compose"]), \
    ...
    def test_stop_k8s_calls_destroy(self):
        ...
        sandbox.write({
            "docker_status": "running",
            "docker_compose_project": "skoll-sandbox-test",
            "docker_port": 18789,
            "docker_gateway_token": "test-token",
        })
        ...
        with patch("odoo.addons.skoll.models.skoll_sandbox_k8s.K8S_AVAILABLE", True), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.config"), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.client") as mock_client:
            mock_core = MagicMock()
            mock_apps = MagicMock()
...
@tagged("post_install", "-at_install")
class TestSandboxReconcile(SkollTestCase):
    ...
    def test_cron_reconcile_updates_k8s_status(self):
        ...
        with patch("odoo.addons.skoll.models.skoll_sandbox_k8s.K8S_AVAILABLE", True), \
             patch("odoo.addons.skoll.models.skoll_sandbox_k8s.config"), \
             patch.object(type(sandbox.env["skoll.sandbox.k8s"]), "get_sandbox_status", return_value="running"):
            self.Sandbox._cron_reconcile()
