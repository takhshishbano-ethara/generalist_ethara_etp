...
from ..controllers.chat import SkollChatController


# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/create_turn
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestChatCreateTurn(SkollTestCase):
...
# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/save_response
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestChatSaveResponse(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestChatSaveQC(SkollTestCase):
...
# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/save_feedback
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestChatSaveFeedback(SkollTestCase):
...
# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/save_trajectory
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestChatSaveTrajectory(SkollTestCase):
    ...
    def test_save_trajectory_extracts_tool_calls(self):
        ...
        extracted = SkollChatController._extract_tool_calls_from_trajectory(traj_json)
    ...
    def test_save_trajectory_extracts_tokens(self):
        ...
        token_usage = SkollChatController._extract_token_usage_from_trajectory(traj_json)
    ...
    def test_save_trajectory_keeps_better_calls(self):
        ...
        extracted_fewer = SkollChatController._extract_tool_calls_from_trajectory(traj_fewer)
        ...
        extracted_more = SkollChatController._extract_tool_calls_from_trajectory(traj_more)
        existing_list = json.loads(turn.tool_calls)
    ...
    def test_save_trajectory_empty(self):
        ...
        result = SkollChatController._extract_tool_calls_from_trajectory("")
        self.assertEqual(result, [])

        result2 = SkollChatController._extract_tool_calls_from_trajectory("[]")
        self.assertEqual(result2, [])

        result3 = SkollChatController._extract_tool_calls_from_trajectory(None)
        self.assertEqual(result3, [])

        token_result = SkollChatController._extract_token_usage_from_trajectory("")
        self.assertEqual(token_result, {"input_tokens": 0, "output_tokens": 0})

        token_result2 = SkollChatController._extract_token_usage_from_trajectory(None)
...
@tagged("post_install", "-at_install")
class TestChatHistory(SkollTestCase):
    ...
    def test_history_backfills_tool_calls(self):
        ...
        if not tool_calls_str and turn.trajectory_messages:
            extracted = SkollChatController._extract_tool_calls_from_trajectory(
...
# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/export_session
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestExportSession(SkollTestCase):
...
# ═══════════════════════════════════════════════════════════════════════
#  /skoll/chat/sandbox_state
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestSandboxState(SkollTestCase):
...
@tagged("post_install", "-at_install")
class TestChatControllerErrorPaths(SkollTestCase):
    """Error-path tests that exercise the controller methods directly."""

    def _ctrl(self):
        return SkollChatController()
