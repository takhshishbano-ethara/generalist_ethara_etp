...
class SkollBrowserAuthController(http.Controller):
    ...
    def _browser_api_base(sandbox):
        ...
        if mode == "k8s":
            ...
            svc_name = "skoll-sandbox-%s" % sandbox.id
            return (
                "http://%s.skoll.svc.cluster.local:18789/browser-api"
                % svc_name
            )
    ...
    @http.route("/skoll/browser/screenshot", type="json", auth="user")
    ...
    @http.route("/skoll/browser/inject_cookies", type="json", auth="user")
    ...
    @http.route("/skoll/browser/status", type="json", auth="user")
