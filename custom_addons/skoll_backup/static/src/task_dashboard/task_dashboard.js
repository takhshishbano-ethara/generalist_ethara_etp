...
export class TaskDashboard extends Component {
    static template = "skoll.TaskDashboard";
        ...
        if (needsReconcile.length > 0) {
            try {
                ...
                const statusMap = await this.orm.call(
                    "skoll.sandbox", "action_check_status", [ids]
                );
                ...
                console.warn("[skoll-dashboard] Status reconciliation failed:", e);
            }
        }
    ...
    async _pollStatus() {
        ...
        try {
            ...
            console.warn("[skoll-dashboard] Poll status failed:", e);
        }
    ...
    async onStartSandbox(sandboxId) {
        ...
        try {
            await this.orm.call("skoll.sandbox", "action_start_sandbox", [[sandboxId]]);
            await this._loadSandboxes();
        } catch (e) {
    ...
    async onStopSandbox(sandboxId) {
        ...
        try {
            ...
            await this.orm.call("skoll.sandbox", "action_stop_sandbox", [[sandboxId]]);
            await this._loadSandboxes();
            await this.props.record.load();
            await this._autoTriggerTaskDescription(modelType);
        } catch (e) {
    ...
    async _autoTriggerTaskDescription(modelType) {
        ...
        try {
            await rpc("/skoll/generate_task_description", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: entryIndex,
            });
            this.env.bus.trigger("SKOLL:TASK_DESC_TRIGGERED", {
                field_name: fieldName,
                entry_index: entryIndex,
            });
            this._pollDescriptionThenTriggerQc(recordId, fieldName, entryIndex);
        } catch (e) {
            console.warn("[skoll-dashboard] Auto task description trigger failed:", e);
        }
    }
    ...
    _pollDescriptionThenTriggerQc(recordId, fieldName, entryIndex) {
        const poll = setInterval(async () => {
            try {
                ...
                await rpc("/skoll/trajectory_qc", {
                    record_id: recordId,
                    field_name: fieldName,
                    entry_index: entryIndex,
                });
                this.env.bus.trigger("SKOLL:QC_TRIGGERED", {
                    field_name: fieldName,
                    entry_index: entryIndex,
                });
            } catch (e) {
                clearInterval(poll);
                console.warn("[skoll-dashboard] Auto QC trigger failed:", e);
            }
        }, 5000);
    }
    ...
    async _checkGogAuthStatus() {
        if (!this.taskId) return;
        try {
            const data = await rpc("/skoll/gog/status", { task_id: this.taskId });
            this.state.gogAuthDone = !!data.authenticated;
        } catch (e) {
            console.warn("[skoll-dashboard] Failed to check gog auth status:", e);
        }
    }
...
registry.category("view_widgets").add("skoll_task_dashboard", taskDashboardDef);
