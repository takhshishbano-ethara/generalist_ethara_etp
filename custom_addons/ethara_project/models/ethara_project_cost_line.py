...
class EtharaProjectCostLine(models.Model):
    _name = 'ethara.project.cost.line'
    _description = 'Ethara Project Cost Line'
    ...
    source = fields.Selection(
        selection=SOURCE_SELECTION, default='aws', required=True, index=True,
    )
    granularity = fields.Selection(
        selection=GRANULARITY_SELECTION, default='month', required=True, index=True,
    )
    ...
    token_type = fields.Selection(selection=TOKEN_TYPE_SELECTION, index=True)
    ...
    def _ensure_ai_models_from_lines(self):
        AiModel = self.env['ethara.project.ai.model'].sudo().with_context(active_test=False)
        Infra = self.env['ethara.project.infra.type'].sudo().with_context(active_test=False)
        ...
        _logger.info(
            'ethara.project.cost.line catalog sync: lines=%s '
