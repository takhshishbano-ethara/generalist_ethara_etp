...
For each talos_talos row:
  ...
  3. UPDATE talos_turn.sandbox_id to point at the claude sandbox.
"""
...
def migrate(cr, version):
    ...
    # Guard: only run if source columns still exist on talos_talos
    if not _column_exists(cr, "talos_talos", "docker_compose_project"):
        _logger.info(
    ...
    # Guard: talos_sandbox table must already exist (created by ORM before
    ...
    cr.execute(
        ...
        WHERE table_name = 'talos_sandbox'
        LIMIT 1
        """
    )
    if not cr.fetchone():
        _logger.warning(
            "pre-migrate 19.0.6.0.0: talos_sandbox table does not exist yet "
            "— skipping (will be handled by post-migrate or manual step)."
        )
        return

    _logger.info(
        "pre-migrate 19.0.6.0.0: creating sandbox rows from talos_talos docker fields"
    )

    # ── 1. Insert 'claude' sandbox per task ─────────────────────────────
    cr.execute(
        """
        INSERT INTO talos_sandbox (
            talos_id,
            model_type,
        ...
        SELECT
            ...
            CASE
                WHEN EXISTS (
                    SELECT 1 FROM talos_turn turn WHERE turn.talos_id = t.id
                ) THEN 'in_progress'
                ELSE 'not_started'
            END,
            t.create_uid,
        ...
        FROM talos_talos t
        WHERE NOT EXISTS (
            SELECT 1 FROM talos_sandbox s
            WHERE s.talos_id = t.id AND s.model_type = 'claude'
        )
        """
    )
    ...
    for model_type in ("glm", "1p"):
        cr.execute(
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM talos_sandbox s
                WHERE s.talos_id = t.id AND s.model_type = %s
            )
            """,
    ...
    if _column_exists(cr, "talos_turn", "sandbox_id"):
        cr.execute(
            """
            UPDATE talos_turn
            SET sandbox_id = (
                SELECT s.id
                FROM talos_sandbox s
                WHERE s.talos_id = talos_turn.talos_id
        ...
        _logger.info(
            "pre-migrate 19.0.6.0.0: sandbox_id column not yet on talos_turn "
