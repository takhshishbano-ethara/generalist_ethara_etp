# Talos K8s Sandbox — Full Workflow Documentation
...
### talos.talos (Task Record)

| Field                    | Type      | Description                                     |
|--------------------------|-----------|-------------------------------------------------|
| `persona_id`             | Many2one  | Link to `talos.persona`. Required.               |
| `docker_status`          | Selection | `stopped` / `starting` / `running` / `error`    |
| `docker_compose_project` | Char      | K8s: service name (`talos-sandbox-{id}`)         |
...
User clicks "Start Sandbox" button in Odoo UI
    │
    ▼
talos.talos.action_start_sandbox()
    │
    ▼
_deployment_mode() == "k8s"  (from ir.config_parameter talos.deployment_mode)
    │
    ▼
talos.talos._start_k8s()
```
...
**File:** `talos.py` → `_start_k8s()` (line 237)
...
**File:** `talos.py` → `_start_k8s()` (line 247)
...
**File:** `talos_sandbox_k8s.py` → `deploy_sandbox()` (lines 132-167)
...
persona = task_record.persona_id      # talos.persona record from DB
env = _load_dotenv()                  # Credentials from .env + os.environ

# Image URIs from Odoo settings (ir.config_parameter)
openclaw_image = "talos.openclaw_image"   # default: ghcr.io/openclaw/openclaw:latest
litellm_image  = "talos.litellm_image"    # default: ghcr.io/berriai/litellm:main-stable
storage_class  = "talos.k8s_storage_class" # default: gp3
```

#### Step 4: Create K8s Secret

**File:** `talos_sandbox_k8s.py` → `_create_secret()` (lines 240-269)

**Resource:** `Secret/talos-sandbox-creds-{task_id}`

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: talos-sandbox-creds-{task_id}
  namespace: ethara
  labels:
    platform: talos
    component: sandbox
...
**File:** `talos_sandbox_k8s.py` → `_create_persona_configmap()` (lines 271-295)

**Source:** `talos.persona` database fields (NOT filesystem)

**Resource:** `ConfigMap/talos-sandbox-persona-{task_id}`

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: talos-sandbox-persona-{task_id}
  namespace: ethara
data:
  SOUL.md: |
    {persona.soul_md}
...
**File:** `talos_sandbox_k8s.py` → `_create_openclaw_config_configmap()` (lines 297-315)

**Source:** Generated by `_build_openclaw_config()` (lines 42-122)

**Resource:** `ConfigMap/talos-sandbox-openclaw-config-{task_id}`
...
kind: ConfigMap
metadata:
  name: talos-sandbox-openclaw-config-{task_id}
  namespace: ethara
data:
  openclaw.json: |
    {
...
- `talos-bedrock` provider only added if AWS credentials exist in `.env`
- LiteLLM baseUrl is `http://localhost:4000/v1` (sidecar in same pod)

#### Step 7: Create LiteLLM Config ConfigMap

**File:** `talos_sandbox_k8s.py` → `_create_litellm_configmap()` (lines 317-333)

**Source:** `persona.litellm_config_yaml` if set, otherwise `_DEFAULT_LITELLM_CONFIG` template

**Resource:** `ConfigMap/talos-litellm-config-{task_id}`

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: talos-litellm-config-{task_id}
  namespace: ethara
data:
  config.yaml: |
    model_list:
...
**File:** `talos_sandbox_k8s.py` → `_create_pvc()` (lines 335-359)

Two PVCs per sandbox:

| PVC Name | Size | Purpose |
|----------|------|---------|
| `talos-browser-{persona_name}` | 5Gi | Shared browser profiles per persona |
| `talos-sandbox-db-{task_id}` | 2Gi | PostgreSQL data for LiteLLM |
...
**File:** `talos_sandbox_k8s.py` → `_create_deployment()` (lines 361-665)

**Resource:** `Deployment/talos-sandbox-{task_id}` with 3 containers in one pod.

See [Container Specifications](#container-specifications) below for full details.

#### Step 10: Create Service

**File:** `talos_sandbox_k8s.py` → `_create_service()` (lines 667-695)

**Resource:** `Service/talos-sandbox-{task_id}`

```yaml
apiVersion: v1
kind: Service
metadata:
  name: talos-sandbox-{task_id}
  namespace: ethara
spec:
  type: ClusterIP
  selector:
...
**File:** `talos.py` → `_start_k8s()` (lines 248-254)

```python
self.write({
    "docker_compose_project": "talos-sandbox-{id}",  # K8s service name
    "docker_status": "starting",
    "docker_port": 18789,
})
```
...
|---------------|------|-----------|
| **Secret** | `talos-sandbox-creds-42` | Per-task. Deleted on destroy. |
| **ConfigMap** | `talos-sandbox-persona-42` | Per-task. Deleted on destroy. |
| **ConfigMap** | `talos-sandbox-openclaw-config-42` | Per-task. Deleted on destroy. |
| **ConfigMap** | `talos-litellm-config-42` | Per-task. Deleted on destroy. |
| **PVC** | `talos-browser-marcus` | Per-persona. **NOT** deleted on destroy (shared). Labels: `platform: talos`, `component: browser-profiles`, `persona: {name}` |
| **PVC** | `talos-sandbox-db-42` | Per-task. Deleted on destroy. |
| **Deployment** | `talos-sandbox-42` | Per-task. Deleted on destroy. |
| **Service** | `talos-sandbox-42` | Per-task. Deleted on destroy. |
...
| Property | Value |
|----------|-------|
| **Image** | `talos.openclaw_image` setting (default: `ghcr.io/openclaw/openclaw:latest`) |
...
_load_dotenv()
    ...
    ├─ BEDROCK_MODEL_ARN ────────┤    if both set → talos-bedrock provider
    ├─ AWS_REGION ───────────────┘
    │
...
User clicks "Stop Sandbox"
    │
    ▼
talos.talos.action_stop_sandbox()
    │
    ▼
_deployment_mode() == "k8s"
    │
    ▼
talos.talos._stop_k8s()
    │
    ▼
talos.sandbox.k8s.destroy_sandbox(task_record)
```

### Resources Deleted (in order)

**File:** `talos_sandbox_k8s.py` → `destroy_sandbox()` (lines 697-742)

1. `Deployment/talos-sandbox-{id}`
2. `Service/talos-sandbox-{id}`
3. `Secret/talos-sandbox-creds-{id}`
4. `ConfigMap/talos-sandbox-persona-{id}`
5. `ConfigMap/talos-sandbox-openclaw-config-{id}`
6. `ConfigMap/talos-litellm-config-{id}`
7. `PVC/talos-sandbox-db-{id}`

**NOT deleted:** `PVC/talos-browser-{persona_name}` (shared across tasks with same persona).
...
**File:** `talos.py` → `_cron_reconcile_sandboxes()` (line 218)
...
**File:** `talos.py` → `_compute_dashboard_url()` (line 180)
...
Example: `http://talos-sandbox-42.ethara.svc.cluster.local:18789/#token=abc123...`
...
| **Persona source** | `talos.persona` DB fields | `talos.persona` DB fields |
...
│   ├── talos.py                       # Main model + _load_dotenv + local mode
│   ├── talos_domain.py                # Legacy domain model
│   ├── talos_persona.py               # Persona model (DB-driven)
│   └── talos_sandbox_k8s.py           # K8s deployer
...
|----------|---------|---------|
| `AWS_BEARER_TOKEN_BEDROCK` | `ABSKQm...` | openclaw.json (talos-bedrock provider), K8s Secret |
| `AWS_REGION` | `ap-south-1` | openclaw.json, litellm-config, K8s container env |
| `BEDROCK_MODEL_ARN` | `arn:aws:bedrock:...` | openclaw.json (talos-bedrock provider), litellm-config template |

### Optional in `.env` (auto-generated if missing)

| Variable | Default | Used By |
|----------|---------|---------|
| `LITELLM_MASTER_KEY` | `sk-talos-{random}` | openclaw.json (litellm provider apiKey), K8s Secret |
...
|-----|---------|-------------|
| `talos.deployment_mode` | `local` | `"k8s"` to enable K8s mode |
| `talos.openclaw_image` | `ghcr.io/openclaw/openclaw:latest` | OpenClaw container image URI |
| `talos.litellm_image` | `ghcr.io/berriai/litellm:main-stable` | LiteLLM container image URI |
| `talos.k8s_storage_class` | `gp3` | StorageClassName for PVCs |

These are configured in **Settings → Talos → Kubernetes Mode**.
