# -*- coding: utf-8 -*-
"""Shared test fixtures for the Talos test suite.

All Talos test classes should inherit from ``TalosTestCase`` to get
...
class TalosTestCase(TransactionCase):
    """Base test case with shared fixtures for all Talos tests."""
    ...
    def setUpClass(cls):
        ...
        cls.Sandbox = cls.env["talos.sandbox"]
        cls.Turn = cls.env["talos.turn"]
        cls.Persona = cls.env["talos.persona"]
        cls.Domain = cls.env["talos.domain"]
        cls.Taxonomy = cls.env["talos.taxonomy"]
    ...
    def _create_task(cls, **overrides):
        """Create a talos.talos record with sensible defaults."""
        vals = {
            "persona_id": cls.persona.id,
            "task_status": "NotSubmitted",
        }
    ...
    def _make_session_entry(trajectory_json_str=None, session_id="test-session-001"):
        ...
        if trajectory_json_str is None:
            traj = json.loads(TalosTestCase._make_trajectory_json())
        else:
