# -*- coding: utf-8 -*-
"""Comprehensive tests for Talos, TalosTurn, TalosTaxonomy models
and utility functions in models/talos.py.
"""
import json
...
# ═══════════════════════════════════════════════════════════════════════
# 1. Talos record creation
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestTalosCreation(TalosTestCase):
    """Tests for Talos record creation, ensure_sandboxes, and field defaults."""
...
@tagged("post_install", "-at_install")
class TestTalosComputedFields(TalosTestCase):
    """Tests for computed/related fields on the Talos model."""
    ...
    def test_is_talos_admin_not_admin(self):
        """is_talos_admin is False for non-admin users."""
        ...
        with patch.object(type(self.env.user), "has_group", return_value=False):
            self.task.invalidate_recordset(["is_talos_admin"])
            self.task._compute_is_talos_admin()
            self.assertFalse(self.task.is_talos_admin)

    def test_is_talos_admin_admin(self):
        """is_talos_admin is True for quality lead users."""
        with patch.object(type(self.env.user), "has_group", return_value=True):
            self.task.invalidate_recordset(["is_talos_admin"])
            self.task._compute_is_talos_admin()
            self.assertTrue(self.task.is_talos_admin)
...
@tagged("post_install", "-at_install")
class TestTalosTrajectory(TalosTestCase):
    ...
    def test_export_and_clear_turns(self):
        ...
        self.assertEqual(attachment.res_model, "talos.talos")
...
@tagged("post_install", "-at_install")
class TestTalosActions(TalosTestCase):
    """Tests for action methods on the Talos model."""
    ...
    def test_action_view_turns(self):
        ...
        self.assertEqual(result["res_model"], "talos.turn")
...
@tagged("post_install", "-at_install")
class TestTalosAutoProcess(TalosTestCase):
...
@tagged("post_install", "-at_install")
class TestTalosTurn(TalosTestCase):
    """Tests for TalosTurn model: defaults, ordering, computed, cascade."""
    ...
    def test_related_talos_id(self):
        """talos_id is related to sandbox_id.talos_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.talos_id, self.task)

    def test_related_employee_id(self):
        """employee_id is related through talos_id."""
        turn = self._create_turn(sandbox=self.claude_sandbox, turn_number=1)
        self.assertEqual(turn.employee_id, self.task.employee_id)


# ═══════════════════════════════════════════════════════════════════════
# 7. TalosTaxonomy model
# ═══════════════════════════════════════════════════════════════════════

@tagged("post_install", "-at_install")
class TestTalosTaxonomy(TalosTestCase):
    """Tests for TalosTaxonomy model."""
...
@tagged("post_install", "-at_install")
class TestTalosUtilities(TalosTestCase):
    """Tests for module-level utility functions in models/talos.py."""

    def test_load_dotenv_returns_dict(self):
        """_load_dotenv returns a dict containing os.environ keys."""
        from odoo.addons.talos.models.talos import _load_dotenv
        result = _load_dotenv()
    ...
    def test_load_dotenv_parses_env_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(env_path, "w") as f:
                f.write("# comment\n")
                f.write("TEST_TALOS_VAR=hello123\n")
                f.write("\n")
                f.write("ANOTHER_VAR=world\n")
                f.write("MALFORMED_LINE\n")

            with patch("odoo.addons.talos.models.talos.odoo_config") as mock_config:
                mock_config.rcfile = os.path.join(tmpdir, "odoo.conf")
                result = _load_dotenv()
                self.assertEqual(result.get("TEST_TALOS_VAR"), "hello123")
    ...
    def test_is_degenerate_output_empty(self):
        ...
        from odoo.addons.talos.models.talos import _is_degenerate_output
    ...
    def test_wrap_trajectory_message_user_passthrough(self):
        ...
        from odoo.addons.talos.models.talos import _wrap_trajectory_message
    ...
    def test_wrap_messages_with_turn_feedback_empty_turns(self):
        ...
        from odoo.addons.talos.models.talos import _wrap_messages_with_turn_feedback
        messages = [
    ...
    def test_format_tool_result_none(self):
        ...
        from odoo.addons.talos.models.talos import _format_tool_result
    ...
    def test_generate_task_description_sync_missing_creds(self):
        ...
        from odoo.addons.talos.models.talos import generate_task_description_sync
        with patch("odoo.addons.talos.models.talos._load_dotenv", return_value={}):
    ...
    def test_generate_task_description_sync_degenerate_discarded(self):
        ...
        with patch("odoo.addons.talos.models.talos._load_dotenv", return_value=dotenv):
            with patch("odoo.addons.talos.models.talos._get_taskdesc_prompt", return_value="prompt"):
                with patch(
                    "odoo.addons.talos.controllers.llm_assisst_qc._call_bedrock_converse",
