...
_MOD_TALOS = "odoo.addons.talos.models.talos"
_MOD_SANDBOX = "odoo.addons.talos.models.talos_sandbox"
...
def _build_env(task_exists=True, task_vals=None, sandbox_exists=True, sandbox_vals=None):
    ...
    def _icp_get(key):
        mapping = {
            "talos.bedrock_inference_arn": "arn:aws:bedrock:us-east-1:123:inference/abc",
            "talos.bedrock_region": "us-east-1",
        }
    ...
    def _env_getitem(model_name):
        if model_name == "talos.talos":
            talos_model = MagicMock()
            talos_model.browse.return_value = task_mock
            return talos_model
        if model_name == "talos.sandbox":
            sb_model = MagicMock()
...
class TestRunGoldenGenerationBackground(TalosTestCase):

    def _call_target(self, db_name="testdb", task_id=1, partner_id=10):
        from odoo.addons.talos.models.talos import _run_golden_generation_background
        _run_golden_generation_background(db_name, task_id, partner_id)

    @patch(_MOD_TALOS + "._get_golden_prompt", return_value="golden system prompt")
    @patch(_MOD_TALOS + "._load_dotenv", return_value={"AWS_BEARER_TOKEN_BEDROCK": "key123"})
    @patch(_MOD_TALOS + ".get_module_path", return_value="/fake/talos")
    @patch(_MOD_TALOS + ".os.path.isfile", return_value=False)
    def test_golden_gen_success(self, _isfile, _modpath, _dotenv, _prompt):
        ...
        patches = _patch_registry_and_env(_MOD_TALOS, env)
        bedrock_patch = patch(
            _MOD_TALOS + "._call_bedrock_converse",
            return_value=("golden output text", {"input_tokens": 200, "output_tokens": 300}),
        )
        lock_patch = patch(_MOD_TALOS + "._GOLDEN_GENERATING", new=set())
    ...
    @patch(_MOD_TALOS + "._load_dotenv", return_value={"AWS_BEARER_TOKEN_BEDROCK": ""})
    def test_golden_gen_no_api_key(self, _dotenv):
        env, task_mock, _, _, _ = _build_env()
        patches = _patch_registry_and_env(_MOD_TALOS, env)
        lock_set = set()
        lock_set.add(1)
        lock_patch = patch(_MOD_TALOS + "._GOLDEN_GENERATING", new=lock_set)
        with patches["Registry"], patches["api_env"], lock_patch:
    ...
    @patch(_MOD_TALOS + "._get_golden_prompt", return_value="prompt")
    @patch(_MOD_TALOS + "._load_dotenv", return_value={"AWS_BEARER_TOKEN_BEDROCK": "key123"})
    @patch(_MOD_TALOS + ".get_module_path", return_value="/fake/talos")
    @patch(_MOD_TALOS + ".os.path.isfile", return_value=False)
    ...
    def test_golden_gen_sends_bus_notification(self, _isfile, _modpath, _dotenv, _prompt):
        ...
        self.assertEqual(send_call[0][1], "talos/golden_ready")
...
@tagged("post_install", "-at_install")
class TestRunTaskDescriptionBackground(TalosTestCase):

    def _call_target(self, db_name="testdb", task_id=1, partner_id=10):
        from odoo.addons.talos.models.talos import _run_task_description_background
        _run_task_description_background(db_name, task_id, partner_id)

    @patch(_MOD_TALOS + "._get_taskdesc_prompt", return_value="taskdesc system prompt")
    @patch(_MOD_TALOS + "._load_dotenv", return_value={"AWS_BEARER_TOKEN_BEDROCK": "key123"})
    def test_taskdesc_bg_success(self, _dotenv, _prompt):
        ...
        patches = _patch_registry_and_env(_MOD_TALOS, env)
        lock_patch = patch(_MOD_TALOS + "._TASKDESC_GENERATING", new=set())
        with patches["Registry"], patches["api_env"], lock_patch:
            with patch(
                "odoo.addons.talos.controllers.llm_assisst_qc._call_bedrock_converse",
    ...
    @patch(_MOD_TALOS + "._get_taskdesc_prompt", return_value="prompt")
    @patch(_MOD_TALOS + "._load_dotenv", return_value={"AWS_BEARER_TOKEN_BEDROCK": "key123"})
    ...
    def test_taskdesc_bg_cleans_up_lock(self, _dotenv):
        ...
        lock_set = set()
        lock_set.add(7)
        lock_patch = patch(_MOD_TALOS + "._TASKDESC_GENERATING", new=lock_set)
        with patches["Registry"], patches["api_env"], lock_patch:
    ...
    def test_taskdesc_bg_sends_bus_notification(self, _dotenv, _prompt):
        ...
        bus_mock._sendone.assert_called()
        send_call = bus_mock._sendone.call_args
        self.assertEqual(send_call[0][1], "talos/taskdesc_ready")
        self.assertEqual(send_call[0][2]["status"], "done")
...
@tagged("post_install", "-at_install")
class TestRunSandboxStartBackground(TalosTestCase):

    def _call_target(self, db_name="testdb", sandbox_id=1, mode="local", partner_id=10):
        from odoo.addons.talos.models.talos_sandbox import _run_sandbox_start_background
    ...
    def test_sandbox_start_bg_sends_notification(self):
        ...
        bus_mock._sendone.assert_called()
        send_call = bus_mock._sendone.call_args
        self.assertEqual(send_call[0][1], "talos/sandbox_ready")
...
@tagged("post_install", "-at_install")
class TestInjectTaskDescriptionBg(TalosTestCase):

    def _call_target(self, db_name="testdb", task_id=1, field_name="claude_trajectory",
                     seed_prompt="seed", messages=None, entry_index=-1):
        from odoo.addons.talos.models.talos_sandbox import _inject_task_description_bg
