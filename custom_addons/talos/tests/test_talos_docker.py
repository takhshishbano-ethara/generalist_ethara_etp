...
class TestTalosSandboxLocal(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.Talos = cls.env["talos.talos"]
        cls.Domain = cls.env["talos.domain"]
    ...
    def test_default_persona(self):
        task = self.Talos.create({"task_id": "TEST-002", "task_status": "NotSubmitted"})
    ...
    def test_start_sandbox_local_raises_when_docker_unavailable(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "talos.deployment_mode", "local"
        )
        with patch(
            "odoo.addons.talos.models.talos._docker_available", return_value=False
        ):
    ...
    def test_start_sandbox_local_raises_without_sandbox_dir(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("talos.sandbox_dir", "")
        with patch(
            "odoo.addons.talos.models.talos._docker_available", return_value=True
        ):
            with patch(
                "odoo.addons.talos.models.talos._compose_cmd",
...
@tagged("post_install", "-at_install")
class TestTalosSandboxModeDispatch(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.Talos = cls.env["talos.talos"]
    ...
    def test_k8s_mode_dispatches_to_k8s(self):
        self.env["ir.config_parameter"].sudo().set_param("talos.deployment_mode", "k8s")
...
@tagged("post_install", "-at_install")
class TestTalosSandboxK8s(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.Talos = cls.env["talos.talos"]
