...
class TestCostingData(TalosTestCase):
    ...
    def _call_costing(self, period="week"):
        from ..controllers.costing import TalosCostingController
        controller = TalosCostingController()
        with patch.object(type(controller), "__init__", lambda s: None):
            controller = TalosCostingController.__new__(TalosCostingController)
    ...
    def test_costing_week_returns_rows_and_totals(self):
        ...
        with patch("odoo.addons.talos.controllers.costing.request") as mock_request:
    ...
    def test_costing_empty_no_tasks(self):
        ...
        with patch("odoo.addons.talos.controllers.costing.request") as mock_request:
            mock_request.env = self.env
            mock_request.env["talos.talos"].sudo = lambda: self.env["talos.talos"].browse([])
            result = controller.costing_data(period="all")
