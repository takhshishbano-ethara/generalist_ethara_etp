...
class TestCheckLocalStatus(TalosTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.sandbox.write({
            "docker_compose_project": "talos-test-proj",
    ...
    def _mock_compose_ps(self, stdout="", returncode=0, side_effect=None):
        if side_effect:
            return patch(
                "odoo.addons.talos.models.talos_sandbox.subprocess.run",
    ...
    def test_skip_when_sandbox_starting(self):
        from odoo.addons.talos.models.talos_sandbox import (
            _SANDBOX_LOCK,
            _SANDBOX_STARTING,
        )
    ...
    def test_container_exited(self):
        ...
        with self._mock_compose_ps(stdout=ps_json):
            with patch(
                "odoo.addons.talos.models.talos_sandbox._compose_cmd",
...
@tagged("post_install", "-at_install")
class TestWaitForHealth(TalosTestCase):

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.sandbox = cls.claude_sandbox
        cls.sandbox.write({"docker_port": 19999})

    def _compose_bin(self):
        return ["docker", "compose"]

    @patch("odoo.addons.talos.models.talos_sandbox.time.sleep")
    @patch("odoo.addons.talos.models.talos_sandbox.time.monotonic")
    def test_success(self, mock_mono, mock_sleep):
        ...
        with patch(
            ...
            with patch(
                "odoo.addons.talos.models.talos_sandbox.urllib.request.urlopen"
            ) as mock_urlopen:
...
@tagged("post_install", "-at_install")
class TestBuildComposeEnv(TalosTestCase):
    ...
    def _patch_dotenv(self, env_dict=None):
        return patch(
            "odoo.addons.talos.models.talos_sandbox._load_dotenv",
    ...
    def test_generates_litellm_key(self):
        with self._patch_dotenv({}):
            env = self.sandbox._build_compose_env("tok-123")
        self.assertTrue(env["LITELLM_MASTER_KEY"].startswith("sk-talos-"))
...
@tagged("post_install", "-at_install")
class TestPrepareWorkdir(TalosTestCase):
    ...
    def _base_patches(self, source_dir="/fake/sandbox"):
        ...
        self._mock_module_dir = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox._module_sandbox_dir",
            return_value=source_dir,
        ))
        self._mock_isdir = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.os.path.isdir",
            return_value=True,
        ))
        self._mock_isfile = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.os.path.isfile",
            return_value=False,
        ))
        self._mock_exists = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.os.path.exists",
            return_value=False,
        ))
        self._mock_makedirs = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.os.makedirs",
        ))
        self._mock_rmtree = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.shutil.rmtree",
        ))
        self._mock_copy2 = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.shutil.copy2",
        ))
        self._written_files = {}
        ...
        self._mock_json_dump = stack.enter_context(patch(
            "odoo.addons.talos.models.talos_sandbox.json.dump",
        ))
        return stack

    def test_no_sandbox_dir_raises(self):
        with patch(
            "odoo.addons.talos.models.talos_sandbox._load_dotenv", return_value={}
        ):
            with patch(
                "odoo.addons.talos.models.talos_sandbox._module_sandbox_dir",
                return_value=None,
                ...
                with self.assertRaises(UserError):
                    self.sandbox._prepare_workdir(
                        self.persona, "tok", 19000, 14000, 15432
                    )
    ...
    def test_writes_openclaw_config(self):
        with self._base_patches():
            self.sandbox._prepare_workdir(
                self.persona, "tok-abc", 19000, 14000, 15432
            )
    ...
    def test_writes_nginx_conf(self):
        with self._base_patches():
            self.sandbox._prepare_workdir(
                self.persona, "my-gw-token", 19000, 14000, 15432
            )
    ...
    def test_writes_override(self):
        ...
        write_calls = mock_fh.write.call_args_list
        written_content = "".join(c.args[0] for c in write_calls)
        self.assertIn("19000", written_content)
        self.assertIn("14000", written_content)

    def test_custom_port_in_origins(self):
        with self._base_patches():
            self.sandbox._prepare_workdir(
                self.persona, "tok", 20000, 14000, 15432
            )
        dump_calls = self._mock_json_dump.call_args_list
        config = dump_calls[0].args[0]
