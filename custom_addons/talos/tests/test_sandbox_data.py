...
class TestReadJsonlLocal(TalosTestCase):
    """Tests for TalosSandbox._read_jsonl_local."""
    ...
    @patch("odoo.addons.talos.models.talos_sandbox.os.path.isdir")
    ...
    @patch("odoo.addons.talos.models.talos_sandbox.os.listdir", return_value=[])
    @patch("odoo.addons.talos.models.talos_sandbox.os.path.isdir", return_value=True)
    def test_read_jsonl_local_no_jsonl_files(self, _isdir, _listdir):
        """Empty sessions dir → returns []."""
        sb = self._sandbox(docker_workdir="/tmp/fake")
        self.assertEqual(sb._read_jsonl_local(), [])

    @patch("odoo.addons.talos.models.talos_sandbox.os.path.getmtime", return_value=1.0)
    @patch("odoo.addons.talos.models.talos_sandbox.os.listdir",
    ...
    def test_read_jsonl_local_uses_persona_name(self, mock_isdir, _listdir, _getmtime):
        ...
        persona_name = sb.talos_id.persona_id.name
...
@tagged("post_install", "-at_install")
class TestReadJsonlK8s(TalosTestCase):
    """Tests for TalosSandbox._read_jsonl_k8s."""
    ...
    def test_read_jsonl_k8s_no_k8s(self):
        ...
        sb = self._sandbox()
        with patch(
            "odoo.addons.talos.models.talos_sandbox.TalosSandbox._read_jsonl_k8s",
    ...
    @patch("odoo.addons.talos.models.talos_sandbox.subprocess.run")
    ...
    def test_read_jsonl_k8s_namespace_from_config(self, mock_run):
        ...
        sb = self._sandbox()
        self._set_param("talos.k8s_namespace", "custom-ns")
        mock_run.return_value = MagicMock(
            returncode=0,
...
@tagged("post_install", "-at_install")
class TestQueryLitellmSpend(TalosTestCase):
    """Tests for TalosSandbox._query_litellm_spend."""
    ...
    @patch("odoo.addons.talos.models.talos_sandbox._load_dotenv", return_value={})
    ...
    @patch("odoo.addons.talos.models.talos_sandbox._load_dotenv",
    ...
    @patch("odoo.addons.talos.models.talos_sandbox.urllib.request.urlopen")
    @patch("odoo.addons.talos.models.talos_sandbox._load_dotenv",
           return_value={"LITELLM_MASTER_KEY": "sk-test"})
    ...
    def test_query_spend_k8s_mode_url(self, _dotenv, mock_urlopen):
        ...
        self._set_param("talos.ws_router_host", "ws.example.com")
        sb = self._sandbox(docker_litellm_port=14001)
    ...
    def test_query_spend_k8s_fallback_key(self, _dotenv, mock_urlopen):
        """Falls back to sk-talos-<token[:16]> when no master key."""
        self._set_param("talos.deployment_mode", "k8s")
        self._set_param("talos.ws_router_host", "ws.example.com")
        ...
        self.assertIn("sk-talos-" + token[:16], auth_header)
...
@tagged("post_install", "-at_install")
class TestExportTrajectoryToTask(TalosTestCase):
    """Tests for TalosSandbox._export_trajectory_to_task."""
    ...
    @patch("odoo.addons.talos.models.talos_sandbox.TalosSandbox._query_litellm_spend",
           return_value=(0, 0))
    @patch("odoo.addons.talos.models.talos_sandbox.TalosSandbox._read_session_jsonl",
           return_value=[])
