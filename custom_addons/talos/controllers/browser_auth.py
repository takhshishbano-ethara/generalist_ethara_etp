...
class TalosBrowserAuthController(http.Controller):
    ...
    def _browser_api_base(sandbox):
        ...
        if mode == "k8s":
            ...
            svc_name = "talos-sandbox-%s" % sandbox.id
            return (
                "http://%s.talos.svc.cluster.local:18789/browser-api"
                % svc_name
            )
    ...
    @http.route("/talos/browser/screenshot", type="json", auth="user")
    ...
    @http.route("/talos/browser/inject_cookies", type="json", auth="user")
    ...
    @http.route("/talos/browser/status", type="json", auth="user")
