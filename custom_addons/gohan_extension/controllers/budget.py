...
def _project_owns_batches(env, project_id):
    ...
    Batches reference ``gohan.job`` rows; a project is linked to that backend
    through its ``connected_table``. With no project filter, all batches are
    in scope.
    """
    if not project_id:
        return True
...
def _delivered_batch_count(env, project_id):
    ...
    if not _project_owns_batches(env, project_id):
        return 0
    return env["gohan.batch.delivery"].sudo().search_count(
...
def _scoped_batches(env, project_id):
    if not _project_owns_batches(env, project_id):
        return env["gohan.batch.delivery"].sudo().browse()
    return env["gohan.batch.delivery"].sudo().search(
...
class GohanBudgetController(http.Controller):

    @http.route(
        "/api/v1/gohan_ext/budget/info",
        type="http",
        auth="none",
        methods=["GET"],
        csrf=False,
    ...
    @validate_token
    def gohan_ext_budget_info(self, **kwargs):
        env = request.env
        if _user_role_tag(env) is None:
            return return_Response(
                message="You are not allowed to access Gohan budget.",
                status=403,
            )
        ...
        try:
            ...
            _logger.exception("gohan_ext_budget_info failed")
            return return_Response(
