...
class Valor(http.Controller):
    ...
    def method_get_jsonl_data(self, **params):
        try:
            ...
            if data:
                ...
                for i in data:
                    vals_list.append({
                        'task_id': i['id'] if 'id' in i and i['id'] else '',
                        'client_prompt': i['prompt'] if 'prompt' in i and i['prompt'] else '',
                        'client_response_a': i['response_a'] if 'response_a' in i and i['response_a'] else '',
                        'client_response_b': i['response_b'] if 'response_b' in i and i['response_b'] else '',
                        'is_randomized': i['is_randomized'] if 'is_randomized' in i and i['is_randomized'] else False
                    })
                valor_records = request.env['valor'].sudo().create(vals_list)
                return http.Response(
                    json.dumps({'success': True, 'message': 'Success', 'status': 200}),
                    content_type='application/json',
                    status=200
                )
            else:
    ...
    def method_get_llm_response(self, **params):
        try:
            ...
            valor_model = request.env['valor']
            valor_data = valor_model.sudo().search([('is_processed', '=', False)], order='id asc')
            for i in valor_data:
                i.sudo()._trigger_universal_background_task(gemini_api_key=gemini_api_key, genai_api_key=genai_api_key, openai_api_key=openai_api_key, dialog_id=dialog_id, model_1=model_1, model_2=model_2)
                # try:
                #     rejection_reason = ''
