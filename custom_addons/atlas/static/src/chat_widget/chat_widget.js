        ...
        if (session.ws && session.wsConnected) {
            ...
            if (turnId && widget) {
                try {
                    ...
                    console.warn("[AtlasChatWidget]", "Trajectory save on stop failed:", e);
                }
            }
            if (widget) {
...
async function _resolveLatestTurnId(sandboxId) {
    try {
        const history = await rpc("/atlas/chat/history", {
        ...
        console.warn("[AtlasChatWidget]", "Could not resolve latest turn:", e);
    }
    return null;
}
...
export class AtlasChatWidget extends Component {
    static template = "atlas.ChatWidget";
    static props = {
        sandboxId: Number,
        dockerStatus: String,
        ...
        ws.onclose = (ev) => {
            ...
            if (this._session.streaming && this._session.currentTurnId) {
                ...
                if (currentText) {
                    ...
                    rpc("/atlas/chat/save_response", params).catch(e => console.warn(LOG_PREFIX, "Partial save on disconnect failed:", e));
                }
                let msg = this._session.messages.findLast(m => m.pending);
                if (msg) {
                    msg.pending = false;
    ...
    async _loadHistory() {
        ...
        try {
            const result = await rpc("/atlas/chat/history", { sandbox_id: this.props.sandboxId });
    ...
    async _saveResponse(text, toolCalls = null, rawEvents = null) {
        ...
        try {
            ...
            await rpc("/atlas/chat/save_response", params);
        } catch (e) {
    ...
    async onSend() {
        ...
        try {
            ...
            const r = await rpc("/atlas/chat/create_turn", createParams);
            turnId = r.turn_id;
        ...
        try {
            ...
            if (qcResponse.error) {
                ...
                if (turnId) {
                    ...
                    rpc("/atlas/chat/save_qc", saveParams).catch(e => console.warn(LOG_PREFIX, "save_qc failed:", e));
                }
            } else {
    ...
    async _fetchBrowserScreenshot(widget) {
        try {
            const result = await rpc("/atlas/browser/screenshot", {
                sandbox_id: this.props.sandboxId,
            });
            if (result.image) {
    ...
    async onInjectCookies() {
        ...
        try {
            const result = await rpc("/atlas/browser/inject_cookies", {
                sandbox_id: this.props.sandboxId,
                cookies: raw,
    ...
    onFeedbackSatisfied(msgIndex) {
        ...
        if (msg.turnId) {
            rpc("/atlas/chat/save_feedback", { turn_id: msg.turnId, feedback: "satisfied" })
    ...
    onHintSend() {
        ...
        if (msg && msg.turnId) {
            rpc("/atlas/chat/save_feedback", { turn_id: msg.turnId, feedback: "unsatisfied", hint_text: hint })
                .catch(e => console.warn(LOG_PREFIX, "save_feedback failed:", e));
        }
