...
export class TaskProgressWidget extends Component {
    static template = "atlas.TaskProgressWidget";
    ...
    stepClass(step) {
        return `o_atlas_progress_step o_atlas_progress_step_${step.state}`;
    }

    connectorClass(conn) {
        return `o_atlas_progress_connector ${conn.filled ? "o_atlas_progress_connector_filled" : ""}`;
    }
}

export const taskProgressWidgetDef = { component: TaskProgressWidget };
registry.category("view_widgets").add("atlas_task_progress", taskProgressWidgetDef);
