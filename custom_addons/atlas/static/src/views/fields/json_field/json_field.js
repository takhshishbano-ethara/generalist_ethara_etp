...
function renderTrajectoryHtml(trajectory) {
    if (typeof trajectory === "string") {
        return `<pre class="atlas-json-block"><code>${escapeHtml(trajectory)}</code></pre>`;
    }
    const pretty = JSON.stringify(trajectory, null, 2);
    return `<pre class="atlas-json-block"><code>${syntaxHighlightJsonToHtml(pretty)}</code></pre>`;
}

export class AtlasJsonField extends Component {
    static template = "atlas.JsonField";
    static props = {
        ...standardFieldProps,
    ...
    _saveScrollPositions() {
        ...
        if (!root) return;
        const positions = [];
        const bodies = root.querySelectorAll(".atlas-json-entry-body");
    ...
    async _handleGoldenStatusChanged(payload) {
        ...
        try {
            ...
            console.warn("[AtlasJsonField] Failed to reload after golden status change:", e);
        }
    }
    ...
    async onQcEntry(index) {
        ...
        try {
            const resp = await rpc("/atlas/trajectory_qc", {
                record_id: recordId,
    ...
    async onGenerateTaskDesc(index) {
        ...
        try {
            const resp = await rpc("/atlas/generate_task_description", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
            });
    ...
    async onAbortQc(index) {
        ...
        try {
            await rpc("/atlas/abort_trajectory_action", {
                record_id: recordId,
                field_name: fieldName,
                entry_index: index,
                action_type: "qc",
...
export const atlasJsonField = {
    component: AtlasJsonField,
...
registry.category("fields").add("atlas_json", atlasJsonField);
