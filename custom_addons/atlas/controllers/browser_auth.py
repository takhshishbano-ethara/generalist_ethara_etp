...
class AtlasBrowserAuthController(http.Controller):
    ...
    def _browser_api_base(sandbox):
        ...
        if mode == "k8s":
            ...
            svc_name = "atlas-sandbox-%s" % sandbox.id
            return (
                "http://%s.atlas.svc.cluster.local:18789/browser-api"
                % svc_name
            )
    ...
    @http.route("/atlas/browser/screenshot", type="json", auth="user")
