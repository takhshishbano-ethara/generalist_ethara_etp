    ...
    def _build_summary(self):
        ...
        for tier, _label in Task._fields["difficulty"].selection:
            ...
            tier_stats[tier] = {
                ...
                "no_skill_pass_rate": (sum(1 for r in tier_runs if r.no_skill_binary) / len(tier_runs) * 100.0) if tier_runs else 0.0,
                "mean_delta": (sum(r.delta for r in tier_runs) / len(tier_runs) * 100.0) if tier_runs else 0.0,
                "per_model": {},
            }
            for m in models_recs:
                ...
                tier_stats[tier]["per_model"][m.key] = {
                    ...
                    "no_skill_pass_rate": (sum(1 for r in mruns if r.no_skill_binary) / len(mruns) * 100.0) if mruns else 0.0,
                    "mean_delta": (sum(r.delta for r in mruns) / len(mruns) * 100.0) if mruns else 0.0,
        ...
        return {
            ...
            "no_skill_pass_rate_overall": (sum(1 for r in runs if r.no_skill_binary) / total_runs * 100.0) if total_runs else 0.0,
            "mean_delta_overall": (sum(r.delta for r in runs) / total_runs * 100.0) if total_runs else 0.0,
            ...
            "skill_count": len({t.skill for t in tasks if t.skill}),
    ...
    def _build_tasks(self):
        ...
        for t in Task.search([]):
            rows.append({
                ...
                "no_skill_pass_rate": t.no_skill_pass_rate,
    ...
    def _build_runs(self):
        ...
        for r in Run.search([]):
            rows.append({
                ...
                "no_skill_score": r.no_skill_score,
                "no_skill_binary": r.no_skill_binary,
                "delta": r.delta,
                "recall": r.recall,
