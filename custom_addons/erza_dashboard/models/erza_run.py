...
class ErzaRun(models.Model):
    ...
    task_id = fields.Many2one("erza.task", required=True, ondelete="cascade", index=True)
    model_id = fields.Many2one("erza.model", required=True, ondelete="cascade", index=True)
    ...
    def seed_from_bundle(self):
        ...
        if not payload:
            _logger.warning("erza_dashboard: bundle payload missing; skipping seed")
            return False

        Model = self.env["erza.model"]
        Task = self.env["erza.task"]
