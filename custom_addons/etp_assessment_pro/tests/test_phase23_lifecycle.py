...
class _Base(TransactionCase):
    def setUp(self):
        ...
        self.Day = self.env["etp.assessment.pro.day"]
        self.DaySession = self.env["etp.assessment.pro.day.session"]
        ...
        self.Dimension = self.env["etp.assessment.pro.dimension"]
...
class TestPortalControllerSmoke(HttpCase, _Base):
    ...
    def test_full_walkthrough(self):
        ...
        resp = self.url_open(f"/pro_assessment/day/{token}")
        self.assertEqual(resp.status_code, 200)
        self.assertIn("Start", resp.text)

        resp = self.url_open(f"/pro_assessment/day/{token}/begin", data={})
        ...
        resp = self.url_open(
            f"/pro_assessment/day/{token}/submit", data=post_data)
        self.assertIn(resp.status_code, (200, 302))
        ...
        resp = self.url_open(
            f"/pro_assessment/day/{token}/finish", data={})
        self.assertIn(resp.status_code, (200, 302))
        sess.invalidate_recordset()
        self.assertIn(sess.state, ("submitted", "scored"))
