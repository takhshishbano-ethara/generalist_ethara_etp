...
```
 ...
 models/*.py ── assessment · evaluator · response · response.line
        ...
        │  service calls                         │  7 crons (data/cron.xml, all 1 min)
        ▼                                        ▼
...
**Automation / infra**
- [ ] All 7 crons registered, 1-min, correct model/method/lock (`data/cron.xml`)
- [ ] Advisory-lock keys unique; no aliasing; VERTEX_BEARER xact-scoped (`constants.py:32`)
...
- A closed-tab session past its deadline has no cron rescue; it stays `in_progress` until the candidate reopens or an admin intervenes. Confirm this is acceptable for the 200–300 cohort.
- `_cron_llm_auto_score` at ≤20/tick (1 min) implies ~1,200 candidates/hr max subjective throughput; size the grading window accordingly.
