...
def _get_or_create_key(ICP) -> bytes:
    ...
    env_key = os.environ.get("JAEGER_ENCRYPTION_KEY", "").strip()
    if env_key:
        return env_key.encode()
    with _key_cache_lock:
        if _cached_fernet_key is not None:
            return _cached_fernet_key
    key_str = ICP.get_param("jaeger.encryption_key", "")
    if key_str:
        _logger.error(
            "Jaeger: encryption key loaded from database — NOT SAFE FOR PRODUCTION. "
            "Set JAEGER_ENCRYPTION_KEY environment variable."
        )
    ...
    ICP.set_param("jaeger.encryption_key", key.decode())
    _logger.warning(
        "Jaeger: generated new Fernet encryption key and stored in database. "
        "For production, set JAEGER_ENCRYPTION_KEY environment variable."
    )
    with _key_cache_lock:
...
def _get_previous_key(ICP) -> bytes | None:
    env_prev = os.environ.get("JAEGER_ENCRYPTION_KEY_PREVIOUS", "").strip()
    if env_prev:
        return env_prev.encode()
    prev_str = ICP.get_param("jaeger.encryption_key_previous", "")
    if prev_str:
...
def _get_or_create_key_raw(cr) -> bytes:
    ...
    cr.execute(
        ...
        ("jaeger.encryption_key",),
    )
    row = cr.fetchone()
    if row and row[0]:
        _logger.error(
            "Jaeger: encryption key loaded from database (background thread) — "
            "NOT SAFE FOR PRODUCTION. Set JAEGER_ENCRYPTION_KEY environment variable."
        )
        result = row[0].encode()
        with _key_cache_lock:
    ...
    cr.execute(
        ...
        """,
        ("jaeger.encryption_key", key.decode()),
    )
    cr.commit()
    _logger.info("Jaeger: generated new Fernet encryption key (raw SQL).")
    with _key_cache_lock:
...
def _get_previous_key_raw(cr) -> bytes | None:
    ...
    cr.execute(
        "SELECT value FROM ir_config_parameter WHERE key = %s",
        ("jaeger.encryption_key_previous",),
    )
    row = cr.fetchone()
    if row and row[0]:
        return row[0].encode()
...
def decrypt_value(ICP, stored: str) -> str:
    ...
    try:
        ...
        _logger.error(
            "Jaeger: failed to decrypt config value. "
...
def decrypt_value_raw(cr, stored: str) -> str:
    ...
    try:
        ...
        _logger.error(
            "Jaeger: failed to decrypt config value (raw). "
            "Key may have changed or data is corrupted. Returning empty string."
        )
        return ""
