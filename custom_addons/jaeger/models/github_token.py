...
_HEALTH_CHECK_ADVISORY_LOCK_ID = 83927464
_ORPHAN_RECLAIM_ADVISORY_LOCK_ID = 83927465
_POOL_METRICS_ADVISORY_LOCK_ID = 83927466
...
_ALLOWED_UPDATE_COLUMNS = frozenset({
    ...
    "error_message", "leased_by_repo_id", "leased_at",
})


class JaegerGithubToken(models.Model):
    _name = "jaeger.github.token"
    ...
    leased_by_repo_id = fields.Many2one(
        "jaeger.repository", string="Leased By", index=True, ondelete="set null",
    )
    ...
    def init(self):
        self.env.cr.execute("""
            CREATE INDEX IF NOT EXISTS idx_jaeger_token_available
            ON jaeger_github_token (id)
            WHERE state = 'active' AND leased_by_repo_id IS NULL
        """)
        self.env.cr.execute("""
            ALTER TABLE jaeger_github_token SET (
    ...
    @staticmethod
    def lease_tokens(cr, repo_id, count=_LEASE_BATCH_SIZE):
        cr.execute("""
            UPDATE jaeger_github_token
            SET leased_by_repo_id = %s, leased_at = NOW() AT TIME ZONE 'UTC'
            WHERE id IN (
                SELECT id FROM jaeger_github_token
                WHERE state = 'active'
                  AND rate_limit_remaining > %s
                  AND (expires_at IS NULL OR expires_at > (NOW() AT TIME ZONE 'UTC')::date + INTERVAL '1 hour')
                  AND leased_by_repo_id IS NULL
                  AND (
            ...
            RETURNING id, token
        """, (repo_id, _MIN_REMAINING_FOR_LEASE, count))
        rows = cr.fetchall()
        if not rows:
            return []
        tokens = []
        for _, encrypted in rows:
            raw = JaegerGithubToken._decrypt_token_raw(cr, encrypted)
            if raw:
                tokens.append(raw)
        return tokens

    @staticmethod
    def release_tokens(cr, repo_id, token_summaries=None):
        if token_summaries:
            JaegerGithubToken._write_rate_limits(cr, repo_id, token_summaries)
        cr.execute("""
            UPDATE jaeger_github_token
            SET leased_by_repo_id = NULL, leased_at = NULL,
                last_health_check = NOW() AT TIME ZONE 'UTC'
            WHERE leased_by_repo_id = %s
        """, (repo_id,))

    @staticmethod
    def heartbeat_rate_limits(cr, repo_id, token_summaries):
    ...
    @staticmethod
    def _write_rate_limits(cr, repo_id, token_summaries):
        for tok_hash, info in token_summaries.items():
            ...
            cr.execute("""
                ...
                WHERE token_hash = %s AND leased_by_repo_id = %s
            """, (remaining, reset_dt, tok_hash, repo_id))
        ...
        for token_id, status, body, headers, prev_state, prev_fails in results:
            ...
            try:
                ...
                if prev_state == "quarantined" and new_fails >= _QUARANTINE_THRESHOLD:
                    cr.execute(
                        "SELECT last_health_check FROM jaeger_github_token WHERE id = %s",
                        (token_id,),
                    )
                    lhc_row = cr.fetchone()
    ...
    def _cron_token_expiry_alert(self):
        ...
        cr.execute("""
            SELECT state, COUNT(*) FROM jaeger_github_token GROUP BY state
        """)
        pool_status = dict(cr.fetchall())

        cr.execute("""
            SELECT COUNT(*) FROM jaeger_github_token WHERE expires_at IS NULL AND state != 'expired'
        """)
        unknown_count = cr.fetchone()[0]

        lines = [f"JAEGER TOKEN EXPIRY WARNING\n\n{total_expiring} tokens expiring within 7 days\n"]
        ...
        admin_group = self.env.ref("jaeger.group_jaeger_admin", raise_if_not_found=False)
        if not admin_group:
            _logger.warning("Jaeger Admin group not found, cannot send expiry alert")
            return
        partner_ids = admin_group.user_ids.mapped("partner_id").ids
        if partner_ids:
            self.env["mail.thread"].message_notify(
                body=body,
                partner_ids=partner_ids,
                subject="Jaeger Token Expiry Warning",
            )
    ...
    def _cron_token_orphan_reclaim(self):
        self.env.cr.execute(
            "SELECT pg_try_advisory_lock(%s)", (_ORPHAN_RECLAIM_ADVISORY_LOCK_ID,)
        )
        if not self.env.cr.fetchone()[0]:
            return
        try:
            self._run_orphan_reclaim()
        finally:
            self.env.cr.execute(
                "SELECT pg_advisory_unlock(%s)", (_ORPHAN_RECLAIM_ADVISORY_LOCK_ID,)
            )

    def _run_orphan_reclaim(self):
        cr = self.env.cr

        cr.execute("""
            UPDATE jaeger_github_token t
            SET leased_by_repo_id = NULL, leased_at = NULL,
                last_health_check = NOW() AT TIME ZONE 'UTC'
            FROM jaeger_repository r
            WHERE t.leased_by_repo_id = r.id
              AND (r.crawl_status IN ('done', 'failed')
                   OR t.last_heartbeat < (NOW() AT TIME ZONE 'UTC') - INTERVAL '60 minutes')
            RETURNING t.id, t.name, r.id AS repo_id,
        ...
        cr.execute("""
            ...
            SET leased_by_repo_id = NULL, leased_at = NULL,
                last_health_check = NOW() AT TIME ZONE 'UTC'
            WHERE leased_by_repo_id IS NOT NULL
              AND leased_by_repo_id NOT IN (SELECT id FROM jaeger_repository)
            RETURNING id
        """)
        ...
        normal = sum(1 for r in reclaimed if not r[3])
        forced = sum(1 for r in reclaimed if r[3])
        if reclaimed or orphaned:
            _logger.info(
                "Orphan reclaim: %d normal, %d forced, %d deleted-repo",
    ...
    def _cron_pool_metrics(self):
        self.env.cr.execute(
            "SELECT pg_try_advisory_lock(%s)", (_POOL_METRICS_ADVISORY_LOCK_ID,)
        )
        if not self.env.cr.fetchone()[0]:
            return
        try:
            self._run_pool_metrics()
        finally:
            self.env.cr.execute(
                "SELECT pg_advisory_unlock(%s)", (_POOL_METRICS_ADVISORY_LOCK_ID,)
            )

    def _run_pool_metrics(self):
        cr = self.env.cr
        cr.execute("""
            SELECT
                COUNT(*) AS total,
                ...
                COUNT(*) FILTER (WHERE leased_by_repo_id IS NOT NULL) AS leased,
        ...
        self.env["jaeger.pool.metrics"].create({
            "total_tokens": total,
            "active_count": active,
        ...
        self.env["jaeger.pool.metrics"].search([("timestamp", "<", cutoff)]).unlink()
    ...
    def action_export_tokens(self):
        ...
        cr.execute("""
            SELECT
                ...
                t.rate_limit_reset,
                t.expires_at,
                r.repo_url AS leased_by,
                t.last_health_check,
                t.consecutive_failure_count,
                t.imported_at,
                u.login AS imported_by_login,
                t.error_message
            FROM jaeger_github_token t
            LEFT JOIN jaeger_repository r ON r.id = t.leased_by_repo_id
        ...
        filename = f"jaeger_token_pool_{today}.xlsx"
